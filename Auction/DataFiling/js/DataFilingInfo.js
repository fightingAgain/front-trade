...
$(function() {
	...
	if(uid){
		...
		rollBack('1', 'view_fallback', uid, 'fzbzlgdth')
	}else{
		$('.show_fallback').hide();
	}
	//查看退回历史
	$('#backHistoryList').click(function(){
		openHistory(uid, 'fzbzlgdth')
	});
	...
	$.ajax({
		...
		success: function(result) {
			if(result.success) {
				resData = result.data;
				// 布局
				if (resData.projectPackage.examType == 0) {
					$('.examType0').show();
					$('.examType1').hide();
				} else {
					$('.examType0').hide();
				...
				$('.examType1, .examType0').parent('tr').each(function() {
					var tds = $(this).children('td:not(:hidden)');
					tds.each(function(index) {
						if (index === tds.length - 1) {
							$(this).attr('colspan', 4 - tds.length + 1);
						} else {
							$(this).attr('colspan', 1);
						}
					})
...
$("#btnBao").on('click', function() {
	var fliurl = config.AuctionHost + '/ProjectDateController/saveProjectDateFiles.do';
	if(WORKFLOWTYPE=='zlgd'){
		fliurl = config.AuctionHost + '/ProjectDateController/submitSelfProjectDate.do';
	}
...
$("#btnsubtime").on('click', function() {
	if(WORKFLOWTYPE=='zlgd'){
		...
		var fliurl = config.AuctionHost + '/ProjectDateController/submitProjectDate.do';
		if ($('[name=checkerId]').length > 0) {
			if ($("[name=checkerId]").val() == "") {
...
function sub(fliurl, titles) {
	...
	if(titles == "提交") {
		pare.checkId = $("[name=checkerId]").val()
	}
	for(var i = 0; i < AccessoryList.length; i++) {
