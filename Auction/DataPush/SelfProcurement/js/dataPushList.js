var urlBidResultList = top.config.AuctionHost + '/ProjectPushToDF/selfPage.do';//推送列表接口
            ...
            events: {
                'click .btnPush': function (e, value, row, index) {
                    ...
                    top.layer.open({
                        ...
                        title: '数据推送',
                        area: ['100%', '100%'],
                        content: cont + '?packageId=' + row.packageId + '&id=' + row.id + '&projectId=' + row.projectId + '&organNo=' + row.organNo + '&type=' + type + '&tenderType=' + row.tenderType,
                ...
                'click .btnSend': function (e, value, row, index) {
                    ...
                    layer.open({
                        ...
                        title: "异常推送",
                        area: ['100%', '100%'],
                        content: "bidPrice/DataPush/SelfProcurement/modal/pushAbnormalPublic.html?bidSectionId=" + row.packageId + "&projectId=" + row.projectId + '&tenderType=' + row.tenderType,
                        success: function (layero, index) {
                            var iframeWin = layero.find('iframe')[0].contentWindow;
                            iframeWin.passMessage(function () {
                                $('#table').bootstrapTable('destroy'); // 很重要的一步，刷新url！
                ...
                'click .btnHistory': function (e, value, row, index) {//历史记录
                    ...
                    layer.open({
                        ...
                        title: "历史记录",
                        area: ['1000px', '600px'],
                        content: "bidPrice/DataPush/SelfProcurement/modal/pushHistory.html?id=" + row.id + '&tenderType=' + row.tenderType,
                        success: function (layero, index) {
                            var iframeWin = layero.find('iframe')[0].contentWindow;
