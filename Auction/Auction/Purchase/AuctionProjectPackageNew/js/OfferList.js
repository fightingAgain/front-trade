...
var urlHistoryInfo = 'Auction/Auction/Purchase/AuctionProjectPackageNew/model/OfferHistoryInfo.html';
var urlOfferSetInfo = 'Auction/Auction/Purchase/AuctionProjectPackage/model/OfferSetInfo.html';
var urlOfferViewInfo = 'Auction/Auction/Purchase/AuctionProjectPackageNew/model/OfferViewInfo.html';
...
function AddFunction(value, row, index) { //把需要创建的按钮封装到函数中
	...
	if(row.isStopCheck == 1){
		return [
			'<button type="button" id="btnShow" class="btn-xs btn btn-primary" onclick=btnButton("' + row.projectId + '","' + row.id + '","' + createType + '","' + row.isAgent + '")><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span>查看历史</button>',
		].join("")
	}else{
		if(row.auctionStatus == '已结束') {
			...
			if(row.checkStatus == '未提交') {
				//已结束,未提交
				return [
					'<button type="button"  id="btnCommit" class="btn-xs btn btn-primary" onclick=btnButton("' + row.projectId + '","' + row.id + '","' + createType + '","' + row.isAgent + '")><span class="glyphicon glyphicon-ok" aria-hidden="true"></span>提交</button>',
				].join("")
			}
		...
		if(row.auctionStatus == '竞价中') {
			return [
				'<button type="button" id="btnView" class="btn-xs btn btn-primary" onclick=btnButton("' + row.projectId + '","' + row.id + '","' + createType + '","' + row.isAgent + '")><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span>查看</button>',
			].join("")
		}
		...
		if(row.auctionStatus == '未开始') {
			if(row.isShow == 0) {
				...
				return [
					'<button type="button" id="btnSet" class="btn-xs btn btn-primary" onclick=btnButton("' + row.projectId + '","' + row.id + '","' + createType + '","' + row.isAgent + '")><span class="glyphicon glyphicon-edit" aria-hidden="true"></span>设置</button>',
				].join("")
			}
		}
	}
...
function btnPrice($index) {
	...
	//	if(rowData[$index].auctionStartDate < nowSysDate) {
	var rowObj = rowData[$index]
	var tipStr = ''
	// auctionType 6 是清单式竞价 不加这段提示
	if (rowObj.auctionType != 6 && rowObj.supplierCount > 0 && rowObj.isFile == 0 && rowObj.offerSupplierCount < rowObj.supplierCount) {
			...
			if(result.auctionStatus == '已结束') {
				if(result.checkStatus == '未提交') {
					...
					if(result.auctionType == 6) {
						htmlUrl = 'Auction/Auction/Purchase/AuctionProjectPackage/model/detailListHistoryMsg.html?projectId=' + projectId + '&id=' + packageId + '&type=commit&createType=' + creType + '&isAgent=' + isAgent
					} else if(result.auctionType == 7) {
						htmlUrl = "Auction/Auction/Purchase/AuctionProjectPackage/model/singleBidHistory.html?projectId=" + projectId + "&id=" + packageId + "&tType=1&createType=" + creType + '&isAgent=' + isAgent
					} else {
						htmlUrl = urlHistoryInfo + '?projectId=' + projectId + '&id=' + packageId + '&type=commit&createType=' + creType + '&isAgent=' + isAgent
					}
				} else {
					var checkStatus = 0
					if(result.auctionType == 6) {
						htmlUrl = 'Auction/Auction/Purchase/AuctionProjectPackage/model/detailListHistoryMsg.html?projectId=' + projectId + '&id=' + packageId + '&createType=' + creType + '&isAgent=' + isAgent
					} else if(result.auctionType == 7) {
						htmlUrl = "Auction/Auction/Purchase/AuctionProjectPackage/model/singleBidHistory.html?projectId=" + projectId + "&id=" + packageId + "&tType=1&createType=" + creType + '&isAgent=' + isAgent
					} else {
						htmlUrl = urlHistoryInfo + '?projectId=' + projectId + '&id=' + packageId + '&createType=' + creType + '&isAgent=' + isAgent
					}
				}
						...
						success: function(response) {
							if(response.success) {
								if(response.data.offerCount < 3) {
									if(result.isStopCheck == 1 || response.data.supplierCountConfig == 1 || response.data.supplierCountConfig == 2) {
										layer.open({
											type: 2,
											title: '竞价采购历史详情',
											area: ['100%', '100%'],
										...
										layer.open({
											...
											resize: false, //是否允许拉伸
											// id: 'packageclass',
											content: 'Auction/Auction/Purchase/AuctionProjectPackage/model/notThreeSetUp.html?projectId=' + projectId + '&id=' + packageId + '&createType=' + creType + '&isAgent=' + isAgent + '&offerCount=' + response.data.offerCount + '&creType=' + creType + '&checkStatus=' + checkStatus,
											success: function(layero, index) {
												var body = layer.getChildFrame('body', index);
												var iframeWin = layero.find('iframe')[0].contentWindow;
											}
			...
			if(result.auctionStatus == '竞价中') {
				...
				if(result.auctionType == 6) {
					layer.open({
						...
						resize: false, //是否允许拉伸
						id: 'packageclass',
						content: 'Auction/Auction/Purchase/AuctionProjectPackage/model/detailListMsg.html' + '?projectId=' + projectId + '&id=' + packageId + '&createType=' + creType + '&isAgent=' + isAgent,
						success: function(layero, index) {
							var body = layer.getChildFrame('body', index);
							var iframeWin = layero.find('iframe')[0].contentWindow;
						}
