...
//	$("#projectCode").html(rowData.projectCode);
	...
	// warnLists(projectId, 'jj', false, false);
...
function loadAuctionFileCheckRecord(data) {
	var dataParam = { //获取projectid
		"projectId": projectId,
		"enterpriseType": '04' //0是采购人 1 是供应商
	}
	$.ajax({
		type: "get",
...
function fileHandTable(data) {
	...
	html += '<tr><th style="width:50px;text-align: center;">序号</th><th style="width:150px;">供应商名称</th><th style="width:100px;text-align: center;">社会信用代码</th><th style="width:200px;">竞价文件名称</th><th style="width:150px;text-align: center;">递交/撤回时间</th><th style="width:100px;text-align: center;">操作类型</th></tr>'
	for(var n = 0;n<data.length;n++){
		...
		for(var i = 0;i < data[n].simpleFiles.length;i++){
			...
			html += '<td style="width:120px;text-align: center;">'+(data[n].simpleFiles[i].rebackType==1?'递交':data[n].simpleFiles[i].rebackType==2?'撤回':'')+'</td>';
			html += '</tr>';
		};
