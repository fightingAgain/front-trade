...
$(function () {
	...
	if (keyId == 'undefined') {
		$("input[name='pushPlatform'][value=2]").attr("checked", "checked"); //公告发布媒
...
/*************            公告模板  --end                */
//但项目竞价
function singleOfferList(param) {
	$.ajax({
		type: "POST",
...
var saveMask;
function commitRst() {
	saveMask =parent.layer.load(0, {shade: [0.3, '#000000']});
	let fromData = JSON.stringify(commitData);
	let contentType = "application/json;charset=utf-8";
	let urlSave = urlInsertProjectBidResult;
	if(auctionType != 6 && auctionType != 7){
		urlSave = urlInsertProjectBidResult1;
		fromData = commitData;
		contentType = "application/x-www-form-urlencoded;charset=utf-8";
	}
	$.ajax({
...
function commit(){
	...
	let fromData = JSON.stringify(commitData);
	let contentType = "application/json;charset=utf-8";
	let urlSave = top.config.AuctionHost + "/ProjectBidResultController/JjcheckBlackList.do";
	if(auctionType != 6 && auctionType != 7){
		urlSave = top.config.AuctionHost + "/ProjectBidResultController/checkBlackList.do";
		fromData = commitData;
		contentType = "application/x-www-form-urlencoded;charset=utf-8";
	}
...
function checkBacklist(){
	var isPass = true;
	$.ajax({
		url: config.AuctionHost + "/ProjectBidResultController/checkBlackList.do",
		dataType: 'json',
		data: {
			projectId: projectId,
