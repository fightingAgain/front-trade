var urlBidResultList = top.config.AuctionHost + "/ProjectBidResultController/findProjectBidResult.do"; //竞价采购结果通知
var recallUrl = top.config.AuctionHost + "/WorkflowController/findWorkflowCheckeState.do"; // 撤回项目的接口
var pushHtml = 'Auction/Auction/Purchase/AuctionNotice/modal/pushInfo.html'; //数据推送
...
function checkDetail(packageId, type, index) {
	...
	if(rows.checkState == undefined && rows.createType != undefined && rows.createType == 1) {
		...
		return parent.layer.alert("温馨提示：竞价采购还未发布结果通知!")
	...
	top.layer.open({
		...
		resize: false,
		closeBtn: 1,
		content: 'Auction/Auction/Purchase/AuctionNotice/modal/AuctionResultInfo.html?special=' + type + '&keyId=' + rows.id+ '&id=' + rows.packageId+ '&projectId=' + rows.projectId,
	});
}
		...
		columns: [{
			...
			},
			{
				...
				formatter: function(value, row, index) {
					...
					var strChange = "<button onclick='checkDetail(\"" + row.packageId + "\",\"CHANGE\",\"" + index + "\")' class='btn btn-primary btn-xs'><span class='glyphicon glyphicon-edit'>变更</button>"
					if(row.createType != undefined && row.createType == 1) {
						...
						if(row.isAgent == 1) {
							...
							if(typeof(row.checkState) == 'undefined') {
								return "<button onclick='checkDetail(\"" + row.packageId + "\",\"RELEASE\",\"" + index + "\")' class='btn btn-primary btn-xs'><span class='glyphicon glyphicon-edit'>发布结果</button>"
							} else if(row.checkState == 0) {
								return "<button onclick='checkDetail(\"" + row.packageId + "\",\"RELEASE\",\"" + index + "\")' class='btn btn-primary btn-xs'><span class='glyphicon glyphicon-edit'>发布结果</button>" + (row.changeCount >0?strCancel:"")
							} else if(row.checkState == 1) {
								return "<button onclick='checkDetail(\"" + row.packageId + "\",\"view\",\"" + index + "\")' class='btn btn-primary btn-xs'><span class='glyphicon glyphicon-eye-open'>查看详情</button>&nbsp;&nbsp;<button onclick='updates(\"" + row.id + "\")' class='btn btn-warning btn-xs'><span class='glyphicon glyphicon-share'>撤回</button>"
							} else if(row.checkState == 3) {
								return "<button onclick='checkDetail(\"" + row.packageId + "\",\"RELEASE\",\"" + index + "\")' class='btn btn-primary btn-xs'><span class='glyphicon glyphicon-edit'>重新发布</button>" + (row.changeCount >0?strCancel:"")
							} else {
								//日产的，按包件的竞价项目推送
								if((row.auctionModel || row.auctionModel == 0) && row.auctionModel != 1 && row.organNo && row.organNo != '' && row.organNo != 0) {
	
									return "<button onclick='checkDetail(\"" + row.packageId + "\",\"view\",\"" + index + "\")' class='btn btn-primary btn-xs'><span class='glyphicon glyphicon-eye-open'>查看详情</button>" + (row.checkState == 2 && row.auctionType != 6 && row.auctionType != 7?strChange:"")
								} else {
									return "<button onclick='checkDetail(\"" + row.packageId + "\",\"view\",\"" + index + "\")' class='btn btn-primary btn-xs'><span class='glyphicon glyphicon-eye-open'>查看详情</button>" + (row.checkState == 2 && row.auctionType != 6 && row.auctionType != 7?strChange:"")
								}
							}
...
function pushDetail(pId, id) {
	top.layer.open({
		...
		// maxmin: false,
		resize: false,
		content: pushHtml + '?packageId=' + pId + '&projectId=' + id,
	});
}
