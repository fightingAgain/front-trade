...
function offer(e) {
	...
	if(data.auctionType <= 1 && data.auctionModel == 0){
		...
		if (data.priceReduction) offerMoney = minPrice - offerMoney * data.priceReduction;
	...
	if(data.priceReduction){
		if(isNaN(offerMoney) || !/^\+?[0-9][0-9]*$/.test(offerMoney)) return layer.alert('请输入正整数');
	}else{
		if(isNaN(offerMoney) || !/(^[1-9](\d+)?(\.\d{1,2})?$)|(^0$)|(^\d\.\d{1,2}$)/.test(offerMoney)) return layer.alert('请输入最多两位小数的正数金额');
	}		
	if (!isNaN(minPrice) && minPrice > 0 && offerMoney >= minPrice) return layer.alert('报价金额不得大于当前最低报价');
...
function view_2() {
	if(data.auctionType == 0){//自由竞价
		$('#offerDetail').append([
			...
			'</table>',
			'</div>',
			'<div class="col-md-7 no-padding" style="height: 500px;">',
			'<div style="border:1px solid #ddd;height:60%;overflow-y:auto;">',
			'<table id="offerLogsTable" class="table table-condensed">',
			'<caption style="padding-left: 20px;font-size: 14px;background-color:#3995C8;color:#000000;">报价记录</caption>',
			'<thead>',
			'<tr><th data-formatter="getIndex">序号</th>',
			'<th data-field="offerMoney">报价（元）</th><th data-field="subDate">报价时间</th></tr></thead>',
			'<tbody>',
			'</tbody></table></div>',
			'<div style="border:1px solid #ddd;height:40%;overflow-y:auto;">',
...
function view_3() {
	$('#offerDetail').append(['<table class="table">',
			...
			if(data.auctionOffer.isEliminated!=1){
				if(curStage!==0){				
					...
					rounds += 
						...
						'<tr><th>本轮报价(元)</th><td>' + 
						'<input  '+((round_data.myOfferMoney) && 'disabled')+' value="' + (round_data.myOfferMoney||'')+'">'+
						'</td>'+
		...
		data.isEnd ? ('<tr><td colspan="4">本项目竞价已结束！</td></tr><tr><td colspan="4">成功竞买号为：' + data.successOfferCode +
...
function liView(liData){
	$('#liTable').bootstrapTable({
		columns:[
		{field:"lun",title:'轮次'},
		{field:"jjqj",title:'竞价起价(元)'},
		{field:"blbj",title:'本轮报价(元)'},
		{field:"blbjdx",title:'本轮报价大写'},
		{field:"blbjzd",title:'本轮最低报价(元)'},
		],
