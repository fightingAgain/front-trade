...
function packagePriceData(){
	$("#projectId").val(packageData.projectId);
	etype = [];
	$.ajax({
	   	url:pricelist,
	   	type:'get',
						...
						if(packagePriceDataS[i].priceName=="项目保证金"){
							...
							if(packagePriceDataS[i].payMethod!==0){
								...
								if(packagePriceDataS[i].agentType){
									$('input[name="projectPrices[1].agentType"][value="'+ packagePriceDataS[i].agentType +'"]').prop("checked",true);
								}else{
									$('input[name="projectPrices[1].agentType"][value="0"]').prop("checked",true);
								}							
								$('input[name="projectPrices[1].bankAccount"]').val(packagePriceDataS[i].bankAccount||"");
								$('input[name="projectPrices[1].bankName"]').val(packagePriceDataS[i].bankName||"");
								$('input[name="projectPrices[1].bankNumber"]').val(packagePriceDataS[i].bankNumber||""); 
							}
						}
					}
...
function priceTable(){
	...
	for(var i=0;i<optionFeiYong.length;i++){						
		if(optionFeiYong[i].optionText=="平台服务费"){
			...
			$('input[name="projectPrices[0].packageId"]').val(packageData.id);
			$('input[name="projectPrices[0].projectId"]').val(packageData.projectId);
			...
			$('input[name="projectPrices[0].price"]').val(ptPrice);	
		}
		if($("input[name='isPayDeposit']:checked").val()==0){
			$('.isDepositShow').show();	
			if(optionFeiYong[i].optionText=="项目保证金"){
				$('input[name="projectPrices[1].payWay"]').val(1);
				$('input[name="projectPrices[1].priceId"]').val(optionFeiYong[i].id);
				$('input[name="projectPrices[1].packageId"]').val(packageData.id);
				$('input[name="projectPrices[1].projectId"]').val(packageData.projectId);
				$('input[name="projectPrices[1].priceName"]').val(optionFeiYong[i].optionText);	
			...
			$('input[name="projectPrices[1].price"]').val("");
			$('input[name="projectPrices[1].agentType"]').attr("checked",false);
			$('input[name="projectPrices[1].bankAccount"]').val("");
			$('input[name="projectPrices[1].bankName"]').val("");
			$('input[name="projectPrices[1].bankNumber"]').val(""); 
		}
		if($("input[name='isSellFile']:checked").val()==0){
			$(".isSellShow").show()
			...
			if(optionFeiYong[i].optionText=="竞价采购文件费"){
				...
				$('input[name="projectPrices[2].packageId"]').val(packageData.id);
				$('input[name="projectPrices[2].projectId"]').val(packageData.projectId);
			...
			$('input[name="projectPrices[2].payWay"]').val("");
			$('input[name="projectPrices[2].priceId"]').val("");
			$('input[name="projectPrices[2].packageId"]').val("");
			$('input[name="projectPrices[2].projectId"]').val("");
			$('input[name="projectPrices[2].priceName"]').val("");					
			$('input[name="projectPrices[2].payType"]').val("");
			$('input[name="projectPrices[2].payTime"]').val("");
			$('input[name="projectPrices[2].price"]').val("");		
		}	
	}
}
//切换是否需要保证金
$('input[name="isPayDeposit"]').on('change',function(){
	if($('input[name="isPayDeposit"]:checked').val() == 0){
		$('[name="projectPrices[1].agentType"]').eq(0).click();
	}
	priceTable()
})
//保证金缴纳方式
...
$('input[name="projectPrices[1].agentType"]').on('change',function(){
	findBank($(this).val());
})
//验证金额
...
function checkBank(){
	...
	if($('input[name="isPayDeposit"]:checked').val()==0){
		if($("#payMethod").val()!=0){
			if($("input[name='projectPrices[1].agentType']:checked").val()===undefined){
	...
	if($('input[name="isSellFile"]:checked').val()==0){
		...
		if(filesData.length==0){	
			parent.layer.alert("当需要缴纳竞价采购文件费时，竞价采购文件必须上传");		
			fag="请上传竞价采购文件";	
			return fag;
		}
	};
