...
function auctionTypeBtn(type){
    ...
    if(type==0){
        ...
        html+='<td class="auctionModelShow" style="text-align: left;">'
        ...
        // html+='<td style="text-align: left;" colspan="3">'
        // html+='<input  class="form-control priceNumber" id="noTaxBudgetPrice" name="noTaxBudgetPrice"/>'
        // html+='</td>'
        // html+='</tr>'
        html+='<tr>'
        html+='<td class="th_bg">竞价类型<i class="red">*</i></td>'
   ...
   //清单报价附件初始化
    if(type==6){
		...
		$("#taxBudgetPriceTd").removeAttr('dataType');
        $("#noTaxBudgetPriceTd .red").show(); //预算价(含税)(元)不必填
        $('.operation_ades').hide()
		...
		if(isDfcm){
			...
			$("#taxBudgetPriceTd").attr('dataType','*');
		}
        $("#noTaxBudgetPriceTd .red").hide(); //预算价(含税)(元)不必填
        $('.operation_ades').show()
    }
