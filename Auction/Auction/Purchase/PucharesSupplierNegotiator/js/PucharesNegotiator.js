...
$(function() {
	...
	$("#btnSearch").on('click', function() {
		$('#PucharesNegotiator').bootstrapTable('refresh', {
			url: searchUrl
		});
	});
...
window.operateEvents = { //添加一个按钮对应的事件
	"click #btnCheck": function(e, value, row, index) {
		...
		layer.open({
			...
			title: '价格谈判',
			area: ['1000px', '600px'],
			content: 'Auction/Auction/Purchase/PucharesSupplierNegotiator/pPriceNegotiator.html?'+"createType="+createType+'&projectId='+row.projectId+'&id='+row.packageId,
	...
	"click #btnUnderCheck": function(e, value, row, index) {
		...
		layer.open({
			...
			area: ['1000px', '600px'],
			id: "negotiators",
			content: 'Auction/Auction/Purchase/PucharesSupplierNegotiator/underline.html?projectId='+row.projectId+'&id='+row.packageId,
			success: function(layero, indexs) {
				var body = layer.getChildFrame('body', indexs);
				var iframeWin = layero.find('iframe')[0].contentWindow;
	...
	"click #btnNegotiator": function(e, value, row, index) {
		...
		layer.open({
			...
			resize: false, //是否允许拉伸
			content: 'Auction/Auction/Purchase/PucharesSupplierNegotiator/NegotiatorMethod.html?projectId='+row.projectId+'&id='+row.packageId,
			success: function(layero, indexs) {
	...
	"click #btnNegotiatorMingXi": function(e, value, row, index) {
		...
		layer.open({
			...
			resize: false, //是否允许拉伸
			id:'negoCenter',
			content: 'Auction/Auction/Purchase/PucharesSupplierNegotiator/NegotiatorCenter.html?'+"createType="+createType+'&projectId='+row.projectId+'&id='+row.packageId,
			success: function(layero, indexs) {
				var body = layer.getChildFrame('body', indexs);
				var iframeWin = layero.find('iframe')[0].contentWindow;
			}
...
function AddFunction(value, row, index) { //把需要创建的按钮封装到函数中
	if(row.negotiationState == '0' && row.negotiationType == '0') {
		if(row.auctionModel != undefined  && row.auctionModel == '1'){
			return [
				//未谈判
				'<button type="button" id="btnNegotiatorMingXi" class="btn-xs btn btn-primary"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span>查看</button>',		
			].join("")
		}else{
			//明细
			return [
	...
	if(row.negotiationState == '1') {
		if(row.auctionModel != undefined  && row.auctionModel == '1'){
			return [
				//未谈判
				'<button type="button" id="btnNegotiatorMingXi" class="btn-xs btn btn-primary"><span class="glyphicon  glyphicon-edit" aria-hidden="true"></span>谈判</button>',
			...
			if(row.createType != "undefined" && row.createType == 1 ){//当前项目不为自己发布的项目时
				return [
					...
					'<button type="button" id="btnView" class="btn-xs btn btn-primary"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span>查看</button>',
	
				].join("")
			}else{
				return [
					//未谈判
					'<button type="button" id="btnNegotiator" class="btn-xs btn btn-primary"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span>谈判</button>',
