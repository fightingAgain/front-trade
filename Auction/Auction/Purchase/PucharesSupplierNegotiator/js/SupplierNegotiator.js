...
window.operateEvents = { //添加一个按钮对应的事件
		...
		if(GetTime(nowDate)>GetTime(row.endDate) && row.isAccept != '0' && row.isAccept != '1'){
			...
			layer.confirm('已超过价格确认截止时间,默认为拒绝谈判', {
				...
				layer.open({
					...
					content: '0502/Auction/PucharesSupplierNegotiator/sPriceNegotiator.html?type=check'
						//确定按钮
						,
	...
	"click #btnAnswer": function(e, value, row, index) {
		...
		layer.open({
			...
			resize: false, //是否允许拉伸
			content: '0502/Auction/PucharesSupplierNegotiator/sPriceNegotiator.html?type=answer',
			success: function(layero, index) {
				var body = layer.getChildFrame('body', index);
				var iframeWin = layero.find('iframe')[0].contentWindow;
				iframeWin.show(row);
