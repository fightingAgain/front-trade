...
$(function() {
	...
	$("#btnHold").on('click', function() {
		...
		if($("#agencyAddress").val().length > 60) {
			parent.layer.alert("代理机构地址过长")
			return;
		}
		if($("#agencyLinkmen").val().length > 20) {
			parent.layer.alert("代理机构联系人过长")
			return;
		}

		if(!(/(^1\d{10}$)|(^\d{3}-\d{8}$)|(^\d{4}-\d{7}$)/.test($("#agencyTel").val()))) {
			parent.layer.alert("代理机构联系电话格式错误");
			$(this).val("");
	...
	$("#btnSave").on('click', function() {
		...
		if($("#projectCode").val() != '' && $("#projectCode").val().length > 70) {
			parent.layer.alert("采购项目编号过长")
			return;
		};
		if($("#programmeName").val() != '' && $("#programmeName").val().length > 70) {
...
function Purchase() {
	...
	PurchasersDataEnterprojectId = purchaseaData.purchaserId;
...
function nextNotice(projectId) {
	parent.layer.open({
		...
		area: ['1100px', '650px'],
		id: 'packageclass',
		content: 'Auction/Auction/Agent/AuctionPurchase/model/AuctionInfo.html?projectId=' + projectId
			//,btn: ['关闭']
			,
...
function chosePucharse() {
	...
	parent.layer.open({
		...
		title: '选择采购人',
		area: ['1100px', '650px'],
		content: 'Auction/Auction/Agent/AuctionPurchase/model/Purchasers.html'
			//,btn: ['关闭']
			,
		maxmin: false //该参数值对type:1和type:2有效。默认不显示最大小化按钮。需要显示配置maxmin: true即可
			,
...
function showPurchse(PurchasersData) {
	PurchasersDataEnterprojectId = PurchasersData.danweiguid
	$("#purchaserId").val(PurchasersData.danweiguid);
	$("#purchaserName").text(PurchasersData.legalName);
	$("#purchaserAddress").text(PurchasersData.legalUnitAddress);
	$("#ppurchaserName").val(PurchasersData.legalName);
	$("#ppurchaserAddress").val(PurchasersData.legalUnitAddress);
	$("#ppurchaserLinkmen").val(PurchasersData.legalContact);
	$("#ppurchaserTel").val("");
