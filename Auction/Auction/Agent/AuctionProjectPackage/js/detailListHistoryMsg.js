...
var getSupplierBargainUrl = top.config.AuctionHost + "/AuctionSfcOfferController/getMXAllSupplieres.do"; //清单竞价按明细 查询可议价的所有供应商信息
...
$(function () {
	//监督角色没有底部按钮(控制台)和审核人
	if(getUrlParam('isJD') && getUrlParam('isJD') == 'isJD'){
		$('.notJD').remove();
		$('#resultAuditListTable').remove();
	};
	...
	$.ajax({
		...
		success: function (response) {
			if (response.success) {
				...
				if (dataResult) {
					//sfcBarginStatus 议价操作状态  0  保存    1 提交   2 同意  3.拒绝  4 无需议价
...
function openDowload(path, name) {
	if (name) {
		...
		var url = config.FileHost + "/FileController/download.do" + "?fname=" + "" + "&ftpPath=" + path;
		window.location.href = $.parserUrlForToken(url);
	}
...
function checkDetail(packageId, type, index) {
	...
	top.layer.open({
		...
		resize: false,
		closeBtn: 1,
		content: 'Auction/Auction/Agent/AuctionProjectPackage/model/bargainMsg.html?id=' + packageId + '&sfcSuppid=' + rows.supplierEnterpriseId + '&time=' + rows.endTime,
	});
}
...
$("#btn_bargain").click(function () {
	parent.layer.open({
		...
		closeBtn: 1,
		id:"bargainIfram",
		content: 'Auction/Auction/Agent/AuctionProjectPackage/model/bargain.html?id=' + packageId + '&type=' + auctionModelType + '&auctionType=' + auctionType,
...
var isEndCheck = true; //是否会签完成 true是完成，false没完成
var isHasFail = false;
var isHasBid = false;

//findWorkflowCheckerAndAccp();
	...
	$.ajax({
		...
		success: function (response) {
			if (response.success) {
				...
				if(Number(response.data) > 0 && $("#btn_submit").attr("data-type")==0){
					submitState = true;
					$("#btn_submit").hide();
				}
...
$("#btn_submit").click(function(){
	...
	// if(btnType == 1){
		var isGo = true;
		console.log(barginList)
		for(var i = 0;i<barginList.length;i++){
			if(barginList[i].sfcBarginStatus == 1 ){
				isGo = false;
			}
		}
	// }
	if(isGo){
		...
		top.layer.confirm('供应商议价未完成，是否确认发起会签？',{title:'询问'}, function(index){
			top.layer.close(index);
			if (workflowLevel == 0) {
			    top.layer.confirm('温馨提示：该流程未设置审批节点，您是否继续提交？', function () {
			        saveForm(btnType);
...
function saveForm(btnType){
	var arr = {packageId:packageId, boFailAuctiones:[], boAuctionOfferItemes:[]};
	if(btnType == 0){
		arr.identifier = 0;
	}
	var flag = false;
	$("#detailPackageList tbody tr").each(function(){
		...
		var col = $(this).find("input:radio:checked").closest("label").attr("data-index");
