...
/**
 *
 * @returns number - 正值，负值，零
 */
function compareDateBS() {
	...
	switch(argLen) {
		...
		default:
			throw '至少需要一个字符串时间，格式“YY-MM-DD HH:mm:ss”';
	}
};
...
$(function() {
	...
	$('#btn_submit').click(function() {
		...
		$.extend(params,mediaEditor.getValue())
		// 时间处理
		var timeObj = params;
		['noticeEndDate'].forEach(function(key) {
...
function showPurchse(Data) {
	...
	$("#isAgentText").html(purchaseaData.project[0].isAgentText);
	...
	if(projectSourceCount != "undefined" && projectSourceCount != null && projectSourceCount != "0") {
		...
		$("#packageSourceCount").html('(第' + projectSourceCount + '次重新竞价)');
	...
	** 2、多轮竞价/不限轮次-设置竞价起始价-设置降价幅度
	...
	if((data.auctionType == 4 || data.auctionType == 2 || data.auctionType == 3) || (data.auctionType == 0 && data.auctionModel == 0)){
		...
		if(data.isPrice == 0){//设置竞价起始价
			...
			$('.isPriceText').removeAttr('colspan');
		}else{//不设置竞价起始价
