...
function priceTable(){
	...
	for(var i=0;i<optionFeiYong.length;i++){						
		if(optionFeiYong[i].optionText=="平台服务费"){
			...
			$('input[name="projectPrices[0].projectId"]').val(data1.projectId);
		...
		if($("input[name='isPayDeposit']:checked").val()==0){
			...
			if(optionFeiYong[i].optionText=="项目保证金"){
				...
				$('input[name="projectPrices[1].packageId"]').val(data1.id);
				$('input[name="projectPrices[1].projectId"]').val(data1.projectId);
		...
		if($("input[name='isSellFile']:checked").val()==0){
			...
			if(optionFeiYong[i].optionText=="竞价采购文件费"){
				...
				$('input[name="projectPrices[2].projectId"]').val(data1.projectId);
