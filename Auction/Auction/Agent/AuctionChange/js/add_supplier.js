...
var isPublics="";
var classificaCodes
var purchaserIds=JSON.parse(sessionStorage.getItem('purchaserId'));
function du(isPublic,projectIds,classificaCode){	
	projectId=projectIds;
	isPublics=isPublic;
	classificaCodes=classificaCode;
	if(isPublic==3){
		supplierListUrl=top.config.AuctionHost + "/SupplierDataTypeSetController/findEnterprisePageListByCodes.do";
	}else{
		supplierListUrl=config.Syshost+'/EnterpriseController/page.do'
	}
...
$("#evenTrue").on('click',function(){
    ...
    for(var i=0;i<newData.length;i++){
    	para["projectSuppliers["+i+"].supplierId"]= newData[i].supplierId;    	
    	para["projectSuppliers["+i+"].enterpriseName"]= newData[i].enterprise.enterpriseName;    	
    }
    $.ajax({
    	type:"post",
    	url:top.config.AuctionHost+"/ProjectSupplierController/saveProjectSupplier.do",
    	async:true,
    	data:para,
...
　　　　　　	checkboxed.indexOf(v.id) == -1 ? checkboxed.push(v.id) : -1;
			newData.push({supplierId:v.id,enterprise:v,isAppend:1});
　　　　	});
		...
		$.each(datas,function(i,v){		        	
			...
			for(var m=0;m<data.length;m++){
				if(data[m].supplierId==v.id){
					newData.splice(m,1);    //删除取消选中行
				}
			}	
		});
	};
	newData=newData.unique1();
};
