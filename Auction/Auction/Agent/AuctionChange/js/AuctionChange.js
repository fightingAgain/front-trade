...
/// 表格初始化
var purchaseaddurl = 'Auction/Auction/Agent/AuctionChange/model/addAuctionProject.html' //添加路径
var auditPurchase = 'Auction/Auction/Agent/AuctionChange/model/auditPurchase.html' //审核路径
var editPurchase = 'Auction/Auction/Agent/AuctionChange/model/editAuctionProject.html' //编辑路径
		...
		columns: [{
			...
			formatter: function(value, row, index) {
				if(row.projectSource == 1) {
					...
					if(count){
						var projectName = ' <div>' + row.projectName + '<span class="text-danger">(第'+count+'次 重新竞价)</span></div>';
					}else{
						var projectName = ' <div>' + row.projectName + '<span class="text-danger">(重新竞价)</span></div>';
					}
