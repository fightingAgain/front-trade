...
var tenderType = 1; //0是询价采购  1是竞价采购
	...
	$('form').on('click', '#noticeEndDate', function(){
		...
		if(oldNoticeStartDate){
			if(NewDate(oldNoticeStartDate)>NewDate(nowSysDate)){	
				noticeMin=oldNoticeStartDate.substring(0,10)
			}else{
	...
	$.ajax({
		...
		success: function(res) {
			if(res.success == true) {
				...
				$('input[name="noticeStartDate"]').val(res.data.noticeStartDate);
