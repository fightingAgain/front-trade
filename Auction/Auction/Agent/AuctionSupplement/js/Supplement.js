...
var view='Auction/Auction/Agent/AuctionSupplement/model/view_model.html';
function initTable() {
		...
		,{
			...
			formatter:function(value, row, index){	
				...
				var State='<div class="btn-group">'
						+'<button btn class="btn-xs btn btn-primary"onclick=showProjectSupplementInfo(\"'+row.id+'\",\"'+(row.enterpriseId || "")+'\",\"'+(row.pEnterpriseId||"")+'\",\"'+row.createType+'\")>'
...
function queryParams(params) {	
    return {		 
			...
			tenderType:1,//
			enterpriseType:"02", //0是采购人 1是供应商身份
...
function showProjectSupplementInfo(projectId,enterpriseId,pEnterpriseId,createType){
	...
	layer.open({
		...
		maxmin: true,
		resize: false,
		content:view + "?projectId="+projectId+"&tenderType=2&enterpriseType=02&createType="+createType,
