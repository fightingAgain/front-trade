...
var urlSingleOriginatorOfferesList =top.config.AuctionHost +  "/AuctionSfcSingleOfferesController/getSingleOriginatorOfferesList.do"
//查询采购审核
var workflowLevel = 0;
//是否为提交过审核人
...
//	setPurchaseFileDownloadDetail(packageId);
	buyFileDetail(packageId, true);  //查看文件购买记录
	warnLists(packageId, 'jj', true, true,null,projectId,'1','1');
	detailItem();

	if(createType == 1 || offerData.isStopCheck == 1) {
		...
		success: function(res) {
			if(res.success) {
				...
				if(offerData.offerEndDate){
					$('.offerEndDate').html('本次竞价于【'+offerData.offerEndDate+'】终止'+(offerData.isStopCheck == 1?'(项目失败)':'')).addClass('offerEndDateBox');
				}else{
					if(offerData.isStopCheck == 1){
						$('.offerEndDate').html('本次竞价已项目失败')
					}else{
				...
				$("td[id]").each(function() {
					...
					if(this.id=='projectName' || this.id=='projectCode' || this.id=='packageCode' || this.id=='packageName' ){
						$(this).html('<span style="white-space: normal;">'+data[this.id]+'</span>');
					}
					...
					if(this.id == "projectName") {
						if (data.projectSource > 0) {
							$(this).html('<span style="white-space: normal;">'+data[this.id] + '<span class="red">(重新竞价)</span>'+'</span>');
						}
					}
				});
				if(workflowLevel&&workflowLevel>0){
					if(isEndCheck && offerData.isStopCheck != 1){
				...
				if(data.isShowPrice || data.isShowPrice == 0){
...
function freeDetailRK() {
	$("#freeDetailRK").bootstrapTable({
		...
		onCheck: function(row, ele) {
			...
			if(offerData && offerData.details && offerData.details[row.id] && offerData.details[row.id].offerItems !== undefined && offerData.details[row.id].offerItems !== null && offerData.details[row.id].offerItems !== "") {
				var or = [];
				for(let i = 0; i<offerData.results.length; i++){
					if(offerData.results[i].packageDetailedId == row.id){
						or.push(offerData.results[i]);
					}
				}
				$("#freeDetailRank").bootstrapTable("load", or);
				detailedId = row.id;
				if(status == '已提交审核' || status == '审核通过') {
					// $('#freeDetailRank').bootstrapTable('hideColumn', 'bbb');
					// $('#freePackageRank').bootstrapTable('hideColumn', 'aaa');
				...
				$("#freeDetailRank").bootstrapTable("load", []);
			}
		},
	...
	$("#freeDetailRD").bootstrapTable({
		...
		onCheck: function(row, ele) {
			...
			if(offerData && offerData.details && offerData.details[row.id] && offerData.details[row.id].offerItems !== undefined && offerData.details[row.id].offerItems !== null && offerData.details[row.id].offerItems !== "") {
				...
				$("#freeDetailRecord").bootstrapTable("load",[]);
			}
		},
		onPostBody: function() {
...
function freeDetailRank() {
	...
	// $("#freeDetailRank").bootstrapTable("load", offerData.details);
}
...
function roundItem() {
	$("#roundItem").bootstrapTable({
		...
		columns: [{
			...
			},
			{
				...
				align: "center",
				formatter: function(value, row, index) {
					return "第" + sectionToChinese(value) + "轮报价";
				}
			}, {
	...
	if(offerData && offerData.offerRounds){
...
function selectClick(e) {
	...
	layer.open({
		...
		content: '../../AuctionProjectPackage/model/getResultAuditList.html',
