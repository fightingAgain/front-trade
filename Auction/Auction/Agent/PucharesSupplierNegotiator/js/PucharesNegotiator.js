...
window.operateEvents = { //添加一个按钮对应的事件
	"click #btnCheck": function(e, value, row, index) {
		...
		layer.open({
			...
			title: '价格谈判',
			area: ['1000px', '600px'],
			content: 'Auction/Auction/Agent/PucharesSupplierNegotiator/pPriceNegotiator.html?'+"createType="+createType+'&projectId='+row.projectId+'&id='+row.packageId,
	...
	"click #btnUnderCheck": function(e, value, row, index) {
		...
		layer.open({
			...
			area: ['1000px', '600px'],
			id: "negotiators",
			content: 'Auction/Auction/Agent/PucharesSupplierNegotiator/underline.html?projectId='+row.projectId+'&id='+row.packageId,
			success: function(layero, indexs) {
				var body = layer.getChildFrame('body', indexs);
				var iframeWin = layero.find('iframe')[0].contentWindow;
	...
	"click #btnNegotiator": function(e, value, row, indexv) {
		...
		layer.open({
			...
			resize: false, //是否允许拉伸
			content: 'Auction/Auction/Agent/PucharesSupplierNegotiator/NegotiatorMethod.html?projectId='+row.projectId+'&id='+row.packageId,
			success: function(layero, indexs) {
				var iframeWin = layero.find('iframe')[0].contentWindow;
	...
	"click #btnNegotiatorMingXi": function(e, value, row, index) {
		...
		layer.open({
			...
			resize: false, //是否允许拉伸
			id:'negoCenter',
			content: 'Auction/Auction/Agent/PucharesSupplierNegotiator/NegotiatorCenter.html?'+"createType="+createType+'&projectId='+row.projectId+'&id='+row.packageId,
			success: function(layero, indexs) {
				var body = layer.getChildFrame('body', indexs);
				var iframeWin = layero.find('iframe')[0].contentWindow;
