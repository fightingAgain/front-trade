...
$(function () {
	...
	$('.search-btn').click(function () {
		$('#projectPackage').bootstrapTable('destroy');
		initTable();
	})
...
function initTable() {
	$('#projectPackage').bootstrapTable({
		...
		responseHandler: function (res) {
			supplierId = res.model.enterpriseId;
			return res;
		},
		columns: [
			[{
			...
			},
			{
				...
				width: '200',
				formatter: function (value, row, index) {
					return value ? '<span style="color:' + (value == "已过期 - 未报价" || value == "未报价" ? "red" : "green") + '">' + value+(row.isStopCheck==1?'（项目失败）':'') +
						'</span>' : '';
				}
			}, {
				field: 'isAccept',
			...
			},
			{
				...
				events: {
					'click .btn-opera': function (e, value, row, index) {
						...
						detailListType = row.auctionType
						openIfiame(row.projectId, row.id, 1, detailListType)
					}
				},
				formatter: function (value, row, index) {
					var now = new Date();
					if (row.isOpenQuotation == 0 && row.manual == 1) {
						return '<button class="btn btn-primary btn-xs btn-opera">报价</button>';
					} else {
...
function openOrder(packageId, uid) {
	if (detailListType == 6) {
		var index = layer.open({
			...
			area: ['100%', '100%'],
			resize: false, //是否允许拉伸
			content: $.parserUrlForToken('Auction/Auction/Supplier/AuctionProjectPackage/model/detailListInfo.html') + '#' + packageId
		});
	} else if (detailListType == 7) {
		var index = layer.open({
			type: 2,
			title: "单项目报价",
			area: ['100%', '100%'],
			resize: false, //是否允许拉伸
			content: $.parserUrlForToken('Auction/Auction/Supplier/AuctionProjectPackage/model/singleBidding.html') + '#' + packageId
		});
	} else {
		var index = layer.open({
			type: 2,
			...
			area: ['100%', '100%'],
			resize: false, //是否允许拉伸
			content: $.parserUrlForToken('Auction/Auction/Supplier/AuctionProjectPackageNew/model/view_Package.html') + '&packageId=' + packageId + '&supplierId=' + supplierId
		});
	}
}