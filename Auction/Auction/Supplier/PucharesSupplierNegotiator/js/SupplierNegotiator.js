...
window.operateEvents = { //添加一个按钮对应的事件
		...
		if(GetTime(nowDate)>GetTime(row.endDate) && row.isAccept != '0' && row.isAccept != '1'){
			...
			layer.confirm('已超过价格确认截止时间,默认为拒绝谈判', {
				...
				layer.open({
					...
					area: ['1200px', '600px'],
					resize: false, //是否允许拉伸
					content: 'Auction/Auction/Supplier/PucharesSupplierNegotiator/sPriceNegotiator.html?type=check' + '&isAgent='+row.isAgent
						//确定按钮
						,
	...
	"click #btnAnswer": function(e, value, row, index) {
		...
		layer.open({
			...
			resize: false, //是否允许拉伸
			content: 'Auction/Auction/Supplier/PucharesSupplierNegotiator/sPriceNegotiator.html?type=answer' + '&isAgent='+row.isAgent,
			success: function(layero, index) {
				var body = layer.getChildFrame('body', index);
				var iframeWin = layero.find('iframe')[0].contentWindow;
				iframeWin.show(row);
					...
					formatter:function(value, row, index){
						if(row.negotiation.isPublic>1){
							if(row.negotiation.projectSource==1){
								return row.negotiation.projectName+'<span class="text-danger" style="font-weight: bold;">(重新竞价)</span><span class="text-danger" style="font-weight: bold;">(邀请)</span>'
							...
							if(row.negotiation.projectSource==1){
								return row.negotiation.projectName+'<span class="text-danger" style="font-weight: bold;">(重新竞价)</span>'
							}else if(row.negotiation.projectSource==0){
