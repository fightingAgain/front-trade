var detailUrl = top.config.AuctionHost + '/BidExceptionController/findBidExceptioInfo.do'; // 详情
 ...
 */
			...
			if(arr){
				...
				for(var key in arr){
					if(key == "fileList"){
						...
						$("#exceptionTypes").html(arr[key] == 1 ? "项目终止" : "项目失败公示")
					} else if(key == "foreign"){
 ...
 function fileTable(data){
 	...
 	$('#fileTables').bootstrapTable("load", data); //重载数据
 };
