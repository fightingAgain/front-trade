...
var bidHistoryCheckId = getUrlParam('id'); //预审采购文件还是询比采购文件
...
$(function() {
	...
	findBusinessPriceSet(packageInfo.checkPlan, bidHistoryCheckId);
	PackageCheckList(0);
	offerFileList();
});
	...
	if(packageCheckListInfo.length > 0) {
		...
		if(packageCheckListInfo[_index].checkType == 4) {
			strHtml += '<tr>' +
				...
				'<td style="text-align: left;">' + ['参与竞价供应商最低报价', '未淘汰供应商最低报价', '参与报价供应商最低报价'][packageCheckListInfo[_index].bidStartPriceFrom] +
				'</td>' +
				'</tr>';
		}
		if(packageCheckListInfo[_index].checkType == 4) {
