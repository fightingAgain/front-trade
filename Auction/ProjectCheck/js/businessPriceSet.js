function findBusinessPriceSet(_d,uid){
	$.ajax({
	   	url:config.AuctionHost+'/BusinessPriceSetController/findBusinessPriceSet.do',
	   	type:'get',
	   	dataType:'json',
	   	async:false,
	   	...
	   	success:function(data){	
	   		if(data.success){
				if(data.data){
					$(".isBusiness").show();
					var businessPriceSetData=data.data;
					scoreTypeBtn(businessPriceSetData.checkType,_d,businessPriceSetData)
				}else{
					$(".isBusiness").hide();					
				}	
	   		}
...
//价格评审
function scoreTypeBtn(_c,_d,businessPriceSetData){
	var Assessment="";	
	//自定义评分法
	if(_c==0){
		$("#")
	...
	if(_c==3){
		Assessment+='<tr>'
					...
					+'</tr>'
					+'<tr>'
						+'<td rowspan="2" class="th_bg" style="border:none">1、商务报价参数：</td>'							
					+'</tr>'
					+'<tr>'
	...
	if(_c==5 || _c==6){
		Assessment+='<tr>'
					...
					+'<tr>'
						+'<td class="th_bg">权重值（0~1）</td>'
						+'<td style="text-align: left;">'+ businessPriceSetData.weight +'</td>'						
						+'<td style="text-align: left;" colspan="3">商务报价分值：'+businessPriceSetData.baseScore+'</td>'
					+'</tr>'
					if(businessPriceSetData.basePriceType=="3"){
						...
						+'</tr>'
							...
							+'<td colspan="2" style="text-align: left;">基准价：'+(businessPriceSetData.basePrice==undefined?"":businessPriceSetData.basePrice)+'元</td>'				
						+'</tr>'
					}else{
