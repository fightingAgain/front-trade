...
var urlHistoryInfo = 'bidPrice/ProjectCheck/modal/AuctionOfferHistoryInfo.html';
var urlOfferViewInfo = 'bidPrice/ProjectCheck/modal/AuctionOfferViewInfo.html';
//查询按钮
...
function AddFunction(value, row, index) { //把需要创建的按钮封装到函数中
	if(row.checkState=='2' ){//已提交状态
		...
		return [
			'<a href="javascript:void(0)" id="btnCheck" class="btn-sm btn-primary" onclick=btnButton("'+row.projectId+'","'+row.packageId+'") style="text-decoration:none" ><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>审核</a>&nbsp;&nbsp;',
		].join("")
		
	}else{
		if(row.auctionStatus == '已结束'){
			return [
				'<a href="javascript:void(0)" id="btnShowLast" class="btn-sm btn-primary" onclick=btnButton("'+row.projectId+'","'+row.packageId+'") style="text-decoration:none" ><span class="glyphicon glyphicon-search" aria-hidden="true"></span>查看</a>&nbsp;&nbsp;',
			].join("")
		}else{
			return [
				'<a href="javascript:void(0)" id="btnShow" class="btn-sm btn-primary" onclick=btnButton("'+row.projectId+'","'+row.packageId+'") style="text-decoration:none" ><span class="glyphicon glyphicon-search" aria-hidden="true"></span>查看</a>&nbsp;&nbsp;',
			].join("")
		}
	}	
}
			...
			if(data.success){
				...
				if(result.checkState=='2' ){//已提交状态
					...
					layer.open({
						...
						resize: false, //是否允许拉伸
						content: urlHistoryInfo + '?projectId=' + result.projectId + '&packageId=' + result.packageId +'&id='+result.id+ '&type='+"check" + "&tender=1"
					});
					...
					if(result.auctionStatus == '已结束'){
						...
						layer.open({
							...
							maxmin: false, //该参数值对type:1和type:2有效。默认不显示最大小化按钮。需要显示配置maxmin: true即可
							resize: false, //是否允许拉伸
							content: urlHistoryInfo + '?projectId=' + result.projectId + '&packageId=' + result.packageId+'&id='+result.id + '&type='+"look"+ "&tender=1"
						});
					}else{
