...
$(function() {
	...
	$("#query").on('click', '#btnDownload', function() {
		...
		var url = config.AuctionHost + "/BusinessStatisticsController/exportDateReportPageList.do" + "?tenderProjectName=" +
			tenderProjectName + '&tenderProjectCode=' + tenderProjectCode + '&bidSectionCode=' + bidSectionCode + '&bidSectionName=' +
			bidSectionName + '&tenderType=4&checkState=' + checkState +
			...
			"&purchaserName=" + purchaserName + "&purchaserLinKmen=" + purchaserLinKmen + "&legalName=" + legalName + '&processType=' + pageUrlParams.processType
...
//var auditPurchase = 'bidPrice/DataReport/Bidding/model/ViewBidReport.html' //查看路径
var auditPurchase = 'bidPrice/DataReport/public/model/publicListView.html' //查看路径
//var editPurchase = 'bidPrice/DataReport/Bidding/model/editBidReport.html' //编辑路径
...
var noticeStateUrl = config.AuctionHost + "/AuctionPurchaseController/updateNoticeState.do"
//取消项目方法
	...
	$('#table').bootstrapTable({
		...
		columns: [{
			...
			formatter: function(value, row, index) {
				...
				return abnormalReason[value] || value || "";
		...
		},
			{
				...
				events: {
					'click .recall': function(e,value, row, index){
						recall_btn2(row.id, row.tenderProjectID, row.processType)
					},
				},
				formatter: function(value, row, index) {
					var Tdr = "";
					if(row.createType && row.createType == 1){
						return '<button type="button" class="btn btn-sm btn-primary view" onclick=audit_btn(\"' + index + '\")><span class="glyphicon glyphicon-search" aria-hidden="true"></span>查看</button>'
					}else{
...
function queryParams(params) {
	return {
		...
		'processType': pageUrlParams.processType,
...
//审核成功撤回
function recall_btn2(id, bidSectionId, processType) {
	var rowData = $('#table').bootstrapTable('getData'); //bootstrap获取当前页的数据
	/* ***************        验证项目是否重采       ****************** */
	var tender = processType==2?'99':'4';
	var msg = checkRePurchase(tender, bidSectionId);
	if(msg != ''){
		top.layer.alert(msg);
		return
	}
...
function edit_btn($index, projectState) { //$index当前选择行的下标，projectState是当前选择行的审核状态0为未审核，1为审核中，2为审核通过。
	...
	parent.layer.open({
		...
		resize: false,
		content: editPurchase + '?projectId=' + rowData[$index].tenderProjectID + "&flage=" + flag+'&tenderType='+rowData[$index].tenderType + '&processType=' + pageUrlParams.processType,
...
function audit_btn($index) { //$index当前选择行的下标
	...
	parent.layer.open({
		...
		maxmin: true ,//开启最大化最小化按钮
		resize: false,
		content: auditPurchase + '?projectId=' + rowData[$index].tenderProjectID + "&flage=" + flag + '&edittype=detailed' + "&id=" + rowData[$index].id+'&tenderType='+rowData[$index].tenderType + '&processType=' + pageUrlParams.processType,
		// btn: ['关闭'], //确定按钮
		yes: function(index, layero) {
			parent.layer.close(index);
		}
...
//开始时间
$('#noticeSendStartTime').click(function() {
	WdatePicker({
		el: this,
		dateFmt: 'yyyy-MM-dd HH:mm',
	})
...
//结束时间
/*$('#noticeSendEndTime').datetimepicker({
		step: 5,
		lang: 'ch',
		format: 'Y-m-d H:i',
		onShow: function() {
			alert(12)
			$("#noticeSendStartTime").datetimepicker()
		},

	});*/
/*$('#noticeSendEndTime').click(function() {
	WdatePicker({
		el: this,
		dateFmt: 'yyyy-MM-dd HH:mm',
		onShow: function() {
...
$('#noticeSendEndTime').click(function() {
	WdatePicker({
		...
		minDate:'#F{$dp.$D(\'noticeSendStartTime\')}'
	})
});
*/
//取消撤回
