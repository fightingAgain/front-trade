...
function time() {
	...
	// 			if (NewDate($('#noticeSendTime').val()) > NewDate(nowSysDate)) {
	// 				var bidEndMin = $('#noticeSendTime').val() + ':00';
	// 			} else {
	...
	$('#bidOpenTime').datetimepicker({
		...
		onShow: function () {
			var nowSysDate = top.$("#systemTime").html() + " " + top.$("#sysTime").html();
			if ($('#subDate').val() != "") {
				if (NewDate($('#subDate').val()) > NewDate(nowSysDate)) {
					var bidEndMin = $('#subDate').val() + ':00';
				} else {
					var bidEndMin = nowSysDate//否则则是当前时间为最小值
				}
			} else {
	...
	$('#noticeSendTime').datetimepicker({
		...
		onShow: function () {
			var answersMinDate = "";
			var nowSysDate = top.$("#systemTime").html() + " " + top.$("#sysTime").html();
			if ($('#bidOpenTime').val() != "") {//判断是否存在提出澄清截止时间
				if (NewDate($('#bidOpenTime').val()) > NewDate(nowSysDate)) {//如果存在判断提出澄清截止时间跟当前时间，
					answersMinDate = $('#bidOpenTime').val() + ':00';//如果提出澄清截止时间大于当前时间，则询比的最小值是报价时间
				} else {
