var urlPurchaseList = config.AuctionHost + '/BusinessStatisticsController/findDateReportPageList.do';
var saveProjectPackage = config.AuctionHost + '/PurchaseController/startWorkflowAccep.do'; //添加包件的接口
var findPackage = config.AuctionHost + '/ProjectPackageController/findProjectPackageList.do' //包件列表
var recallUrl = config.AuctionHost + '/WorkflowController/updateAuctionYwtjbState' //撤回
var deletPackageUrl = config.AuctionHost + '/PurchaseController/deletePackage.do' //删除包件接口
var pricedelete = config.AuctionHost + '/ProjectPriceController/deleteProjectPriceByPackage.do' //费用删除
//var addpackage = 'bidPrice/DataReport/Bid/Agent/Purchase/model/noticeInfo.html'; //后审的编辑页面
var addpackage = 'bidPrice/DataReport/public/model/publicListEdit.html'; //后审的编辑页面
var viewpackage = 'bidPrice/DataReport/public/model/publicListView.html';
//var viewpackage = 'bidPrice/DataReport/Bid/Agent/Purchase/model/noticeView.html'
var iframeWinAdd = "";
//取消项目方法
...
$(function() {
	...
	$("#query").on('click', '#btnDownload', function() {
		...
		var url = config.AuctionHost + "/BusinessStatisticsController/exportDateReportPageList.do" + "?tenderProjectName=" + tenderProjectName +
			'&tenderProjectCode=' + tenderProjectCode + '&bidSectionCode=' + bidSectionCode + '&bidSectionName=' + bidSectionName + '&tenderType=0&checkState=' + checkState +
			...
			},
			{
				...
				events: {
					'click .recall': function(e,value, row, index){
						recall_btn2(row.id, row.tenderProjectID)
					},
				},
				formatter: function(value, row, index) {
					var Tdr = "";
					if(row.createType != undefined && row.createType == 1) {
						Tdr += '<button type="button" class="btn btn-sm btn-primary view" onclick=viewbao(\"' + index + '\")><span class="glyphicon glyphicon-search" aria-hidden="true"></span>查看</button>'
					} else {
						if(row.checkState == 0) { //未提交			
								Tdr += '<button type="button" class="btn btn-sm btn-primary view" onclick=viewbao(\"' + index + '\")><span class="glyphicon glyphicon-search" aria-hidden="true"></span>查看</button>'
								Tdr += '<button type="button" class="btn btn-sm btn-primary edit" onclick=edit_bao(\"' + index + '\")><span class="glyphicon glyphicon-edit" aria-hidden="true" ></span>编辑</button>'
						} else if(row.checkState == 1) { //审核中
								Tdr += '<button type="button" class="btn btn-sm btn-primary view" onclick=viewbao(\"' + index + '\")><span class="glyphicon glyphicon-search" aria-hidden="true"></span>查看</button>'
							//Tdr += '<button type="button" class="btn btn-sm btn-warning recall" onclick=recall(\"' + row.id + '\")><span class="glyphicon glyphicon-search" aria-hidden="true" ></span>撤回</button>'
...
function viewbao($index) {
	...
	parent.layer.open({
		...
		resize: false,
		content: viewpackage + "?packageId=" + rowData[$index].tenderProjectID + "&flage=" + flag + "&id=" + rowData[$index].id,
...
function edit_bao($index) {
	...
	parent.layer.open({
		...
		maxmin: true ,//开启最大化最小化按钮
		resize: false,
		content: addpackage + "?packageId=" + rowData[$index].tenderProjectID + "&flage=" + flag + "&tenderType=" + rowData[$index].tenderType,
		success: function(layero, index) {
			var iframeWind = layero.find('iframe')[0].contentWindow;
...
//xinzeng
//审核成功撤回
function recall_btn2(id, bidSectionId) {
	/* ***************        验证项目是否重采       ****************** */
	var msg = checkRePurchase(0, bidSectionId);
	if(msg != ''){
		top.layer.alert(msg);
		return
	}
