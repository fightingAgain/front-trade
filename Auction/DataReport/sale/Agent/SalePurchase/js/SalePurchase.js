...
$(function() {
	...
	$("#query").on('click', '#btnDownload', function() {
		...
		var url = config.AuctionHost + "/BusinessStatisticsController/exportDateReportPageList.do" + "?tenderProjectName=" + tenderProjectName + "&tenderProjectCode=" + tenderProjectCode +
			"&tenderType=2" + "&checkState=" + checkState + "&subDateStart=" + subDateStart + "&subDateEnd=" + subDateEnd + "&noticeSendTimeStart=" + noticeSendTimeStart +
			"&noticeSendTimeEnd=" + noticeSendTimeEnd + "&departmentName=" + departmentName + "&departmentId=" + departmentId +
			"&purchaserName=" + purchaserName + "&purchaserLinKmen=" + purchaserLinKmen + "&legalName=" + legalName
		console.log(url)
	...
	/*$("#btnList").click(function() {
		...
		var checkState = $("#checkState").val()
		//var url = config.AuctionHost + "/BusinessStatisticsController/exportDateReportPageList.do" + "?tenderProjectName=" + tenderProjectName + '&tenderProjectCode=' + tenderProjectCode + '&bidSectionCode='+ bidSectionCode + '&bidSectionName=' + bidSectionName + '&tebererType=2';
		var url = config.AuctionHost + "/BusinessStatisticsController/exportDateReportPageList.do" + "?tenderProjectName=" + tenderProjectName + "&tenderProjectCode=" + tenderProjectCode +
			"&tenderType=2" + "&checkState=" + checkState;
		console.log(url)
		window.location.href = $.parserUrlForToken(url);
	});*/
});
/// 表格初始化
//var auditPurchase = 'bidPrice/DataReport/sale/Agent/SalePurchase/model/auditPurchase.html' //审核路径
//var editPurchase = 'bidPrice/DataReport/sale/Agent/SalePurchase/model/edit_model.html' //编辑路径
var auditPurchase = 'bidPrice/DataReport/public/model/publicListView.html'; //审核路径
var editPurchase = 'bidPrice/DataReport/public/model/publicListEdit.html'; //编辑路径
var pageAuctionPurchase = config.AuctionHost + '/BusinessStatisticsController/findDateReportPageList.do';
...
var urlWithdraw = config.AuctionHost + "/WorkflowController/updateAuctionYwtjbState"
//取消项目方法
		...
		columns: [{
			...
			formatter: function(value, row, index) {
				if(row.packageSource == 1) {
					var projectName = row.tenderProjectName + '<span class="text-danger"  style="font-weight:bold">(重新竞卖)</span>'
				} else {
					var projectName = row.tenderProjectName
				}
				return projectName
			}
		},
...
function edit_btn($index, projectState) {
	...
	parent.layer.open({
		...
		resize: false,
		content: editPurchase + "?packageId=" + rowData[$index].tenderProjectID + "&flage=" + flag + "&tenderType=" + rowData[$index].tenderType,
...
function audit_btn($index, projectId) {
	...
	parent.layer.open({
		...
		resize: false,
		id: 'packageclass',
		content: auditPurchase + '?projectId=' + rowData[$index].tenderProjectID + "&flage=" + flag + "&tenderType=" + rowData[$index].tenderType + '&edittype=detailed&id=' + rowData[$index].id,
		// btn: ['关闭']
		//确定按钮
		// ,
