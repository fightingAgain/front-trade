...
$(function() {
	if(PAGEURL.split("?")[1] != undefined) {
		tenderType = PAGEURL.split("?")[1].split("=")[1]; 
	}
	initTable();
	...
	$("#query").on('click', '#btnDownload', function() {
		...
		var url = config.AuctionHost + "/BusinessStatisticsController/exportDateReportPageList.do" + "?tenderProjectName=" + tenderProjectName +
			'&tenderProjectCode=' + tenderProjectCode + '&bidSectionCode=' + bidSectionCode + '&bidSectionName=' + bidSectionName + '&tenderType='+tenderType+'&checkState=' + checkState +
		...
		columns: [{
				...
				formatter: function(value, row, index) {
					if(row.packageSource == 1) {
						return value + '<span class="text-danger">(重新'+(row.tenderType==1?'竞价':(row.tenderType==2?'竞卖':'采购'))+')</span>';
					} else {
						return value;
					}
