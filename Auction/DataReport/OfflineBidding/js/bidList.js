...
//var auditPurchase = 'bidPrice/DataReport/OfflineBidding/model/ViewBidReport.html' //查看路径
var auditPurchase = 'bidPrice/DataReport/public/model/publicListView.html' //查看路径
//var editPurchase = 'bidPrice/DataReport/OfflineBidding/model/editBidReport.html' //编辑路径
...
function recall_btn2(id, bidSectionId) {
	/* ***************        验证项目是否重采       ****************** */
	var msg = checkRePurchase('99', bidSectionId);
	if(msg != ''){
		top.layer.alert(msg);
		return
	}
...
function audit_btn($index) { //$index当前选择行的下标
	...
	parent.layer.open({
		...
		maxmin: true,//开启最大化最小化按钮
		resize: false,
		content: auditPurchase + '?projectId=' + rowData[$index].tenderProjectID + "&flage=" + flag + '&edittype=detailed' + "&id=" + rowData[$index].id + '&tenderType=' + rowData[$index].tenderType,
		// btn: ['关闭'], //确定按钮
		yes: function (index, layero) {
			parent.layer.close(index);
		}
