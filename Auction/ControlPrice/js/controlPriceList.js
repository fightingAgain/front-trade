var listUrl = config.AuctionHost + '/ControlPriceController/findControlPricePage.do';  //列表接口
var delUrl = config.AuctionHost + '/ControlPriceController/delete.do';  //删除接口
var revokeUrl = config.AuctionHost + '/ControlPriceController/revokeWorkflowItem.do';  //撤回
var changeUrl = config.AuctionHost + '/ControlPriceController/getChangeControlPrice.do';  //变更接口

var pageEdit = "bidPrice/ControlPrice/model/controlPriceEdit.html";  //编辑页面
var pageView = "bidPrice/ControlPrice/model/controlPriceView.html";  //查看页面
var bidSectionPage = "bidPrice/ControlPrice/model/bidSectionList.html";	//标段列表
var tenderType,isAgent;

$(function(){
	if(PAGEURL.split("?")[1] != undefined) {
		tenderType = PAGEURL.split("?")[1].split("=")[1].substring(0,1); 
	}
	if(PAGEURL.split("?")[1] != undefined) {
		isAgent = PAGEURL.split("&")[1].split("=")[1]; 
	}
	initDataTab();
	//查询
...
function openEdit(rowData){
	...
	layer.open({
		...
		area: ['100%', '100%'],
		resize: false,
		content: pageEdit+'?id='+rowData.packageId+'&special=KZT',
...
function openChang(rowData,id){
	var url = pageEdit+'?id='+rowData.packageId+'&special=KZT',
	title = "编辑控制价";
	layer.open({
		type: 2,
		title: title,
...
function openView(index){
	...
	layer.open({
		...
		area: ['1200px', '600px'],
		resize: false,
		content: pageView +'?id='+rowData.packageId,
		success:function(layero, index){
			var iframeWin = layero.find('iframe')[0].contentWindow;
...
function getQueryParams(params) {
	var projectData = {
		...
		enterpriseType: isAgent==1?'02':'04',
		...
		columns: [
			[{
				...
				},
				{
					...
					formatter: function(value, row, index){
						// 0为临时保存，1为提交审核，2为审核通过，3为审核未通过，4已撤回
						if(value == 0) {
				...
				},
				{
					...
					formatter: function (value, row, index) {
						...
						if(row.createType && row.createType == 1){//权限  是否有操作权限1是  0否
							if(!row.checkState){
								return '';
							}else{
								return strSee;
							...
							} else if(row.checkState == 3 || row.checkState == 4) {
								str += strSee + strEdit + strDel;
							}else {
								str += strEdit;
							}
