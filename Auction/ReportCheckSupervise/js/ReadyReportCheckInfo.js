...
function ProjectCheckState(packageData) {
	...
	/*if(projectCheckerItems.length > 0) {
		if(packageData.isStopCheck != 1&&(packageData.stopCheckReason==""||packageData.stopCheckReason==undefined)&&packageData.checkResult != "已完成"&&createType!=1){
			...
			for(var x = 0; x < packageCheckLists.length; x++) {
				titletr += "<td class='no_"+packageCheckLists[x].id+"' ><a style='text-decoration: none;' class='btn btn-primary btn-sm openBreak' onclick='openBreak(\"" + packageCheckLists[x].id + "\",\"" + packageCheckLists[x].checkName + "\")'>退回到此阶段</a></td>";
			}
			...
			titletr += "<td><a style='text-decoration: none;' class='btn btn-primary btn-sm openBreak' onclick='openBreak(\"report\",\"预审报告\")'>退回到此阶段</a></td>";
...
function packageDetaileds(packageData) {
	...
	$("#packageDetailedsTable").bootstrapTable({
		...
		onClickRow: function(row) {
			var offerDetaileds = packageData.offerDetaileds; //询比采购报价详情
...
$("#downloadAllFile").click(function(){
	window.open($.parserUrlForToken(top.config.AuctionHost+"/ProjectReviewController/downloadAllPurFile.do?projectId="+ProjectData.projectId+"&packageId="+packageId+"&examType="+"0"));
});
...
$("#downloadCheckReport").click(function() {
	...
	if(CheckTotals.checkReport == '已完成') {
		window.open($.parserUrlForToken(top.config.FileHost + "/FileController/download.do?ftpPath=" + CheckTotals.checkReports[0].reportUrl + "&fname=" + str2 + "_评审报告"));
	} else {
		top.layer.alert("评审报告未生成！");
	}
});
					...
					columns: [{
						...
						formatter: function(value, row, index) {
							if(row.isCheck == "2") {
								...
								/*return "<div id='submitCheckReport'><button onclick='subAudit(this)' class='btn  btn-primary subAudit btn-xs PriceCheckBtn'>重新提交</button></div>";*/
							} else if(row.isCheck == "0") {
								/*return "<div id='submitCheckReport'><button onclick='subAudit(this)' class='btn  btn-primary subAudit btn-xs PriceCheckBtn'>提交审核</button></div>";*/
