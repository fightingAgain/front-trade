 ...
 * @LastEditTime: 2021-02-25 19:44:22
...
function du(uid, examType, isShowSupplier){
	$.ajax({
	   	...
	   	success:function(data){
				...
				var checkEndDate = examType == 1?Date.parse(new Date(packageInfo.checkEndDate.replace(/\-/g, "/"))):Date.parse(new Date(packageInfo.examCheckEndDate.replace(/\-/g, "/")));
				var nowDate = Date.parse(new Date((top.$("#systemTime").html() + " " + top.$("#sysTime").html()).replace(/\-/g, "/")))
				if(isShowSupplier ==1 && examType == 1){
					$('.showIP').show();
					getMachineList();
				}else{
					$('.showIP').hide();
				}
				// if(packageInfo.reviewSetting && packageInfo.reviewSetting.show == 1 && tenderType == 0 && createType == 0 && packageInfo.checkTimeOut == 1){
					// $('.showIP').show();
