 ...
 * @LastEditTime: 2021-01-18 17:31:55
...
$(function(){
   ...
   /*设置规则 */
	...
	if(((tenderType == 0 && examType == 1) || tenderType == 6) && (packageInfo.checkPlan == 1 || packageInfo.checkPlan == 3 || packageInfo.checkPlan == 4 || packageInfo.checkPlan == 5)){
		rule1 =true
	};
	if(((tenderType == 0 && examType == 1) || tenderType == 6) && (packageInfo.checkPlan == 2 || packageInfo.checkPlan == 0)){
		rule2 =true
	};
	/* if(tenderType == 0 && examType == 1 && packageInfo.checkPlan == 0){
		rule3 =true
	}; */
	 /*设置规则   --end */
	PriceCheck();
...
function PriceCheck() {
	$.ajax({
		type: "post",
		url: url + "/ManagerCheckController/getPriceCheckInfoForManager.do",
		async:false,
		data: {
			projectId: projectId,
			packageId: packageId,
		...
		success: function (resle) {
			if(!resle.success){
				parent.layer.alert(resle.message);
				$("#"+_THISID).click();
				return
			}
			_THISID=_thisId;
			...
			if (data) {
				...
				if (checkPlan == 1 || checkPlan == 3 || checkPlan == 4 || checkPlan == 5) { //最低评标价法					
					...
					if (businessPriceSet.checkType == 1) $("#businessType").html("商务报价参数：扣分的最小档次：" + businessPriceSet.reduceLevel + "%，最小档扣分：" + businessPriceSet.reduceScore + "分，基准价" + businessPriceSet.basePrice + "，有效报价范围（基准价上下浮动比例）±" + businessPriceSet.offerRange + "%");
						...
						formatter: function (value, row, index) {
							if (data.priceCheck == "未完成") {
								if (businessPriceSet.checkType == 0) {
									if (row.isOut == 0) {
										return "<input type='text' style='width:100%' id='deviatePrice_" + row.supplierId + "' onchange='fixCount(\"" + 'deviatePrice' + "\",\"" + row.supplierId + "\")' value='0'></input>";
									} else {
					...
					columnList.push({
						...
						formatter: function (value, row, index) {
							if (data.priceCheck == "未完成") {
								if (businessPriceSet.checkType == 0) {
									if (row.isOut == 0) {
										return "<input style='width:100%' type='text' id='score_" + row.supplierId + "' value='" + (row.score || '') + "'/>";
									} else {
										return "<input style='display:none' type='text' id='score_" + row.supplierId + "' value=''/>";
									}
							...
							if (row.isOut == 0) {
								if(priceCheckMode==1){
									if (row.finalDiscountPrice) {
										...
										return " <input type='checkbox'"+( row.bidpriceAmount?'checked':'') +"  class='finalChecked' id='finalChecked_" + row.supplierId + "' value=''><input style='width:80px;' "+( row.bidpriceAmount?'':'disabled') +" type='text' onchange='fixCount(\"" + 'finalDiscountPrice' + "\",\"" + row.supplierId + "\")' id='finalDiscountPrice_" + row.supplierId + "' value='"+ (row.bidpriceAmount||"")+"'/>";
									} 
								}else{
									if (row.finalDiscountPrice) {
										return " <input type='checkbox' checked class='finalChecked' id='finalChecked_" + row.supplierId + "' value=''><input style='width:80px;' type='text' onchange='fixCount(\"" + 'finalDiscountPrice' + "\",\"" + row.supplierId + "\")' id='finalDiscountPrice_" + row.supplierId + "' value='"+ (row.finalDiscountPrice||"") +"'/>";
...
function fixCount(typeName, supplierId) {
	...
	if (typeName == "fixCount" || typeName == 'baseCheckPrice' || typeName == 'deviatePrice') {
		if (typeName == "fixCount") {
			...
			// $("#baseCheckPrice_" + supplierId + "").text(prices);
...
function savepriceChecks() {
	for (var i = 0; i < priceChecks.length; i++) {
		if (priceChecks[i].isOut == 0) {
			if (checkPlan == 1 || checkPlan == 3 || checkPlan == 4 || checkPlan == 5) {
				if ($("#deviatePrice_" + priceChecks[i].supplierId).val() != "" && priceChecks[i].isOut == 0 && (checkPlan == 1 || checkPlan == 4 || checkPlan == 5)) {
					...
					if (!testRule.test($("#deviatePrice_" + priceChecks[i].supplierId).val())) {
						top.layer.alert(priceChecks[i].enterpriseName + "的偏离调整"+(checkPlan == 4?"减":"加")+"价只能是数字");
						return false;
					};
					if($("#deviatePrice_" + priceChecks[i].supplierId).val().split('.')[0].length > priceInte){
						top.layer.alert(priceChecks[i].enterpriseName + "的偏离调整"+(checkPlan == 4?"减":"加")+"价最多"+ priceInte +"位整数");
						return false;
					}
				}
			} else {
			...
			if($("#totalCheck_" + priceChecks[i].supplierId).val().split('.')[0].length > priceInte){
				top.layer.alert(priceChecks[i].enterpriseName + "的评审总价最多"+ priceInte +"位整数");
				return false;
			}
			if ($("#finalChecked_" + priceChecks[i].supplierId).is('checked') == true) {
				if ($("#finalDiscountPrice_" + priceChecks[i].supplierId).val() == "") {
					...
					if (!/^((\d+\.\d*[0-9]\d*)|(\d*[0-9]\d*\.\d+)|(\d*[0-9]\d*))$/.test($("#finalDiscountPrice_" + priceChecks[i].supplierId).val())) {
						top.layer.alert(priceChecks[i].enterpriseName + "的最终优惠价只能是数字,且最多两位小数");
						return false;
					};
					if($("#finalDiscountPrice_" + priceChecks[i].supplierId).val().split('.')[0].length > priceInte){
						top.layer.alert(priceChecks[i].enterpriseName + "的最终优惠价最多"+ priceInte +"位整数");
						return false;
					};
				}
