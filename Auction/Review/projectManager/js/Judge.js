...
var tobeChangeExpertItemId = getUrlParam("id"),
	...
	ProjectData = JSON.parse(sessionStorage.getItem('FormaData')),
...
$(function() {
	...
	if(titleType=="cqpw"){
		...
		var tableUrl = '';
		if(enterpriseType == '02'){
			tableUrl = '/CheckController/findEnterpriseExpert.do';
		}else{
			tableUrl = '/CheckController/findEnterpriseExperts.do';
		}
		tableBae(tableUrl);
		$(".JudgeDiv").show();		
	}
});
$("#number").off('blur').on('blur',function(){
	var type = /^[1-9][0-9]*$/;
	var re = new RegExp(type);
	if($.trim($("#number").val()) !=""&&!(re.test($(this).val()))) {
		parent.layer.alert("请输入大于0的整数");
		return false;
	}
})
//抽取按钮
$("#SbmJudge").off('click').click(function() {
	if($.trim($("#number").val()) == "") {
...
//取消按钮
$("#btn_cancle").off('click').click(function(){
...
function CheckItem(id) {
	parent.layer.prompt({
		...
		if($.trim(text)==""){
			parent.layer.alert('温馨提示:替换原因不能为空');
			return
		}
		$.ajax({
			type: "get",
	...
	$("#Judgetable").bootstrapTable({
		...
		columns: [{
				...
				formatter: function(value, row, index) {
					var pageSize = $('#Judgetable').bootstrapTable('getOptions').pageSize || 15; //通过表的#id 可以得到每页多少条  
					var pageNumber = $('#Judgetable').bootstrapTable('getOptions').pageNumber || 1; //通过表的#id 可以得到当前第几页  
			...
			},
			{
				...
				width: '70px',
				formatter: function(value, row, index) {
					return "<button  type='button' class='btn-xs btn btn-primary' onclick='CheckItem(\"" + row.id + "\")'><span class='glyphicon glyphicon-ok'></span>选择</button>";
				}
			}
		]
	});
};
$('#Judgetable').off('uncheck.bs.table check.bs.table check-all.bs.table uncheck-all.bs.table').on('uncheck.bs.table check.bs.table check-all.bs.table uncheck-all.bs.table',function(e,rows){
        var datas = $.isArray(rows) ? rows : [rows];        // 点击时获取选中的行或取消选中的行
        examine(e.type,datas); // 保存到全局 Array() 里
});

var itemTypeId=[];
$("#dataTypeName").off('click').click(function() {
	...
	top.layer.open({
		...
		resize: false, //是否允许拉伸
		closeBtn: 1,
		content: 'view/projectType/projectType.html?type=1&select=1&interpriceType=2', // type,0为供应商类型，1为专家评委类型，2为项目类型,select 0为单选，1为多选
		btn: ['确定', '取消'],
		scrolling: 'no',
