...
var accepType = getUrlParam('accepType');
	...
	$(".fileBox").each(function() {
		...
		for(let i in uploadFileArr) {
			...
			if(uploadFileArr[i].fileType == obi) {
				...
				str += '<button type="button" class="btn btn-primary btn-xs" onclick="fileDownload(\'' + uploadFileArr[i].fileName + '\',\'' + uploadFileArr[i].filePath + '\')">下载</button></div>';
...
function getBiddingApprovalRecord() {
	...
	$("#biddingApprovalRecord").bootstrapTable({
		url: top.config.AuctionHost + '/WorkflowController/findWorkflowCheckerAndAccp',
		dataType: 'json',
		method: 'get',
		...
		onLoadSuccess: function(data) {
			$("#biddingApprovalRecord").bootstrapTable("load", data.data.workflowItems);
		},
		queryParams: {
			'businessId': id,
