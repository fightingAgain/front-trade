...
function add_bao() {
	parent.layer.open({
		...
		, content: 'Auction/common/Agent/Purchase/model/choicePackage.html?examType=0'
...
function callback(rowData) {
	...
	datetimepicker(rowData.examType)
	...
	if (timeCheck($("#timeList"))) {
		...
		if (isDf) {
			if (depositList.subitemData()) {
				top.layer.confirm('{' + depositList.subitemData() + '},保证金没有转移到本项目，确认不转移吗？', function (index) {
					saveFuct();
					parent.layer.close(index);
				})
			} else {
			...
			$.ajax({
				...
				success: function (data) {
					if (data.success == true) {
						...
						// 校验报价表是否存在 
						top.checkBidFilesTips(packageId, '02', examType,"提交成功"); 
		...
		$.ajax({
			...
			success: function (data) {
				if (data.success == true) {
					...
					// 校验报价表是否存在 
					top.checkBidFilesTips(packageId, '02', examType, "提交审核成功");
					parent.$('#table').bootstrapTable(('refresh')); // 很重要的一步，刷新url！
					parent.layer.close(parent.layer.getFrameIndex(window.name));
				} else {
					parent.layer.alert(data.message);
...
$("#btn_bao").click(function () {
	...
	// 时间处理
	var vmForm = $("#formbackage");
	['noticeEndDate', 'signEndDate', 'sellFileEndDate', 'sellFileEndDate','acceptEndDate'].forEach(function(key) {
		var value = vmForm.find('#' + key).val();
		if (value && value.length == 16) {
			vmForm.find('#' + key).val(value + ':59')
		}
