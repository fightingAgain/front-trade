...
window.operateEvents = { //添加一个按钮对应的事件
	"click #btnCheck": function(e, value, row, index) {
		...
		layer.open({
			...
			title: '价格谈判',
			area: ['1000px', '600px'],
			content: 'Auction/common/Agent/PucharesSupplierNegotiator/pPriceNegotiator.html?'+"createType="+createType+'&projectId='+row.projectId+'&id='+row.packageId,
	...
	"click #btnNot": function(e, value, row, index) {
		...
		layer.open({
			...
			title: '价格谈判',
			area: ['900px', '400px'],
			content: 'Auction/common/Agent/PucharesSupplierNegotiator/NotNegotiator.html',
			success: function(layero, indexs) {
				var body = layer.getChildFrame('body', indexs);
				var iframeWin = layero.find('iframe')[0].contentWindow;
				iframeWin.tableShow(row);
	...
	"click #btnUnderCheck": function(e, value, row, index) {
		...
		layer.open({
			...
			area: ['1000px', '600px'],
			id: "negotiators",
			content: 'Auction/common/Agent/PucharesSupplierNegotiator/underline.html?projectId='+row.projectId+'&id='+row.packageId,
			success: function(layero, indexs) {
				var body = layer.getChildFrame('body', indexs);
				var iframeWin = layero.find('iframe')[0].contentWindow;
	...
	"click #btnNegotiator": function(e, value, row, index) {
		...
		layer.open({
			...
			resize: false, //是否允许拉伸
			content: 'Auction/common/Agent/PucharesSupplierNegotiator/NegotiatorMethod.html?projectId='+row.projectId+'&id='+row.packageId,
			success: function(layero, indexs) {
				var body = layer.getChildFrame('body', indexs);
				var iframeWins = layero.find('iframe')[0].contentWindow;
				iframeWins.getMessage(row);  
			}
		});
