...
function initTable() {
	$('#table').bootstrapTable({
		...
		columns: [{
			...
			formatter: function(value, row, index) {
				//编辑
				var State = '<button href="javascript:void(0)"  type="button" class="btn-sm btn-primary" style="text-decoration:none;margin-right:5px;border: none;padding:3px 5px" onclick=edit_btn(\"' + index + '\")>' +
					'<span class="glyphicon glyphicon-edit" aria-hidden="true"></span>编辑' +
					'</button>'

				//撤回
				var States = '<button href="javascript:void(0)"  type="button" class="btn-sm btn-warning" style="text-decoration:none;margin-right:5px;border: none;padding:3px 5px" onclick=recall(\"' + index + '\")>' +
					'<span class="glyphicon glyphicon-share" aria-hidden="true"></span>撤回' +
					'</button>'
				//查看
				var audit = '<button href="javascript:void(0)"  type="button" class="btn-sm btn-primary" style="text-decoration:none;margin-right:5px;border: none;padding:3px 5px" onclick=audit_btn(\"' + index + '\")>' +
					'<span class="glyphicon glyphicon-search" aria-hidden="true"></span>查看' +
					'</button>'
				// 删除
				var delet = '<button  type="button" class="btn-sm btn-danger" href="javascript:void(0)" style="text-decoration:none;padding:3px 5px;border: none;" onclick=deletPackage(\"' + index + '\")><span class="glyphicon glyphicon-trash" aria-hidden="true"></span>删除</button>'
				if(row.createType != undefined && row.createType == 1) {
					return '<div class="btn-group-sm">' + audit + '</div>';
				} else {
					if(row.packageState == 2) {
						if(row.inviteState == 1) {
							return '<div class="btn-group-sm">' + audit + States + '</div>';
						} else {
							if(row.examType == 1) {
								return '<div class="btn-group-sm">' + State + delet + '</div>';
							} else {
								return '<div class="btn-group-sm">' + State + '</div>';
							}
						}
...
function deletPackage($index) {
	var rowData = $('#table').bootstrapTable('getData')[$index]; //bootstrap获取当前页的数据
	if(rowData.examType == 1&&rowData.packageSource == 1) {
		...
		parent.layer.confirm('温馨提示：您确定要删除该邀请函吗？', {
			btn: ['是', '否'] //可以无限个按钮
		}, function(index, layero) {
			if(top.sysEnterpriseId&&rowData.packageSource==1){
				var arrlist = sysEnterpriseId.split(',');
				if(arrlist.indexOf(top.enterpriseId)!=-1){							
					reDeposit(rowData)
				}
...
function edit_btn($index) {
	...
	if(rowData[$index].examType == 1) {
		if(rowData[$index].packageSource == 1){
			var inurl ="Auction/common/Agent/invAgainPurchase/model/invitationInfo.html"
		}else{
			var inurl = 'Auction/common/Agent/invitation/model/invitationInfo.html'
		}		
	} else if(rowData[$index].examType == 0) {
		var inurl = 'Auction/common/Agent/invitation/model/examInvitationInfo.html'
	}
	parent.layer.open({
		type: 2 //此处以iframe举例
			,
...
function audit_btn($index) {
	...
	if(rowData[$index].examType == 1) {
		...
		var inurl = 'Auction/common/Agent/invitation/model/examInvitationCheck.html'
	}
	parent.layer.open({
		type: 2 //此处以iframe举例
			,
