...
$(function(){
	...
	new UEditorEdit({
		// pageType:"view",
		examType:examType,
		publishName:(examTypeShow==0?'examReleaseForm':'releaseForm'),
		contentKey:(examTypeShow==0?'examRemark':'remark'),
	})
	changeInvitDatetimepicker(projectData.examType, 'change');
	package();
	/*start报价*/
	/* offerFormData();
	fileList(); */
	...
	$.ajax({
		...
		success: function(result) {
			if(result.success){					
				...
				$("input[name='offerEndDate']").val(result.data.offerEndDate!=""?result.data.offerEndDate.substring(0,16):"")
				WORKFLOWTYPE = "xmby";
	...
	if(isCheck) {
		top.layer.confirm('温馨提示：该流程未设置审批节点，您是否继续提交？', function(index) {
			...
			$.ajax({
				...
				data: vmForm.serialize()+'&examType=1',
				success:function(data){ 
					if(data.success==true){
