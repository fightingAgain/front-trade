...
var addsupplier='Auction/common/Agent/invitation/model/examAddSupplier.html';//邀请供应商的弹出框路径
	   	...
	   	success:function(data){
			...
			if(feeConfirmVersion && feeConfirmVersion == 2){
				...
				if(packageInfo.feeUnderparty || packageInfo.feeUnderparty == 0){
					$('[name="feeUnderparty"]').val([packageInfo.feeUnderparty]).attr('disabled', true);
				}
			}else{
...
function package(){
	...
	if(packageInfo.checkPlan==1||packageInfo.checkPlan==3||packageInfo.checkPlan==4){					
		$("#DeviateNum").show();
