...
var findPurchaseUrl;
function initTable() {
	findPurchaseUrl=config.bidhost+'/ProjectPackageController/findProjectPackageContactPageList.do';
	...
	if(tenderType==6){
		$('#table').bootstrapTable("hideColumn", 'examType'); //隐藏项目的审核状态
	}
};
// 分页查询参数，是以键值对的形式设置的
function queryParams(params) {
	    var para={		 
		    ...
		    'enterpriseType':'02',
		    'tenderType':tenderType,//采购方式0为询比采购、1为竞低价2、竞卖		
...
var addpackage='Auction/common/Agent/invAgainPurchase/model/invitationInfo.html';//后审的编辑页面
var flag="";
function btn($index){	  
	    var rowData=$('#table').bootstrapTable('getData');//bootstrap获取当前页的数据
		/* ***************    验证业务统计报表     *********** */
		var bidSectionIds = [];
		bidSectionIds.push(rowData[$index].id);
		var msg = checkStatisticalReport(bidSectionIds, 0)
		if(msg != ''){
			top.layer.alert(msg);
			return
		}
		  ...
		  btn: ['是', '否'] //可以无限个按钮
			...
			if(uid!=""&&uid!=undefined){             			
				parent.layer.open({
			        ...
			        ,content:addpackage+'?projectType=' + rowData[$index].projectType+"&projectId="+rowData[$index].projectId+'&packageId='+uid+'&examType=1&createType='+rowData[$index].createType
