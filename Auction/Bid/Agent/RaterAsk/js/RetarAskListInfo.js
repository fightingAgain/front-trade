	...
	$.ajax({
		...
		success: function(result) {
			if(result.success) {
				...
				parent.layer.msg("回复失败:" + result.message);
			}
		}
	})
})
		...
		},
		{
			...
			formatter: function(value, row, index) {
				...
				if(type == "update") {
					...
					return "<a href='javascript:void(0)' class='btn btn-primary btn-xs' onclick='openAccessory(\"" + row.fileName.substring(0, row.fileName.lastIndexOf(".")) + "\",\"" + row.filePath + "\")'>下载</a>&nbsp;&nbsp;<a href='javascript:void(0)' class='btn btn-primary btn-xs' onclick='previewFile(\"" + row.filePath + "\")'>预览</a>";
				}
			}
		},
	]
