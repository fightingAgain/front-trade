...
var examType,projectId;
var isRater= $.getUrlParam('isRater')
var reg = /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/;
var enterpriseType="02";
var projectType;
//打开弹出框时加载的数据和内容。
function du(uid){
	$.ajax({
...
function package(){
	...
	$("#isOfferDetailedItem").html(packageInfo.isOfferDetailedItem == 1 ? "不需要":"需要");
			...
			success: function(result) {
				if(result.success){	
				    ...
				    if(result.data.examType==0){
						releaseForm=result.data.examReleaseForm		
						noticeInfoFilePath=result.data.examNoticeInfoFilePath							
				    	$("#remark").html(result.data.remark);
				    	oldremarks=result.data.oldRemark;
				    }else{
						releaseForm=result.data.releaseForm		
						noticeInfoFilePath=result.data.noticeInfoFilePath						
					...
					// if(typeof initUEditor === 'function'){
