...
function time(){	
	...
	$('form').on('click', '#noticeEndDate', function(){
		...
		if($('#noticeStartDate').val()!=""){
			if(NewDate($('#noticeStartDate').val())>NewDate(nowSysDate)){	
				var noticeMin=$('#noticeStartDate').val().substring(0,10)
			}else{
	...
	$('form').on('click', '#askEndDate', function(){
		...
		if($('#noticeEndDate').val()!=""){
			if(NewDate($('#noticeEndDate').val())>NewDate(nowSysDate)){	
				var askEndMin=$('#noticeEndDate').val()
			}else{
				var askEndMin=nowSysDate
			}				
		}else{
			var askEndMin=nowSysDate
		}
	...
	$('form').on('click', '#answersEndDate', function(){
		...
		if($('#askEndDate').val()!=""){
			if(NewDate($('#askEndDate').val())>NewDate(nowSysDate)){	
				var answersMinDate=$('#askEndDate').val()
			}else{
				var answersMinDate=nowSysDate
			}				
		}else{
			var answersMinDate=nowSysDate
		}
	...
	$('form').on('click', '#bidEndDate', function(){
		...
		if($('#noticeEndDate').val()!=""){
			if(NewDate($('#noticeEndDate').val())>NewDate(nowSysDate)){	
				var bidEndMin=$('#noticeEndDate').val()
			}else{
	...
	$('form').on('click', '#submitExamFileEndDate', function(){
		...
		if($('#noticeEndDate').val()!=""){
			if(NewDate($('#noticeEndDate').val())>NewDate(nowSysDate)){	
				var submitExamMinDate=$('#noticeEndDate').val()
			}else{
				var submitExamMinDate=nowSysDate
			}				
		}else{
			var submitExamMinDate=nowSysDate
		}
...
function times(){
	...
	$('form').on('click', '#noticeEndDate', function(){
		...
		WdatePicker({
		    ...
		    onpicked:function(dp){
				var selectTime = dp.cal.getNewDateStr(); //选中的时间
				// $('#noticeEndDate').val(selectTime+' 23:59')
		        // $dp.$('contractEndDate').click();
				if(packageInfo.isSellFile==0){
					$("#sellFileEndDate").val($('#noticeEndDate').val());
				}			
	...
	$('form').on('click', '#acceptEndDate', function(){
		var nowSysDate=top.$("#systemTime").html();
		WdatePicker({
			el:this,
			dateFmt:'yyyy-MM-dd 23:59',
			minDate: nowSysDate,
...
function datetimepicker(examType){
	...
	timeList+='<tr>'
			...
			timeList+='<td style="text-align: left;width:320px">'
				...
				if(packageInfo.isSellFile==0){
					timeList+='<input type="hidden" class="btn btn-default ageinTime Wdate" id="sellFileStartDate" value="'+ packageInfo.noticeStartDate +'" name="sellFileStartDate">'//文件发售开始时间
				}
			timeList+='</td>'
     		...
     		timeList+='<td style="text-align: left;width:320px">'
				...
				if(packageInfo.isSellFile==0){
					timeList+='<input type="hidden" class="btn btn-default ageinTime Wdate" id="sellFileEndDate" value="'+ packageInfo.noticeEndDate +'" name="sellFileEndDate">'//<!--文件发售截止时间-->
				}				
     		timeList+='</td>'     		
     	timeList+='</tr>'
     	if(packageInfo.isSign==1){
     		timeList+='<tr>'     		
				...
				timeList+='<td style="text-align: left;">'
					timeList+='<input type="text" autocomplete="off" data-min="signEndDate" data-max="'+ (examType==1?'offerEndDate':'examCheckEndDate') +'" data-datename="报名截止时间" class="btn btn-default ageinTime" id="signEndDate" value="'+ (packageInfo.signEndDate!=undefined?packageInfo.signEndDate.substring(0,16):"") +'" name="signEndDate" style="width: 200px;text-align: left;" readonly="readonly">'
				timeList+='</td>'
	        timeList+='</tr>'
     	} 
	if(examType==1){    	
	...
	if(examType==1){
	    timeList+='<tr>'
	    	timeList+='<td class="th_bg">接受邀请开始时间<i class="red">*</i></td>'
     		...
     		timeList+='<td style="text-align: left;width:320px">'
     			// timeList+='<input type="text" autocomplete="off" data-min="noticeStartDate" data-datename="接受邀请截止时间" class="btn btn-default ageinTime" id="noticeEndDate" value="'+ (packageInfo.noticeEndDate!=undefined?packageInfo.noticeEndDate.substring(0,16):"") +'" name="noticeEndDate" style="width: 200px;text-align: left;">'
