...
var addpackage; //添加包件弹出框路径
var viewpackage;
var iframeWinAdd = "";
var WORKFLOWTYPE = "xmgg";
//表格初始化
$(function() {
	addpackage = 'Auction/common/Agent/Purchase/model/purchaseChangeInfo.html';
	viewpackage = 'Auction/common/Agent/Purchase/model/noticeChangeView.html';
	editpackage = 'Auction/common/Agent/Purchase/model/purchaseChangeEdit.html';
	initTable();
	// 搜索按钮触发事件
				...
				formatter: function(value, row, index) {
					...
					if(row.createType != undefined &&row.createType==1 ) {
						...
						if(row.checkState == 1 || row.checkState == 2) {
							...
							Tdr = '<button class="btn-xs btn btn-primary" onclick=eidtbao(\"' + index + '\")><span class="glyphicon glyphicon-edit" aria-hidden="true"></span>编辑</button>'
						}
					}
...
function viewbao($index) {
	...
	parent.layer.open({
		...
		maxmin: true //开启最大化最小化按钮
			,
		content: viewpackage+"?projectId="+rowData[$index].projectId+'&packageId='+rowData[$index].packageId+'&examType='+rowData[$index].examType+'&keyId='+rowData[$index].id,
