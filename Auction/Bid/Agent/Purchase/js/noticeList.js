...
var addpackage = 'Auction/common/Agent/Purchase/model/noticeInfo.html'; //后审的编辑页面
...
$("#btnSearch").click(function() {
	if($("#packageState").val() != "") {
		urlPurchaseList = config.bidhost + '/ProjectPackageController/findProjectPackagePageList.do';
	} else {
		urlPurchaseList = config.bidhost + '/ProjectPackageController/findProjectPackageNoticePageList.do';
	};
		...
		columns: [{
				...
				formatter: function(value, row, index) {
					if(row.packageSource == 1) {
						if(row.packageSourceCount){
							return value + '<span class="text-danger">(第'+ row.packageSourceCount +'次重新采购)</span>';
						}else{
...
function deletPackage($index) {
	...
	if(rowData.packageSource == 1){
		...
		parent.layer.confirm('温馨提示：您确定要删除该公告吗？', {
			btn: ['是', '否'] //可以无限个按钮
		}, function(index, layero) {
			$.ajax({
...
function edit_bao($index) {
	...
	if(rowData[$index].packageSource == 1){
	 ...
	 var addpackageurl="Auction/common/Agent/againPurchase/model/noticeInfo.html"	
	}else{
		var addpackageurl=addpackage	
	}
	parent.layer.open({
		type: 2 //此处以iframe举例
			,
		...
		content: addpackageurl+"?projectId="+rowData[$index].projectId+'&packageId='+rowData[$index].id+'&examType='+rowData[$index].examType,
		success: function(layero, index) {
			var iframeWind = layero.find('iframe')[0].contentWindow;
