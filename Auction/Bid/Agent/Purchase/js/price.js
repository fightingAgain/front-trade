				...
				if(packagePrice.length>0){					
					for(var z=0;z<packagePrice.length;z++){
						...
						if(packagePrice[z].priceName=="项目保证金"){
							...
							$('[name=depositPrice]').val(packagePrice[z].price)
							$("#payMethod").html(packagePrice[z].payMethod===0?'虚拟子账号':'指定银行');
							$('#bankType').html(packagePrice[z].bankType== 2?'招商银行':'工商银行');
							...
							if(packagePrice[z].bankType){
								$('input[name="bankType"]').val([packagePrice[z].bankType]);
							};
						}
					};   
				};
