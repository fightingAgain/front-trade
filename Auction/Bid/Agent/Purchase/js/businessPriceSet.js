...
function scoreTypeBtn(_c,_d,type){
	...
	if(_c==2){
		Assessment+='<tr>'
					...
					+'</tr>'
					+'<tr>'
						+'<td rowspan="'+(businessPriceSetData.basePriceType?"6":"5")+'" class="th_bg">1、商务报价参数</td>'						
					+'</tr>'
					if(businessPriceSetData.basePriceType){
					Assessment+='<tr>'
	...
	if(_c==3){
		Assessment+='<tr>'
					...
					+'</tr>'
					+'<tr>'
						+'<td rowspan="'+(businessPriceSetData.basePriceType?"3":"2")+'" class="th_bg" style="border:none">1、商务报价参数：</td>'							
					+'</tr>'
					if(businessPriceSetData.basePriceType){
						Assessment+='<tr>'
							+'<td colspan="3"  style="text-align: left;width:200px">基准价计算范围：'+(businessPriceSetData.basePriceType==1?"计算价格评审环节中有效供应商的报价":businessPriceSetData.basePriceType==2?'计算所有供应商的报价':'')+'</td>'						
