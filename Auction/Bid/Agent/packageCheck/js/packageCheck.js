...
var examTypeCode; //资格审查方式：0为预审1为后审
/// 表格初始化
$(function () {
		...
		columns: [{
			...
			formatter: function (value, row, index) {
				if (row.projectSource == 1) {
					var projectName = '<div>' + row.projectName + '<span class="text-danger"  style="font-weight:bold">(重新采购)</span></div>';
				} else {
					var projectName = '<div>' + row.projectName + '</div>';
				}
				return projectName;
			}
		}, {
...
function audit_btn($index) {
	...
	parent.layer.open({
		...
		content: 'Auction/common/Agent/packageCheck/model/packageCheckLookPSView.html?examType=' + examTypeCode + "&projectId=" + rowData[$index].projectId + '&packageId=' + rowData[$index].id + '&bidHistoryCheckId=' + rowData[$index].historyId,
...
function view_btn($index) {
	...
	parent.layer.open({
		...
		area: ['1100px', '650px'],
		maxmin: true, //开启最大化最小化按钮
		content: 'Auction/common/Agent/packageCheck/model/packageCheckPSView.html?examType=' + examTypeCode + "&projectId=" + rowData[$index].projectId + '&packageId=' + rowData[$index].id + '&bidHistoryCheckId=' + rowData[$index].historyId,
		success: function (layero, index) {
			var iframeWindl = layero.find('iframe')[0].contentWindow;
	...
	if (dataRow.encipherStatus == 1 && dataRow.sysFileState == 1) {
		...
		function openPackageEditPop(changeKeys) {
			parent.layer.open({
				...
				maxmin: true,  //开启最大化最小化按钮
				id: 'packageclass',
				content: 'Auction/common/Agent/packageCheck/model/packageCheckEdit.html?examType=' + examTypeCode + "&projectId=" + rowData[$index].projectId + '&packageId=' + rowData[$index].id + '&changeKeys=' + changeKeys,
				success: function (layero, index) {
		...
		parent.layer.open({
			...
			maxmin: true, //开启最大化最小化按钮
			id: 'packageclass',
			content: 'Auction/common/Agent/packageCheck/model/packageCheckEdit.html?examType=' + examTypeCode + "&projectId=" + rowData[$index].projectId + '&packageId=' + rowData[$index].id,
			success: function (layero, index) {
				var iframeWindsl = layero.find('iframe')[0].contentWindow;
			}
		});
...
function update_btn($index) {
	...
	parent.layer.open({
		...
		,
		id: 'packageclass',
		content: 'Auction/common/Agent/packageCheck/model/packageCheckUpdate.html?examType=' + examTypeCode + "&projectId=" + rowData[$index].projectId + '&packageId=' + rowData[$index].id + '&flagType=' + rowData[$index].historyFlagType + '&historyCheckState=' + rowData[$index].historyCheckState,
		success: function (layero, index) {
			var iframeWindsl = layero.find('iframe')[0].contentWindow;
		}
		//确定按钮
