...
var CheckItemUrl = config.bidhost + '/CheckItemHistoryController/list.do'
var findBusinessPriceList = config.bidhost + '/PriceSetHistoryController/findBusinessPriceList.do';//变更记录接口
var supplierType;
var AccessoryList = [];
...
var bidHistoryCheckId = getUrlParam('bidHistoryCheckId');
...
$(function () {
	$.ajax({
		type: "post",
		url: config.bidhost + "/ProjectPackageController/findById.do",
		data: {
			'id': packageId
		},
		async: false,
		...
		success: function (response) {
			if (response.success) {
				packageInfodata = response.data;
			}
		}
	});
	$('div[id]').each(function () {
		$(this).html(packageInfodata[this.id]);
	});
	if (supplierType != 1) {
		$.ajax({
	...
	findWorkflowCheckerAndAccp(bidHistoryCheckId);
})
function packagedd() {
...
function setpackageCheckListInfo($index) {//Num询评审内容的下标，$index包件的下标
	...
	if (packageCheckListInfo.length > 0) {
		...
		strHtml += '<tr class="' + (packageCheckListInfo[_index].checkType == 2 ? "" : "none") + '">'
			...
			+ '</tr>'
			+ '<tr class="' + ((packageInfo.checkPlan == 1||packageInfo.checkPlan == 5) && packageCheckListInfo[_index].checkType == 2 ? "" : "none") + '">'
			+ '<td style="text-align:right">是'+(packageInfo.checkPlan == 4?'减':'加')+'价：</td>'
	...
	$('#packageSetBtn').click(function () {
		parent.layer.open({
			...
			, id: 'packageSet'
			, area: ['1000px', '600px']
			, content: 'Auction/AuctionOffer/Agent/AuctionPurchase/viewAuctionPackage.html?packageId=' + packageInfo.id + '&biddingTimes=1&supplierRound=1&isbidding=1'
		});
	})
};
...
function view($index) {
	var id = $('#workflowListDetailChange').bootstrapTable('getData')[$index].id; //bootstrap获取当前页的数据
	...
	parent.layer.open({
		...
		,
		content: 'Auction/common/Agent/packageCheck/model/packageCheckLookPS.html?id=' + id,
