...
var searchProjectInfo = parent.config.bidhost + '/AskAnswersController/findProjectInfoForAskAnswers.do';

var inviteState; //项目邀请状态
var examType;    //项目资格审查方式
var examTypeCode = getQueryString("examType");//当前阶段资格审查方式
var packageId = getQueryString("id");
var projectId = getQueryString("projectId");
var createType = getQueryString("createType") ;//是否项目创建人  0是  1不是
var tenderType = 0;//询比采购
var purchase; //项目信息
	...
	$.ajax({
		...
		success: function(data) {
			if(data.success) {
				...
				examType = purchase.examType;//项目资格审查方式
				inviteState = purchase.inviteState;//项目邀请状态
...
function AddFunction(value, row, index) { //把需要创建的按钮封装到函数中
	...
	if(createType != undefined && createType == 1) {
		return [
			'<button type="button" id="TableEditor" class="btn-xs btn btn-primary"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span>查看</button>',
		].join("")
	} else {
		...
		if(answersEmployeeId == "" || typeof(answersEmployeeId) == "undefined" || answersEmployeeId == null) {
			if(endType == '1') {
				...
				return [
					'<button type="button" id="editAnswer" class="btn-xs btn btn-primary"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span>答复</button>',
					'<button type="button" id="TableEditor" class="btn-xs btn btn-primary"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span>查看</button>',
				].join("")
			}
		} else {
...
window.operateEvents = { //添加一个按钮对应的事件
	...
	"click #editAnswer": function(e, value, row, index) {
		...
		parent.layer.open({
			...
			resize: false, //是否允许拉伸
			area: ['800px', '600px'],
			content: 'Auction/common/Agent/AnswerAsk/CommitAnswer.html' + '?examType=' + examType + '&inviteState=' + inviteState + "&tenderTypeCode=" + tenderType + '&examTypeCode=' + examTypeCode,
			// btn: ['提交', '取消']
			// 	//确定按钮
			// 	,
			yes: function(index, layero) {
				...
				if(iframeWin.$("#answersContent").val().length == 0) {
					top.layer.alert("答复内容不能为空");
					return;
				};

				if(iframeWin.$("#answersContent").val().length > 1000) {
					top.layer.alert("答复内容不能超过1000字");
					return;
				};
				iframeWin.Ti();
			...
			success: function(layero, index) {
				...
				iframeWin.$("#projectCode").text(purchase.projectCode);
				iframeWin.$("#packageName").text(purchase.packageName);
				iframeWin.$("#packageNum").text(purchase.packageNum);
	...
	"click #TableEditor": function(e, value, row, index) {
		...
		parent.layer.open({
			...
			maxmin: true, //该参数值对type:1和type:2有效。默认不显示最大小化按钮。需要显示配置maxmin: true即可
			resize: false, //是否允许拉伸
			content: 'Auction/common/Agent/AnswerAsk/AskContent.html',
			success: function(layero, index) {
				var body = parent.layer.getChildFrame('body', index);
				var iframeWin = layero.find('iframe')[0].contentWindow;
				iframeWin.show(row);
