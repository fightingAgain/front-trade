...
var urlInsertNewProjectBidResult = top.config.bidhost + "/ProjectBidResultController/insertNewProjectBidResult.do?";//发布提交新接口代理服务费 
var urlSendMsg = top.config.bidhost + "/InFormController/inFormUser";
...
$(function () {
	...
	if (type != "RELEASE" && type != "CHANGE") { //查看
		...
		if(examType == 1){
			resultNoticeCheckSatisfaction('0', projectId, packageId);//验证满意度评价
		}
		$('.view').hide();
		$('.edit').show();
		if(type == "CHANGE"){
	...
	$.ajax({
		...
		success: function (result) {
			...
			if (type == "RELEASE" || type == "CHANGE") { //当发布时
				...
				/*************            公告模板                 */
				if(findPackageInfoData.projectType==0){//项目的项目类型。
					var projectTypes='A';
				}
				if(findPackageInfoData.projectType==1){
					var projectTypes='B';
				}
				if(findPackageInfoData.projectType==2){
					var projectTypes='C';
				}
				if(findPackageInfoData.projectType==3){
					var projectTypes='C50';
				}
				if(findPackageInfoData.projectType==4){
					var projectTypes='W';
				}
				...
				if (examType == 0) {
					...
					for (var i = 0; i < BidResultItem.length; i++) {
						...
						if((type == "RELEASE" || type == "CHANGE") && isReminderCollection(BidResultItem[i].enterpriseCode)){
							html += "<td style='text-align:left'><span class='glyphicon glyphicon-exclamation-sign' style='color: #ccbb09;margin-right: 5px'></span>" + showBidderRenameList(BidResultItem[i].supplierId, BidResultItem[i].enterpriseName, RenameData, 'body') + "</td>";
						}else{
					...
					$(".bidPrice").on('change', function (_index) {
						...
						if (BidResultItem[_pindex].resultContent != "" && BidResultItem[_pindex].resultContent != undefined) {
							parent.layer.confirm('温馨提示：修改成交价格后您编辑的内容将会改变，确定修改？', {
								btn: ['是', '否'] //可以无限个按钮
							}, function (index, layero) {
				...
				$('#content').html(data.content?data.content:'');
				$('[name=pushPlatform][value='+data.pushPlatform+']').attr('checked','checked');
...
$("#btn_submit").on("click", function () {
	if(examType == 1){
		//验证满意度评价
		resultNoticeCheckSatisfaction('0', projectId, packageId, function(){
			submit()
		})
	}else{
		submit()
...
function submit(){
	...
	$("#btn_submit").attr("data-type") == 0 ? bidResult.checkState = 0 : bidResult.checkState = 1;
	if (keyId && keyId != '' ) {
		bidResult.id = keyId
	}
	...
	/*start代理服务费*/
	//opraType:1是提交0是保存
	$("#btn_submit").attr("data-type") == 0 ? bidResult.opraType = 0 : bidResult.opraType = 1;
	var newUrl;
	var notText = '';
	if ($("#btn_submit").attr("data-type") == 0) {
		...
		if (examType == 1) {
			var params = {
				'relationId': findPackageInfoData.packageId,
				...
				if (data.success) {
					...
					if (projectServiceFee && ($("#btn_submit").attr("data-type") == 0)) {
						openAgentFee({
							...
							packageId: packageId,
							projectId: projectId,
							agentPurchaseType: tenderTypeCode,
							examType: examType,
							unit: quotePriceUnit,
							...
							content: content,
							id: keyId,
							optionId: bidResult.optionId,
							optionValue: bidResult.optionValue,
							optionName: bidResult.optionName,
							isOpen: bidResult.isOpen,
...
function editBidResult($index) {
	...
	top.layer.open({
		...
		closeBtn: 1,
		// btn: ["保存", "取消"],
		content: 'Auction/common/Agent/BidNotice/modal/editViewResult.html',
		success: function (layero, index) {
			var body = layer.getChildFrame('body', index);
			var iframeWin = layero.find('iframe')[0].contentWindow;
