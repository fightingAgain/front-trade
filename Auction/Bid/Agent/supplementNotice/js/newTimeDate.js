...
function time(examType){
	...
	$('form').on('click', '#noticeEndDate', function(){
		...
		if(NewDate(packageInfo.noticeStartDate)>NewDate(nowSysDate)){	
			var noticeMin=packageInfo.noticeStartDate.substring(0,10);
		}else{
			var noticeMin=nowSysDate.substring(0,10);
		}				
		WdatePicker({
			el:this,
	...
	$('form').on('click', '#bidEndDate', function(){
		...
		if($('#noticeEndDate').val()!=""){
			if(NewDate($('#noticeEndDate').val())>NewDate(nowSysDate)){	
				...
				bidEndMin=nowSysDate;//否则则是当前时间为最小值
			}				
		}else{
			bidEndMin=nowSysDate;//否则则是当前时间为最小值
		}
	...
	$('form').on('click', '#checkEndDate', function(){
		...
		if($('#bidEndDate').val()!=""){
			if(NewDate($('#bidEndDate').val())>NewDate(nowSysDate)){	
				...
				checkEndMin=nowSysDate;//否则则是当前时间为最小值
			}				
		}else{
			checkEndMin=nowSysDate;//否则则是当前时间为最小值
		}
...
function datetimepickers(examType){
		...
		timeList+='<tr>'
		   timeList+='<td  class="th_bg" >'+ (examType==1?'公告开始时间':'资格预审公告开始时间') +'</td>'
     		timeList+='<td style="text-align: left;" colspan="'+ (packageInfo.isSign==0?'3':"") +'" >'
				 ...
				 timeList+='<input type="hidden" data-datename="'+ (examType==1?'公告开始时间':'资格预审公告开始时间') +'" name="noticeStartDate" value="'+ packageInfo.noticeStartDate +'">'
     			if(packageInfo.isSellFile==0){
	     			timeList+='<input type="hidden" class="btn btn-default Wdate ageinTime" id="sellFileStartDate"  name="sellFileStartDate" value="'+ packageInfo.noticeStartDate +'" style="width: 200px;text-align: left;" readonly="readonly">'
	     		}
     		timeList+='</td>'
     		if(packageInfo.isSign==1){ 
     	...
     	timeList+='</tr>'
     	timeList+='<tr>'     		
     		timeList+='<td class="th_bg" style="width:250px;">'+ (examType==1?'原始公告截止时间':'原始资格预审公告截止时间') +'</td>'
			...
			timeList+='<td style="text-align: left;">'
				...
				if(packageInfo.isSellFile==0){
					timeList+='<input type="hidden" class="btn btn-default ageinTime Wdate" id="sellFileEndDate"  name="sellFileEndDate" style="width: 200px;text-align: left;" readonly="readonly">'
				}
			timeList+='</td>'
		timeList+='</tr>'
		if(packageInfo.isSign==1){    		
	        timeList+='<tr>'     		
	     		timeList+='<td class="th_bg" style="width:250px;">原始报名截止时间</td>'
				timeList+='<td class=""style="text-align: left;">'
	...
	if(examType==1){			   	
     	timeList+='<tr>'
     		...
     		timeList+='<td style="text-align: left;">'
     			timeList+='<input type="text" autocomplete="off" data-min="noticeEndDate" data-max="answersEndDate" data-datename="提出澄清截止时间" class="btn btn-default Wdate ageinTime" id="askEndDate"  name="askEndDate" style="width: 200px;text-align: left;">'
     		timeList+='</td >'
     	timeList+='</tr>'
     	timeList+='<tr>'
     		...
     		timeList+='<td style="text-align: left;">'
     			timeList+='<input type="text" autocomplete="off" data-min="answersEndDate" data-datename="答复截止时间" class="btn btn-default Wdate ageinTime" id="answersEndDate"  name="answersEndDate" style="width: 200px;text-align: left;">'
     		timeList+='</td >'
     	timeList+='</tr>'
     	timeList+='<tr>'
     		...
     		timeList+='<td  style="text-align: left;">'
     			timeList+='<input type="text" autocomplete="off" data-min="'+ (packageInfo.isSign==1?'signStartDate':'noticeEndDate') +'" data-max="checkEndDate" data-datename="报价截止时间" class="btn btn-default Wdate ageinTime" name="offerEndDate"  id="bidEndDate"  style="width: 200px;text-align: left;">'
     		timeList+='</td>'    		    		
     	timeList+='</tr>'
     	timeList+='<tr>'
     		...
     		timeList+='<td  style="text-align: left;">'
     			timeList+='<input type="text" autocomplete="off" data-min="offerEndDate" data-datename="' + reviewStartTimeLabel + '" class="btn btn-default Wdate ageinTime" name="checkEndDate"  id="checkEndDate"  style="width: 200px;text-align: left;">'
     		timeList+='</td>'    		    		
     	timeList+='</tr>'
	}else{		
     	timeList+='<tr>'
     	...
     	timeList+='<tr>'
     		...
     		timeList+='<td  style="text-align: left;">'
     			timeList+='<input type="text" autocomplete="off"  data-min="submitExamFileEndDate"  data-datename="预审评审时间" class="btn btn-default Wdate ageinTime" name="examCheckEndDate" id="checkEndDate"   style="width: 200px;text-align: left;">'
     		timeList+='</td>'    		    		
     	timeList+='</tr>'
	}
	...
	if(examType==1){
		timeList+='<tr>'
		   timeList+='<td  class="th_bg" >接受邀请开始时间</td>'
			 timeList+='<td style="text-align: left;" colspan="'+ (packageInfo.isSign==1?'1':"3") +'">'
				 timeList+='<div id="oldnoticeStartDate">'+ (packageInfo.noticeStartDate!=undefined?packageInfo.noticeStartDate.substring(0,16):"") +'</div>'
				 timeList+='<input type="hidden" data-datename="接受邀请开始时间" name="noticeStartDate" value="'+ packageInfo.noticeStartDate +'">'
				 if(packageInfo.isSellFile==0){
					 timeList+='<input type="hidden" class="btn btn-default ageinTime Wdate" id="sellFileStartDate"  name="sellFileStartDate" value="'+ packageInfo.noticeStartDate +'" style="width: 200px;text-align: left;" readonly="readonly">'
				 }	     			
			 timeList+='</td>'
			 if(packageInfo.isSign==1){
				 timeList+='<td  class="th_bg" style="width:250px;" colspan="'+ (packageInfo.isSign==0?'3':"") +'">报名开始时间</td>'
				 timeList+='<td style="text-align: left;">'
					 timeList+='<div id="oldsignStartDate">'+(packageInfo.signStartDate!=undefined?packageInfo.signStartDate.substring(0,16):"")+'</div>'	
				 timeList+='</td>'  
			 }
			 timeList+='</tr>'
			 timeList+='<tr>'     		
				 timeList+='<td class="th_bg" style="width:250px;">原始接受邀请截止时间</td>'
				timeList+='<td class=""style="text-align: left;">'
					timeList+='<div id="oldnoticeEndDate">'+(packageInfo.noticeEndDate!=undefined?packageInfo.noticeEndDate.substring(0,16):"")+'</div>'		
				timeList+='</td>'
				 timeList+='<td class="th_bg" style="width:250px;">接受邀请截止时间</td>'
				timeList+='<td style="text-align: left;">'
					timeList+='<input type="text" autocomplete="off" data-min="noticeStartDate" data-datename="接受邀请截止时间" class="btn btn-default Wdate ageinTime" id="noticeEndDate"  name="noticeEndDate" style="width: 200px;text-align: left;">'
					timeList+='<input type="hidden"  id="acceptEndDate" name="acceptEndDate"/>'
					if(packageInfo.isSellFile==0){
						timeList+='<input type="hidden" class="btn btn-default Wdate ageinTime" id="sellFileEndDate"  name="sellFileEndDate" style="width: 200px;text-align: left;" readonly="readonly">'
					}
				timeList+='</td>'
			timeList+='</tr>'
		 if(packageInfo.isSign==1){	     		
	 ...
	 timeList+='<tr>'
		 ...
		 timeList+='<td style="text-align: left;">'
			 ...
			 timeList+='<input type="hidden" class="btn btn-default ageinTime Wdate"  id="sellPriceFileStartDate" name="sellPriceFileStartDate" value="'+ packageInfo.noticeStartDate +'"  style="width: 200px;text-align: left;">'
		 timeList+='</td >'
	 timeList+='</tr>'
	 timeList+='<tr>'
		 timeList+='<td  class="th_bg" >原始答复截止时间</td>'
	 ...
	 if(examType==0){
		 timeList+='<tr>'
			 ...
			 timeList+='<td  style="text-align: left;">'
				 timeList+='<input type="text" autocomplete="off" data-min="today" data-max="offerEndDate"  data-datename="接受邀请截止时间" class="btn btn-default Wdate ageinTime"  id="acceptEndDate" name="acceptEndDate" style="width: 200px;text-align: left;">'	     			
				 timeList+='<input type="hidden" class="btn btn-default Wdate ageinTime"  id="sellPriceFileEndDate" name="sellPriceFileEndDate" style="width: 200px;text-align: left;">'
	 ...
	 timeList+='<tr>'
		 ...
		 timeList+='<td  class="th_bg" >报价截止时间</td>'
		 timeList+='<td  style="text-align: left;">'
			 timeList+='<input type="text" autocomplete="off" data-min="'+(examType==0?'acceptEndDate':'noticeEndDate') +'" data-max="checkEndDate"  data-datename="报价截止时间" class="btn btn-default Wdate ageinTime Wdate" name="offerEndDate"  id="bidEndDate"  style="width: 200px;text-align: left;">'
		 timeList+='</td>'    		    		
	 timeList+='</tr>'
	 timeList+='<tr>'
		 timeList+='<td  class="th_bg" >原始' + reviewStartTimeLabel + '</td>'
