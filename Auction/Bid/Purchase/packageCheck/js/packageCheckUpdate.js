...
var CheckListlist = config.bidhost + '/CheckListHistoryController/list.do'//
var CheckListSave = config.bidhost + '/CheckListHistoryController/save.do' //评审项添加
var deleteChenkUrl = config.bidhost + '/CheckListHistoryController/delete.do'//评审项删除
var updateChenkUrl = config.bidhost + '/CheckListHistoryController/update.do'//评审项修改


//var CheckItemUrl=config.bidhost +'/PackageCheckItemController/list.do'//评价项查询
var CheckItemUrl = config.bidhost + '/CheckItemHistoryController/list.do'//评价项查询
var CheckItemsave = config.bidhost + '/CheckItemHistoryController/save.do'//评价项添加
var CheckItemdelete = config.bidhost + '/CheckItemHistoryController/delete.do'//评价项删除
var CheckItemupdate = config.bidhost + '/CheckItemHistoryController/update.do'//评价项修改
var saveByExcel = config.bidhost + '/CheckItemHistoryController/saveByExcel.do'//批量导入评价项

var CheckUrl = "Auction/common/Purchase/packageCheck/model/add_CheckPS.html" //添加评价项页面
var tempCheckPlanUrl = config.bidhost + "/tempPackageCheckController/findPackageCheckList.do"//模板接口
var savePackageCheckAllUrl = config.bidhost + '/PriceSetHistoryController/savePackageCheckAll.do'//选择评审项模板
var excelDownloadUrl = config.bidhost + "/OutSidesController/mbDownload.do";//Excel下载接口
...
var historyCheckState = getUrlParam('historyCheckState');
var flagType;
var packageCheckListInfo = []//评审项信息
var packageInfo, offerFile;
var clearBidding = null;
$(function () {
	if (getUrlParam('flagType') == "" || getUrlParam('flagType') == 'undefined') {
		flagType = 1
	} else if (historyCheckState == 2) {
		flagType = parseInt(getUrlParam('flagType')) + 1
		historyCheckState = 0
	} else {
		flagType = parseInt(getUrlParam('flagType'))
	}
...
function packagedd() {
	...
	if (examType == 1) {
		...
		$(".tenderTypeW").show();
		if (packageInfo.checkPlan == 1 || packageInfo.checkPlan == 3 || packageInfo.checkPlan == 5) {//判断评审方法，如果为最低评标价法或最低投标价法
		...
		//		$('input[name="doubleEnvelope"][value="'+ (packageInfo.doubleEnvelope) +'"]').prop('checked',true);
		//		doubleEnvelopes=$('input[name="doubleEnvelope"]:checked').val();//当前双信封状态		
...
function site() {
	...
	if (examType == 1 && packageCheckListInfo.length > 0) {
		doubleEnvelopeCheck = packageCheckListInfo[packageCheckListInfo.length - 1].envelopeLevel;//获取最新一个评审项的信封
	}
	parent.layer.open({
		type: 2
		, title: '添加评审项'
		, area: ['550px', '500px']
		, content: 'Auction/common/Purchase/packageCheck/model/historyCheckList.html?examType=' + examType + '&checkPlan=' + checkPlan + '&packageId=' + packageInfo.id + '&sort=' + (packageCheckListInfo.length + 1) + '&isShowEnvelope=' + doubleEnvelopes + '&isOpenDarkDoc=' + isOpenDarkDoc + '&flagType=' + flagType
	});
}
//修改评审项
function edit() {
	...
	parent.layer.open({
		...
		, title: '修改评审项'
		, area: ['650px', '500px']
		, content: 'Auction/common/Purchase/packageCheck/model/historyCheckList.html?examType=' + examType + '&checkPlan=' + checkPlan + '&id=' + packageCheckListId + '&packageId=' + packageInfo.id + '&index=' + _$index + '&isShowEnvelope=' + doubleEnvelopes + '&isOpenDarkDoc=' + isOpenDarkDoc + '&flagType=' + flagType
	});
};
//删除评审项
...
function add_Check(num, val) {
	parent.layer.open({
		...
		, title: '添加评价项'
		, area: ['650px', '600px']
		, content: CheckUrl + '?packageCheckListInId=' + packageCheckListInfo[num].id + '&checkType=' + packageCheckListInfo[num].checkType + '&index=' + num + '&flagType=' + flagType
...
function itemEdit(uid, $index, $num) {
	parent.layer.open({
		...
		, title: '编辑评价项'
		, area: ['650px', '600px']
		, content: CheckUrl + '?packageCheckListInId=' + packageCheckListInfo[$num].id + '&id=' + uid + '&checkType=' + packageCheckListInfo[$num].checkType + '&index=' + $num + '&flagType=' + flagType
...
function exportExcel(_index) {
	...
	if (_index == 0 || _index == 2) {
		...
		var url = excelDownloadUrl + "?fname=评分制模板&filePath=Pur_Scoring_System_Derived_Template.xls";
	} else if (_index == 3) {
		// var url = config.bidhost + "/FileController/download.do?fname=评分合格制模板.xls&ftpPath=/Templates/Pur/Pur_Scoring_System_Derived_Template.xls";
		//var url = config.bidhost + "/FileController/download.do?fname=评分合格制模板.xls&ftpPath=/Templates/Pur/Pur_Scoring_System_Derived_Template.xls";
		var url = excelDownloadUrl + "?fname=评分合格制模板&filePath=Pur_Accreditation_System_Derived_Template.xls";
	}
...
function setpackageCheckListInfo(_index) {//Num询评审内容的下标，$index包件的下标
	if (_index > 0) {
		doubleEnvelopeCheck = packageCheckListInfo[_index - 1].envelopeLevel;//获取最新一个评审项的信封
	}
	if (packageCheckListInfo.length > 0) {
		packageCheckListId = packageCheckListInfo[_index].id;
		_$index = _index;
		...
		if (packageInfo.checkPlan != 3 && packageCheckListInfo[_index].checkType == 2) {
			...
			if (packageCheckListInfo[_index].isAddPrice == 0) {
				strHtml += '<tr>'
					+ '<td style="width:200px;height:30px;line-height:30px;text-align:right">偏离加价幅度：</td>'
		...
		strHtml += '<tr>'
			...
			+ '<td style="text-align: left;" colspan="3">'
			+ ' <input type="radio" name="isChecks" class="isChecks" value="0" onchange="Is_Show_Checks(' + _index + ',0,1)" />是'
			+ '<input type="radio" name="isChecks" class="isChecks" value="1" onchange="Is_Show_Checks(' + _index + ',1,1)" />否'
			+ '</td>'
			+ '</tr>';
		if (packageCheckListInfo[_index].checkType != 4) {
			strHtml += '<tr id="Show_Check" class="none">'
				...
				+ '<td style="text-align: left;" colspan="3">'
				+ '<input type="checkbox" name="showCheckInfo" value="0" />评价内容'
				+ ' <input type="checkbox" name="showCheckInfo" value="1" />评价标准'
				+ '<input type="checkbox" name="showCheckInfo" value="2" />' + (packageCheckListInfo[_index].checkType == 1 || packageCheckListInfo[_index].checkType == 3 ? '分值' : '是否关键要求')
				+ ' <input type="checkbox" name="showCheckInfo" value="3" />备注'
				+ '</td>'
				+ '</tr>'
				+ '<tr>'
				+ '<td colspan="4" style="text-align: left;">'
				+ '<input type="button" class="btn btn-primary" style="margin-right:10px" value="添加评价项" onclick=add_Check(\"' + _index + '\",\"' + packageCheckListInfo[_index].checkType + '\") />'
				+ '<input type="button" class="btn btn-primary" style="margin-right:10px" value="导出评价项模板" onclick=exportExcel(\"' + packageCheckListInfo[_index].checkType + '\") />'
				+ '<span class="btn btn-primary fileinput-button" style="margin-right:10px">'
				+ '<span>导入评价项</span>'
				+ '<input type="file" onchange="Excel(this,' + _index + ')">'
				+ '</span>'
				+ '<input type="button" class="btn btn-danger"  value="一键清空" onclick=deletAll(\"' + _index + '\",\"' + packageCheckListInfo[_index].id + '\") />'
				+ '</td>'
				+ '</tr>'
				+ '</table>'
			...
			if (packageCheckListInfo[_index].checkType == 1 || packageCheckListInfo[_index].checkType == 3) {
				strHtml += '<tr>'
					...
					+ '<p><input type="radio" name="totalType" value="1" ' + (packageCheckListInfo[_index].totalType == 1 ? 'checked' : '') + ' onchange=Total_Type_btn(\"' + _index + '\",1)>方法一、评委全体成员评分计算出平均得分，即为供应商评审因素最后得分</p>'
					+ '<p><input type="radio" name="totalType" value="0" ' + (packageCheckListInfo[_index].totalType == 0 ? 'checked' : '') + ' onchange=Total_Type_btn(\"' + _index + '\",0)>方法二、评委全部的评分去掉一个最高分和最低分后计算出平均得分，即为供应商评审因素最后得分</p>'
				if (packageCheckListInfo[_index].checkType == 3) {
...
function deletAll($num, uid) {
	parent.layer.confirm('确定要删除该评审项的全部评价项', {
		...
		$.ajax({
			url: config.bidhost + '/CheckItemHistoryController/clean.do',
			type: 'post',
			dataType: 'json',
			async: false,
			//contentType:'application/json;charset=UTF-8',
...
function saveForm() {
	if (examType == 1) {
		...
		if (businessPriceSetData == "" || businessPriceSetData == undefined) {
			$.ajax({
				url: config.bidhost + '/PriceSetHistoryController/insertBusinessPriceSet.do',
				type: 'post',
				dataType: 'json',
				async: false,
				//contentType:'application/json;charset=UTF-8',
				data: $('#formb').serialize() + '&packageId=' + packageId + '&checkState=' + 1 + '&flagType=' + flagType,
				success: function (data) {
					if (data.success) {
						if (top.window.document.getElementById("consoleWindow")) {
							var thisFrame = top.window.document.getElementById("consoleWindow").getElementsByTagName("iframe")[0].id;
			...
			$.ajax({
				url: config.bidhost + '/PriceSetHistoryController/updateBusinessPriceSet.do',
				type: 'post',
				dataType: 'json',
				async: false,
				//contentType:'application/json;charset=UTF-8',
				data: $('#formb').serialize() + '&id=' + businessPriceSetData.id + '&packageId=' + packageId + '&checkState=' + 1 + '&flagType=' + flagType + '&examType=' + examType,
				success: function (data) {
					if (data.success) {
						if (top.window.document.getElementById("consoleWindow")) {
							var thisFrame = top.window.document.getElementById("consoleWindow").getElementsByTagName("iframe")[0].id;
