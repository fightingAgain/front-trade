...
$(function(){
	...
	if(examType==1&&(checkPlan==1||checkPlan==3||checkPlan==5)){//资格后审，且为最低价法
		$(".checkType0").show();
		$(".checkType2").show();
		$(".checkType3").show();
		$(".checkType4").show();
	...
	$("input[name='checkType']").on('click',function(){
		...
		if($(this).val()==2){
			...
			$(".gather").show();
			$(".isSetTotal").show();
			if(examType==1&&(checkPlan==1 || checkPlan==5)){
				$(".isAddPrice").show();
			}			
			$(".addPrice").hide();
			$(".gather3").hide();	
...
function test(){
	...
	if($("input[name='checkType']:checked").val()==2){
		...
		if($("input[name='isAddPrice']:checked").val()==0){
			if($('#addPrice').val()==""){
				return '温馨提示：请填写偏离加价幅度'
			}
			if(!(/^((\d+\.\d*[1-9]\d*)|(\d*[1-9]\d*\.\d+)|(\d*[1-9]\d*))$/.test($('#addPrice').val()))){
				return "温馨提示：偏离加价幅度只能是大于零的数字";
			};
			if($('#addPrice').val()>100){
				return "温馨提示：偏离加价幅度不能超过100";
			};
		}
	}	
	if($("input[name='checkType']:checked").val()==3){
