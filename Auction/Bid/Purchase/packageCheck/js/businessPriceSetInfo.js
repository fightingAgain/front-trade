...
function priceExaminel(_b){
	...
	if(_b==0){	
      scoreTypes+=
      		'<input class="change-disabled" type="radio" name="checkType" value="0" onclick="scoreTypeBtnl(0)"/>自定义评分法' +
			'<input class="change-disabled" type="radio" name="checkType" value="1" onclick="scoreTypeBtnl(1)"/>最低有效投标价法' +
			'<input class="change-disabled" type="radio" name="checkType" value="2" onclick="scoreTypeBtnl(2)"/>基准价评分法' +
			'<input class="change-disabled" type="radio" name="checkType" value="3" onclick="scoreTypeBtnl(3)"/>最低报价为基准价法' +
			'</br><input class="change-disabled" type="radio" name="checkType" value="5" onclick="scoreTypeBtnl(5)"/>固定比例不取整法' +
					...
					+'<tr>'
						+'<td style="text-align: left;">'
							+'<div class="input-group">'
								...
								// +'<div class="input-group-addon">'+ (top.prieUnit||"元" )+'</div>'
							+'</div>'								
						+'</td>'						
						+'<td colspan="2" style="text-align: left;">'
						    +'<div class="input-group" style="width:100%">'
...
//			if(!(/^[+]{0,1}(\d+)$|^[+]{0,1}(\d+\.\d+)$/.test($(this).val()))){
	...
	if(_c==0){
		...
		if(businessPriceSetData != undefined &&businessPriceSetData.businessContent!=undefined&&businessPriceSetData.businessContent!=""&&businessPriceSetData.businessContent!=null){
			ue.ready(function() {
