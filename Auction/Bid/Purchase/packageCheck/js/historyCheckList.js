...
var findData = config.bidhost + "/CheckListHistoryController/showOne.do"//评审项详情
...
var flagType = getUrlParam("flagType");//变更次数
...
$(function () {
	...
	if (examType == 0 && checkPlan == 1) {//资格预审，且为有效数量制
		$(".checkType0").show();
		$(".checkType1").show();
		$(".gather").show();
	}
	...
	if (id) {
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					bidpriceTimes = response.data.bidpriceTimes;
...
function test() {
			...
			for (var i = 0; i < packageCheckListInfo.length; i++) {
				if (packageCheckListInfo[i].sort < $("#sort").val()) {
					if (packageCheckListInfo[i].envelopeLevel == 2 && $('input[name="envelopeLevel"]:checked').val() == 1) {
						return '温馨提示：第二信封评审项后不能添加第一信封评审项。'
					}
				}
				if (packageCheckListInfo[i].sort > $("#sort").val()) {
	...
	if ($("input[name='checkType']:checked").val() == 1) {
		if (examType == 1) {
			...
			if (!(/^0\.([1-9]|\d[1-9])$|^[1-9]\d{0,8}\.\d{0,2}$|^[1-9]\d{0,8}$/.test($('#weight').val())) || $('#weight').val() > 1) {
