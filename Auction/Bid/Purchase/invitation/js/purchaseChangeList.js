...
var addpackage = 'Auction/common/Purchase/invitation/model/purchaseChangeInfo.html';
var editpackage = 'Auction/common/Purchase/invitation/model/purchaseChangeEdit.html';
var viewpackage = 'Auction/common/Purchase/invitation/model/noticeChangeView.html';
var iframeWinAdd = "";
//表格初始化
$(function() {
	initTable();
			...
			},
			{
				...
				formatter: function(value, row, index) {
					...
					if(row.createType != undefined && row.createType == 1) {
						Tdr = '<button class="btn-xs btn btn-primary" type="button" onclick=viewbao(\"' + index + '\")><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span>查看</button>'
					} else {

						if(row.checkState == 1 || row.checkState == 2) {
							Tdr = '<button class="btn-xs btn btn-primary" type="button" onclick=viewbao(\"' + index + '\")><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span>查看</button>'
						} else {
							Tdr = '<button class="btn-xs btn btn-primary" type="button" onclick=eidtbao(\"' + index + '\")><span class="glyphicon glyphicon-edit" aria-hidden="true"></span>编辑</button>'
						}
					}
					return Tdr
				}
