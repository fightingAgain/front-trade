	...
	$('#table').bootstrapTable({
		...
		columns: [{
			...
			formatter: function(value, row, index) {
				...
				//撤回
				var States = '<button type="button" class="btn-xs btn btn-warning" onclick=recall(\"' + row.id + '\",\"' + row.examType + '\")>' +
				...
				var audit = '<button type="button" class="btn-xs btn btn-primary"onclick=audit_btn(\"' + index + '\")>' +
					...
					'</button>'
				// 删除
				var delet = '<button class="btn-xs btn btn-danger" onclick=deletPackage(\"' + index + '\")><span class="glyphicon glyphicon-trash" aria-hidden="true"></span>删除</button>'
...
function edit_btn($index) {
	...
	if(rowData[$index].examType == 1) {
		if(rowData[$index].packageSource == 1){
			var inurl ="Auction/common/Purchase/invAgainPurchase/model/invitationInfo.html"
		}else{
			var inurl = 'Auction/common/Purchase/invitation/model/invitationInfo.html'
		}	
	} else if(rowData[$index].examType == 0) {
		var inurl = 'Auction/common/Purchase/invitation/model/examInvitationInfo.html'
	}
	parent.layer.open({
		type: 2 //此处以iframe举例
			,
...
function audit_btn($index) {
	var rowData = $('#table').bootstrapTable('getData'); //bootstrap获取当前页的数据  
	if(rowData[$index].examType == 1) {
		var inurl = 'Auction/common/Purchase/invitation/model/invitationCheck.html'
	} else if(rowData[$index].examType == 0) {
		var inurl = 'Auction/common/Purchase/invitation/model/examInvitationCheck.html'
	}
	parent.layer.open({
		type: 2 //此处以iframe举例
			,
