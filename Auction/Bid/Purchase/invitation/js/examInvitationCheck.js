...
//	DetailedItemData=JSON.parse(sessionStorage.getItem('DetailedItemData'));//读取分项信息的缓存	
	...
	packageDetailData();	
//	if(businessPriceSetData!=undefined){
...
//  $("#isOfferDetailedItem").html(packageInfo.isOfferDetailedItem==0?'需要分项报价表':'不需要分项报价表');	
	...
	if(packageInfo.isSellPriceFile==0||packageInfo.isSellPriceFile==undefined){//是否发售文件0为是1为否
		$(".isFtimewCol").attr('colspan','1')
		$('.isFtimew').show();
		$("#isSellPriceFile").html("发售文件")
	}else{
		$(".isFtimewCol").attr('colspan','3')
		$('.isFtimew').hide();
		$("#isSellPriceFile").html("不发售文件")
	}
		...
		},
		{
			...
			formatter:function(value, row, index){					
				var Tdr='<div class="btn-group">'
					...
					if(row.isAccept!=0&&row.isAccept!=1&&isAgent==0){
						if(NewDate(packageInfo.acceptEndDate) > NewDate(PublicnowDate)){    	
