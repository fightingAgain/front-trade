...
var newData="";
var purchaserIds;
function du(isPublic,purchaserId){	
...
　　　　　 checkboxed.indexOf(v.id) == -1 ? checkboxed.push(v.id) : -1;
          data.push({supplierId:v.id,checkbox:true,enterprise:v});      
　　　　	});
...
function checkbox(){ 
        sessionStorage.setItem('keysjd', JSON.stringify(checkboxed) );
        sessionStorage.setItem('sadasd', JSON.stringify(newData));
};
