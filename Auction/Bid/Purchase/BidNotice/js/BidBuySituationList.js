...
window.operateEvents = {
	...
	"click #checkDetail": function(e, value, row, index) {
		sessionStorage.setItem("BidSituation", JSON.stringify(row));
		if(examTypeCode==0){
			var title="资格预审文件购买情况"
		}else{
			var title="询比采购文件购买情况"
		}
		//保存对象目标文件对象		
		top.layer.open({
			type: 2,
			...
			resize: false, //是否允许拉伸
			closeBtn: 1,
			content: 'Auction/common/Purchase/BidNotice/modal/BidBuySituationInfo.html?examType='+examTypeCode+'&projectId='+row.projectId+'&id='+row.packageId,
...
function initTable(){
	...
	if(shopType!=2){
		$('#BidSituationList').bootstrapTable("hideColumn", 'offerState');
	}
}
