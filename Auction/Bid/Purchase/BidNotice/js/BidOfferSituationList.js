...
window.operateEvents = {	
	"click #checkDetail": function(e, value, row, index) {
		...
		top.layer.open({
			...
			resize: false, //是否允许拉伸
			closeBtn: 1,
			content: 'Auction/common/Purchase/BidNotice/modal/BidOfferSituationInfo.html?examType='+examTypeCode+'&projectId='+row.projectId+'&id='+row.packageId,
			...
			},
			{
				...
				formatter:function(value, row, index){
					if(row.packageSource==1){
						return value+'<span class="text-danger" style="font-weight:bold">(重新采购)</span>';
					}else{
						return value;
					}
