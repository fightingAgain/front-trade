...
var pushHtml = 'Auction/common/Purchase/BidNotice/modal/pushInfo.html'; //数据推送
...
function checkDetail(packageId, type, examType, purExamType, $index) {
	...
	if (type == "RELEASE") {
		...
		title = "发布询比采购结果通知";
		checkChange(rows.projectId, packageId, examType);
		if (checkBusType == 1) {
			return
		}
	} else {
	...
	top.layer.open({
		...
		resize: false,
		closeBtn: 1,
		content: 'Auction/common/Purchase/BidNotice/modal/BidResultInfo.html?special=' + type + '&examType=' + examType + '&projectId=' + row[$index].projectId + '&id=' + row[$index].packageId + '&keyId=' + row[$index].id,
	});
}
//发布结果通知书验证业务报表信息
