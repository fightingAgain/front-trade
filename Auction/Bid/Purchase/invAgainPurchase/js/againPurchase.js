		...
		columns: [{
			...
			formatter: function(value, row, index) {
				...
				var audit = '<button type="button" class="btn-xs btn btn-primary" onclick=audit_btn(\"' + index + '\")>' +
					...
					'</button>'
				// 删除
				var delet = '<button class="btn-xs btn btn-danger" onclick=deletPackage(\"' + row.id + '\")><span class="glyphicon glyphicon-trash" aria-hidden="true"></span>删除</button>'
...
function add_bao() {
	parent.layer.open({
		...
		maxmin: true //开启最大化最小化按钮
			,
		content: 'Auction/common/Purchase/invAgainPurchase/model/add_project.html?tenderType=0&examType=1',
