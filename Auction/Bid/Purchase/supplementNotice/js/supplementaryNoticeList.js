				...
				if(row.createType != undefined && row.createType == 1) {
					...
					if(row.isAgent == 1) {
						...
						if(row.supplementState == 1) {
							...
							Tdr += '<button class="btn-xs btn btn-danger" onclick=delFile(\"' + index + '\")><span class="glyphicon glyphicon-trash" aria-hidden="true"></span>删除</abutton>';
						}
					}
				}
...
function add_bao() {
	parent.layer.open({
		...
		maxmin: true //开启最大化最小化按钮
			,
		content: 'Auction/common/Purchase/supplementNotice/model/supplementaryNoticeInfo.html',
...
function fileView($index) {
	...
	parent.layer.open({
		...
		maxmin: true //开启最大化最小化按钮
			,
		content: 'Auction/common/Purchase/supplementNotice/model/viewSupplementaryNotice.html?projectId='+rowData[$index].projectId+'&packageId='+rowData[$index].packageId+'&examType='+rowData[$index].examType+'&keyId='+rowData[$index].id,
		success: function(layero, index) {
...
function editFile($index) {
	...
	parent.layer.open({
		...
		maxmin: true //开启最大化最小化按钮
			,
		content: 'Auction/common/Purchase/supplementNotice/model/editSupplementaryNoticeInfo.html?projectId='+rowData[$index].projectId+'&packageId='+rowData[$index].packageId+'&examType='+rowData[$index].examType+'&keyId='+rowData[$index].id,
		success: function(layero, index) {
			var iframeWind = layero.find('iframe')[0].contentWindow;			
		}
	});
