		...
		success: function(data) {
			if(!data.data) {				
				top.layer.open({
					...
					resize: false, //是否允许拉伸
					title: title,
					content: 'Auction/common/Purchase/ReportCheck/Judge.html' + "?id=" + id + "&expertid=" + expertid + "&expertsNumber=" + $("#expertsNumber").val() + "&expertslength=" + ($('#ExpertsTable').bootstrapTable('getData').length)+"&checkOrExtract="+checkOrExtract+'&isAgents='+isAgents+'&enterpriseId='+enterpriseId+'&titleType='+titleType,
		...
		success: function(data) {
			if(!data.data) {
				parent.layer.open({
					...
					resize: false, //是否允许拉伸
					title: "添加评委",
					content: 'Auction/common/Purchase/ReportCheck/AddJudge.html?projectId='+ProjectData.projectId,
					btn: ['保存', '取消'],
...
$("#ExtractRecord").click(function() {
	parent.layer.open({
		...
		resize: false, //是否允许拉伸
		title: "抽取历史",
		content: 'Auction/common/Purchase/ReportCheck/ExtractRecord.html',		
	})
});
		...
		success: function(data) {
			if(data.success) {
				top.layer.open({
					...
					resize: false,
					title: "项目包件详情",
					content: 'Auction/common/Purchase/ReportCheck/PackageInfo.html?id=' + id + '&checkPlan=' + checkPlan,
