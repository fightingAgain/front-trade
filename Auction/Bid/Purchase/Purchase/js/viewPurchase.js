 ...
 var viewpackage='Auction/common/Purchase/Purchase/model/viewPackage.html';//包件详情页面
//该条数据的项目id
...
$(function(){
	...
	$('#projectMember').AddMembers({
		...
		status:((projectData.isAgent==1 || createType == 1)?'3':'2'),//1编辑   2 查看  3 采购人专区的代理项目
	})
})
//获取询比公告发布的数据
function Purchase(){	
...
function PurchaseData(){
	...
	var isSubpackage = projectData.isSubpackage;
	if (isSubpackage == 1) {
		$('#isSubpackage').html('是');
	} else if (isSubpackage == 0) {
		$('#isSubpackage').html('否');
	} else {
		$('#isSubpackage').html('');
	}
...
function viewbao($index){
	...
	parent.layer.open({
        ...
        ,success:function(layero,index){    
        	...
        	iframeWind.Is_Public(iframeWind.packageInfo.isPublic)
        }
