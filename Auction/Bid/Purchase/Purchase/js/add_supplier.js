...
var checkedData = [];//选中的企业信息
var packageId,projectId;
var supplierListUrl="";
var isPublics="";
var purchaserIds;
var classificaCodes;
function du(isPublic,packageIds,projectIds,purchaserId,classificaCode){	
	...
	if(isPublic==3){
		...
		supplierListUrl=top.config.bidhost+'/EnterpriseController/page.do'
	}
	data1 = JSON.parse(sessionStorage.getItem('keysjd'));
	if(data1!=null){
		for(var i=0;i<data1.length;i++){
			checkedData.push(data1[i]);
			checkboxed.push(data1[i].supplierId);
		}
	}
    initTable();
   	// 搜索按钮触发事件
...
function queryParams(params) {
	...
	rowDate.pageNumber=params.offset/params.limit+1;//当前页数
	rowDate.pageSize=params.limit; // 每页显示数量
	rowDate.offset=params.offset;// SQL语句偏移量	
	if(isPublics==3){
...
$("#evenTrue").on('click',function(){
    ...
    /* for(var i=0;i<checkboxed.length;i++){
    	para["projectSuppliers["+i+"].supplierId"]= checkboxed[i]    	
    } */
	para.projectSuppliers = checkedData;
    $.ajax({
    	type:"post",
    	url:top.config.bidhost+"/ProjectSupplierController/saveProjectSupplier.do",
    	async:true,
    	data:para,
...
// 　　　　　　checkboxed.indexOf(v.id) == -1 ? checkboxed.push(v.id) : -1;	   
			if(checkboxed.indexOf(v.id) == -1){
				checkboxed.push(v.id);
				var obj = {
					'supplierId': v.id,
...
　　　　	});
		...
		$.each(datas,function(i,v){		        	
			...
			checkedData.splice(checkboxed.indexOf(v.id),1);    //删除取消选中行
		});
	};		
};
