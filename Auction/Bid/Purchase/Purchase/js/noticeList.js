...
var addpackage = 'Auction/common/Purchase/Purchase/model/noticeInfo.html'; //后审的编辑页面
var viewpackage = 'Auction/common/Purchase/Purchase/model/noticeView.html'
var iframeWinAdd = "";
...
$(function() {
	if(PAGEURL.split("?")[1] != undefined && PAGEURL.split("?")[1] != "") {
		if(PAGEURL.split("?")[1].split("=")[0] == "enter") {
			if(PAGEURL.split("?")[1].split("=")[1] == 0) {
				isAgentCode = 0;
				tenderTypeCode = 0;
			} else if(PAGEURL.split("?")[1].split("=")[1] == 1) {
				isAgentCode = 1;
				tenderTypeCode = 0;
			}
		}
...
function add_bao() {
	parent.layer.open({
		...
		title: '公告发布',
		area: ['1100px', '600px'],
		content: addpackage + '?tenderType=' + 0
			//确定按钮
			,
		success: function(layero, index) {
			iframeWinAdd = layero.find('iframe')[0].contentWindow;
			iframeWinAdd.datetimepicker(1);
			// iframeWinAdd.mediasData();
...
function edit_bao($index) {
	var rowData = $('#table').bootstrapTable('getData'); //bootstrap获取当前页的数据	
	if(rowData[$index].packageSource == 1){
		addpackage="Auction/common/Purchase/againPurchase/model/noticeInfo.html"
	}	
	parent.layer.open({
		type: 2 //此处以iframe举例
			,
