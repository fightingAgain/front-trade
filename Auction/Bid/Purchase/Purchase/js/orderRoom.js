...
function orderRoom(){
	...
	var data={
		...
		'linkTel':top.userTel,
		'examType':parseInt(packageInfo.examType) +1,
		'supplierEnterpriseName':packageInfo.purchaserName,
		'supplierEnterpriseId':packageInfo.purchaserId,
		'biddingRoomPackages':[
			{
				'id':packageInfo.id,
...
function callbackappointment(data){
	...
	data.examType=packageInfo.examType;
	$.ajax({
		type: "post",
...
function callbackData(data){
	...
	if(packageInfo.packageState==1||packageInfo.packageState==2){
		if(data){
			if(packageInfo.examType==0){//预审
				// 当资格预审时间大于当前时间时，显示预约按钮
				if(packageInfo.examCheckEndDate&&(NewDate(packageInfo.examCheckEndDate)> NewDate(nowDate))){
					$('.colseOrder').show();
					$('.showOrder').hide();
				};
			}else{
