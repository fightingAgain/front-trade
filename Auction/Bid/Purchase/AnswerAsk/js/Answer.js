...
window.operateEvents = { //添加一个按钮对应的事件
	...
	"click #editAnswer": function(e, value, row, index) {
		...
		parent.layer.open({
			...
			resize: false, //是否允许拉伸
			area: ['800px', '600px'],
			content: 'Auction/common/Purchase/AnswerAsk/CommitAnswer.html' + '?examType=' + examType + '&inviteState=' + inviteState + "&tenderTypeCode=" + tenderType + '&examTypeCode=' + examTypeCode,
			// btn: ['提交', '取消']
			// 	//确定按钮
			// 	,
	...
	"click #TableEditor": function(e, value, row, index) {
		...
		parent.layer.open({
			...
			// maxmin: false, //该参数值对type:1和type:2有效。默认不显示最大小化按钮。需要显示配置maxmin: true即可
			resize: false, //是否允许拉伸
			content: 'Auction/common/Purchase/AnswerAsk/AskContent.html',
			success: function(layero, index) {
				var body = parent.layer.getChildFrame('body', index);
				var iframeWin = layero.find('iframe')[0].contentWindow;
				iframeWin.show(row);
