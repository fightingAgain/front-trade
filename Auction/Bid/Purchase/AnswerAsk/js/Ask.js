...
if(examType == 0 && inviteState == 0){
		examTypeCode =0;
	}
if((examType == 0 && inviteState == 1) || examType == 1){
		examTypeCode =1;
}*/

$(function(){
   /*tenderTypeCode=sessionStorage.getItem('tenderTypeCode')*/
//    initTable();
});
...
window.operateEvents = {//添加一个按钮对应的事件
	...
	var tenderType=sessionStorage.getItem('tenderTypeCode')//项目类型的缓存
		sessionStorage.setItem('tenderTypes', tenderType);//缓存项目类型
	sessionStorage.setItem('examTypes',examType);	
	sessionStorage.setItem('inviteStates', inviteState);*/	
	parent.layer.open({		
		type: 2,
		title: '查看详情',
		area: ["60%", "70%"],
		...
		resize: false, //是否允许拉伸
		content: '0502/Bid/AnswerAsk/AskContent.html'+'?examType=' + examType +'&inviteState=' + inviteState+ "&tenderType=" + tenderTypeCode,
