...
function BindExperts(data) {
	...
	if(isAgents==1&&top.enterpriseTypeOlny==0){
    	$('#ExpertsTable').bootstrapTable("hideColumn", 'expertRemark'); //隐藏评审内容 
    }
		...
		success: function(data) {
			if(!data.data) {
				...
				top.layer.open({
					...
					resize: false, //是否允许拉伸
					title: title,
					content: 'Auction/common/Purchase/ReportCheck/Judge.html' + "?id=" + id + "&expertid=" + expertid + "&expertsNumber=" + $("#expertsNumber").val() + "&expertslength=" + ($('#ExpertsTable').bootstrapTable('getData').length) + "&checkOrExtract=" + checkOrExtract + '&isAgents=' + isAgents + '&checkType=' + 1+'&enterpriseId='+enterpriseId,
		...
		success: function(data) {
			if(!data.data) {
				parent.layer.open({
					...
					resize: false, //是否允许拉伸
					title: "添加业主评委",
					content: 'Auction/common/Purchase/ReportCheck/AddJudge.html?projectId=' + ProjectData.projectId + '&checkType=' + 1,
					btn: ['保存', '取消'],
...
$("#ExtractRecord").click(function() {
	parent.layer.open({
		...
		resize: false, //是否允许拉伸
		title: "抽取历史",
		content: 'Auction/common/Purchase/ReportCheck/ExtractRecord.html?checkType=1',
