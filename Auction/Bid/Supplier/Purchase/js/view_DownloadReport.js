var UrlsaveFileDownload = config.bidhost + "/BidFileDownloadController/saveBidFileDownload.do"
var dowoloadFileUrl = config.bidhost + '/FileController/download.do';
var urlfindBidFileDownload = config.bidhost + "/BidFileDownloadController/findBidFileDownload.do" //查询文件下载记录
/*
 * 保存  提交按钮
 * */
	...
	$.ajax({
		...
		success: function (data) {
			if (data.success) {
				...
				findCode();
				var thisFrame = parent.window.document.getElementById("downfiles").getElementsByTagName("iframe")[0].id;
				var dcmt = parent.$('#' + thisFrame)[0].contentWindow;
				dcmt.$('#fileLoadList').bootstrapTable('refresh', { url: urlfindBidFileDownload });
			} else {
...
function findCode() {
	$.ajax({
		type: "post",
		url: config.bidhost + "/ProjectReviewController/findCode.do",
		data: {
			"packageId": rowData.packageId,
			...
			if (response.success) {
				if (response.data == "200") {
					...
					parent.layer.confirm('本项目的保证金帐号已生成，请到“保证金管理(虚拟子帐号)-查看项目保证金”中查看！', {
						...
						var viewUrl = "Bond/tenderCashList/model/tenderCashView.html?tenderType=0"; // 查看
						top.layer.open({
							type: 2,
							title: '查看项目保证金',
							area: ['100%', '100%'],
