...
$(function() {
	...
	$("#btnSearch").on('click', function() {
		$('#RaterAskList').bootstrapTable('destroy');
		initTable()
	});
...
window.operateEvents = {
	"click #checkAsk": function(e, value, row, index) {
		...
		top.layer.open({
			...
			resize: false, //是否允许拉伸
			closeBtn: 1,
			content: 'Auction/common/Supplier/RaterAsk/modal/RetarAskListInfo.html?type=check',
		});
	},
	"click #replyAsk": function(e, value, row, index) {
		...
		if(flags == 0){
			top.layer.open({
				...
				resize: false, //是否允许拉伸
				closeBtn: 1,
				content: 'Auction/common/Supplier/RaterAsk/modal/RetarAskListInfo.html?type=reply',
			});
		}else{
