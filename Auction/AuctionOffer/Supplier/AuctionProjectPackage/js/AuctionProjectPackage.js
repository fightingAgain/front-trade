var applicationModule=top.PAGEURL.getQueryString('applicationModule');
//初始化
$(function() {
	initTable();
	//开始时间
...
function initTable() {
	$('#projectPackage').bootstrapTable({
		url: config.offerhost + '/info/findSupplierBidPackages',
		cache: false,
		pagination: true,
					...
					events: {
						'click .btn-opera': function(e, value, row, index) {
							...
							if(NewDatefp(row.biddingStartTime)>NewDatefp(nowSysDate)){
								...
								if(~~applicationModule > 0){
									openIframe(row.packageId,row.biddingTimes);
								}else{
									openOrder(row.packageId,row.biddingTimes)
								}
							}													
						}
					},
...
function openOrder(packageId,biddingTimes) {
	parent.layer.open({
		type: 2,
		title: "竞价报价",
		area: ['100%', '100%'],
		content: 'Auction/AuctionOffer/Supplier/AuctionProjectPackage/model/AuctionOfferInfo.html?packageId='+ packageId+'&biddingTimes='+biddingTimes
	});
}
