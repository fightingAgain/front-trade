...
window.operateEvents = { //添加一个按钮对应的事件
	...
	"click #btnShow": function(e, value, row, index) {
		layer.open({
			...
			resize: false, //是否允许拉伸
			/*content: 'Auction/Sale/Purchase/AnswerAsk/Answer.html',*/
			content: 'Auction/Sale/Purchase/AnswerAsk/Answer.html?projectId='+row.projectId+"&createType="+row.createType,
...
function AddFunction(value, row, index) { //把需要创建的按钮封装到函数中
	return [
		'<button type="button" id="btnShow" class="btn-xs btn btn-primary" style="text-decoration:none" ><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span>查看</button>',
	].join("")
}
function initTable(){
	//加载数据
				...
				},
				{
					...
					formatter:function(value, row, index){
						if(row.project.projectSource==1){
								...
								var projectName='<div style="text-overflow: ellipsis;white-space:nowrap;overflow:hidden;">'+row.project.projectName +'<span class="text-danger"  style="font-weight:bold">(重新竞卖)</span></div>';		
