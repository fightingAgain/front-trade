...
function editview(index) {
	...
	sessionStorage.setItem("AuctionWasteCheckDate", JSON.stringify(rowData));
	layer.open({
		type: 2,
		title: '竞卖时间编辑',
		area: ['1100px', '600px'],
		resize: false,
		content: 'Auction/Sale/Purchase/SaleTime/model/SaleTimeEdit.html?id='+rowData[index].id+'&packageId='+rowData[index].packageId+'&projectId='+rowData[index].projectId,
	});
}
//查看
function viewbao(data,$index) {
	...
	layer.open({
		...
		area: ['1100px', '600px'],
		resize: false,
		content: 'Auction/Sale/Purchase/SaleTime/model/SaleTimeList.html?id='+rowData[$index].id,
	});
}