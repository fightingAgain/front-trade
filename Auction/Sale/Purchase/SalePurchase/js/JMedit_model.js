	...
	if(isCheck) {
		top.layer.confirm('温馨提示：该流程未设置审批节点，您是否继续提交？', {
			...
			$.ajax({
				...
				data: $("#form").serialize() + projectPrices + '&fjwzState=' + 'fjwz_wl',
...
//			$('input[name="project.auctionProjectPackages[0].taxIncluded"][value="0"]').attr("checked", true);
//			$(".tsxstyle").show();
//		}else if(purchaseaData.autionProjectPackage[0].taxIncluded ==1){
//			$(".tsxstyle").hide();
//		}
		//当单轮竞卖是竞卖时常为
...
//是否缴纳保证金
$('input[name="project.auctionProjectPackages[0].taxIncluded"]').on('click', function() {
	if($(this).val() == "0") {
		$(".tsxstyle").show();
		$(".constyle td:nth-child(2)").attr('colSpan','1');
//		$("#depositPrice").attr('disabled', true);
//		$("#depositPrice").val("");
	} else {
		$(".tsxstyle").hide();
		$(".constyle td:nth-child(2)").attr('colSpan','3');
	};
});	
//是否缴纳保证金
...
function acutionData() {
	...
	if($('input[name="project.auctionProjectPackages[0].auctionType"]:checked').val() == 1) {
...
function text() {
	...
	if($('input[name="project.auctionProjectPackages[0].taxIncluded"]:checked').val() == 0) {
		if($("#taxPoint").val()==""){
			parent.layer.alert("请输入税点");
			return;
		}
	}
...
function initDataTab(tabeldata) {
	$("#materialList").bootstrapTable({
		...
		columns: [{
			...
			align: 'left',
			formatter:function(value,row,index){
				return '<select class="form-control"><option value="0">个</option><option value="1">千克</option></select>'
			}
		}, {
		...
		// 	width: '200px',
		// 	formatter: function(value, row, index) {
		// 		var str = '<button  type="button" class="btn btn-secondary btn-sm btnView" onclick=viewbao(\"' + index + '\") data-index="' + index + '"><span class="glyphicon glyphicon-eye-open"></span>保存</button>';
		// 		return str;
...
function viewbao($index) {
	var rowData = $('#materialList').bootstrapTable('getData');
	var id = rowData[$index].id;
	//var budget=rowData[$index].budget;
	$.ajax({
		url: config.AuctionHost + '/AuctionPurchaseController/saveMaterialDetails.do',
		type: 'post',
		async: false,
