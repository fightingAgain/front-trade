...
var urlHistoryInfo="Auction/Sale/Purchase/SaleProjectPackage/model/detailItemSupplier.html"
...
//	setPurchaseFileDownloadDetail(packageId);
	...
	detailItem();
	if(isAgent == 1){
		warnLists(packageId, 'jm', true,null,null,projectId,'2','1');
	}else{
		warnLists(packageId, 'jm', false,null,null,projectId,'2','1');
	}
	if(createType == 1){
		$(".modify").hide();
...
function getMsg(obj){
	if(type == 'commit' && offerData.isStopCheck != 1) {
		//展示提交按钮
		$("#commitBtn").show();
	}
