...
window.operateEvents = { //添加一个按钮对应的事件
	"click #btnCheck": function(e, value, row, index) {
		...
		layer.open({
			...
			resize: false, //是否允许拉伸
			content: 'Auction/Sale/Purchase/SaleSupplierNegotiator/pPriceNegotiator.html?'+"createType="+createType+'&projectId='+row.projectId+'&id='+row.packageId,
	...
	"click #btnNot": function(e, value, row, index) {
		...
		layer.open({
			...
			maxmin: true, //该参数值对type:1和type:2有效。默认不显示最大小化按钮。需要显示配置maxmin: true即可
			resize: false, //是否允许拉伸
			content: 'Auction/Sale/Purchase/SaleSupplierNegotiator/NotNegotiator.html',
			success: function(layero, index1) {
				var body = layer.getChildFrame('body', index1);
				var iframeWin = layero.find('iframe')[0].contentWindow;
				iframeWin.tableShow(row);
	...
	"click #btnUnderCheck": function(e, value, row, index) {
		...
		layer.open({
			...
			maxmin: true, //该参数值对type:1和type:2有效。默认不显示最大小化按钮。需要显示配置maxmin: true即可
			resize: false, //是否允许拉伸
			content: 'Auction/Sale/Purchase/SaleSupplierNegotiator/underline.html?projectId='+row.projectId+'&id='+row.packageId,
			success: function(layero, index1) {
				var body = layer.getChildFrame('body', index1);
				var iframeWin = layero.find('iframe')[0].contentWindow;
	...
	"click #btnNegotiator": function(e, value, row, index) {
		...
		layer.open({
			...
			resize: false, //是否允许拉伸
			content: 'Auction/Sale/Purchase/SaleSupplierNegotiator/NegotiatorMethod.html?projectId='+row.projectId+'&id='+row.packageId,
