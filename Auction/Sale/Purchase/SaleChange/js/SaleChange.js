...
/// 表格初始化
var purchaseaddurl = 'Auction/Sale/Purchase/SaleChange/model/addSaleProject.html' //添加路径
var auditPurchase = 'Auction/Sale/Purchase/SaleChange/model/auditPurchase.html' //审核路径
var editPurchase = 'Auction/Sale/Purchase/SaleChange/model/editSaleProject.html' //编辑路径
		...
		columns: [{
			...
			formatter: function(value, row, index) {
				if(row.projectSource == 1) {
					...
					if(count){
						var projectName = ' <div style="text-overflow: ellipsis;white-space:nowrap;overflow:hidden;">' + row.projectName + '<span class="text-danger">(第'+count+'次 重新竞卖)</span></div>';
					}else{
						var projectName = ' <div style="text-overflow: ellipsis;white-space:nowrap;overflow:hidden;">' + row.projectName + '<span class="text-danger">(重新竞卖)</span></div>';
					}
