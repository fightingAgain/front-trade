...
function choseHistoryProject() {
 	parent.layer.open({
 		...
 		title: '历史变更记录',
 		area: ['1100px', '600px'],
 		content: 'Auction/Sale/Purchase/SaleChange/model/historyPurchasers.html?tenderType=2&projectId=' + globalInfo.projectId
 			//,btn: ['关闭']
 			,
...
function showPurchse(Data){
	...
	if(projectSourceCount != "undefined" && projectSourceCount != null && projectSourceCount != "0") {
		$("#projectName").html(purchaseaData.project[0].projectName);
		$("#packageSourceCount").html('(第' + projectSourceCount + '次重新竞卖)');
