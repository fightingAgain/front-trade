...
function nextNotice(projectId){
	parent.layer.open({
	    ...
	    ,title: '编辑公告'
	    ,area: ['1100px','650px']
		,content:'Auction/Sale/Agent/SalePurchase/model/edit_model.html?projectId='+projectId
		,id:'packageclass'
...
function chosePucharse(){	
	...
	parent.layer.open({
	    ...
	    ,title: '选择采购人'
	    ,area: ['1100px','650px']
	    ,content:'Auction/Sale/Agent/SalePurchase/model/Purchasers.html'
	    //,btn: ['关闭']
	    ,maxmin: true //该参数值对type:1和type:2有效。默认不显示最大小化按钮。需要显示配置maxmin: true即可
		,resize: false //是否允许拉伸
	    ,success:function(layero,index){
...
$("#purchaserLinkmen").click(function(){
	...
	parent.layer.open({
		...
		success:function(layero, index){
			...
			iframeWin.passMessage(function(PurchasersData){
