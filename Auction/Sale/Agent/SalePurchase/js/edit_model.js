...
var editPurchase = 'Auction/Sale/Agent/SalePurchase/model/addSale.html';//上一步修改项目信息
var isCheck;
...
function form() {
	...
	$('#projectBankType').val($('[name=bankType]:checked').val());
	acutionData();
	var arr = {};
	...
	if($('#quotationUnitText').attr('datatype') == '*'){
		arr.quotationUnit = $('#quotationPriceUnit').val() + '/' + $.trim($('#quotationUnitText').val())
	}else{
		arr.quotationUnit = $('#quotationPriceUnit').val()
	}
...
$("#btn_bao").click(function() {
	...
	if($('#quotationUnitText').attr('datatype') == '*' ){
		if($.trim($("#quotationUnitText").val()).length > 10){
			parent.layer.alert("报价单位长度限制10字以内");
			return;
		}
...
function Purchase() {
	...
	if(purchaseaData.project && purchaseaData.project.length>0 &&purchaseaData.project[0].oldProjectId){//推送的项目
		...
		if(purchaseaData.agencyDepartmentName || purchaseaData.project[0].belongDepartmentName){
			$('.agencyDepartment').show().html(purchaseaData.agencyDepartmentName?purchaseaData.agencyDepartmentName:purchaseaData.project[0].belongDepartmentName);
			$('#agencyDepartmentId, #agencyDepartment').hide();
			$("#agencyDepartmentId").val(purchaseaData.agencyDepartmentId?purchaseaData.agencyDepartmentId:purchaseaData.project[0].belongDepartmentId);
			$("#agencyDepartment").val(purchaseaData.agencyDepartmentName?purchaseaData.agencyDepartmentName:purchaseaData.project[0].belongDepartmentName);
		}else{
			$('#agencyDepartmentId, #agencyDepartment').show();
			$('.agencyDepartment').hide();
		}
	...
	if(purchaseaData.autionProjectPackage.length > 0) {
		...
		$('[name=quotationMethod]').val([purchaseaData.autionProjectPackage[0].quotationMethod]);
		$('[name=quotationType]').val([purchaseaData.autionProjectPackage[0].quotationType == undefined?'0':purchaseaData.autionProjectPackage[0].quotationType]);
		if(purchaseaData.autionProjectPackage[0].quotationMethod == 1){
			if(purchaseaData.autionProjectPackage[0].quotationUnit){
				$('#quotationPriceUnit').val(purchaseaData.autionProjectPackage[0].quotationUnit.split('/')[0])
				$('#quotationUnitText').val(purchaseaData.autionProjectPackage[0].quotationUnit.split('/')[1])
			}
			
		};
		if(purchaseaData.autionProjectPackage[0].quotationType != undefined && purchaseaData.autionProjectPackage[0].quotationMethod != undefined){
			showQuotationPriceUnit(purchaseaData.autionProjectPackage[0].quotationType, purchaseaData.autionProjectPackage[0].quotationMethod);
		}
	...
	//start非招标代理服务费
	if(purchaseaData.openServiceFee == 1) {
		var projectServiceFee = purchaseaData.autionProjectPackage[0].projectServiceFee;
		$(".agentBlock").show();
		for(var key in projectServiceFee) {
			$("[name='projectServiceFee." + key + "']").val(projectServiceFee[key]);
		}
		$("[name='projectServiceFee.tenderType']").val(purchaseaData.project[0].projectType);
		$("[name='projectServiceFee.projectId']").val(purchaseaData.projectId);
		//		$("[name='projectServiceFee.packageId']").val(purchaseaData.id);
		agentFeeChange(projectServiceFee && projectServiceFee.collectType || 0);
	} else {
		$(".agentBlock").remove();
	}
	//end非招标代理服务费
	//虚拟子账户生成银行回显
	if(purchaseaData.autionProjectPackage[0].bankType){
		$('input[name="bankType"]').val([purchaseaData.autionProjectPackage[0].bankType]);
		// $('input[name="bankType"][value="'+ purchaseaData.autionProjectPackage[0].bankType +'"]').attr("checked",true);
	}
};
// //切换模板
...
var FileInput = function() {
	...
	oFile.Init = function(ctrlName, uploadUrl) {
		$("#FileName").fileinput({
			...
			if(event.currentTarget.files[0].size > 4 * 1024 * 1024 * 1024) {
				parent.layer.alert('上传的文件不能大于4G');
				$(this).fileinput("reset"); //选择的格式错误 插件重置
				return;
			};
...
function dataTypes() {
	...
	top.layer.open({
		...
		yes: function(index, layero) {
			...
			$("[name='projectServiceFee.tenderType']").val($("#engineering").val()); //代理服务费
			parent.layer.close(index)
		}
...
function text() {
if($('input[name="project.auctionProjectPackages[0].isPayDeposit"]:checked').val()==0&&filesData.length===0) {
		parent.layer.alert("竞卖采购文件不能为空");
		return;
	};
	...
	if(!$('[name=quotationMethod]:checked').val()){
		parent.layer.alert("请选择报价方式");
		return;
	}
	if($('#quotationUnitText').attr('datatype') == '*'){
		if($.trim($("#quotationUnitText").val()) == ''){
			parent.layer.alert("报价单位不能为空");
			return;
		}else if($.trim($("#quotationUnitText").val()).length > 10){
			parent.layer.alert("报价单位长度限制10字以内");
			return;
		}
	...
	if($("#rawPrice").val() == "") {
		...
		// if($('[name=quotationType]:checked').val() == 1){
		// 	regu = /^[1-9][0-9]{0,15}?$/;
		// }
		if(!regu.test($("#rawPrice").val())){
			parent.layer.alert("请正确输入竞卖起始价");
			return;
		}
	};
	if($("#priceReduction").val() == "") {
		...
		// 	regu = /^[1-9][0-9]{0,15}?$/;
		// }
		if(!regu.test($("#priceReduction").val())){
			parent.layer.alert("请正确输入涨价幅度");
			return;
		}
	};
	if($("#dataTypeName").val() == "") {
	...
	//start 采购代理服务费
	if(!checkAgentFee(purchaseaData.openServiceFee)) {
		return;
	}
	//end 采购代理服务费
	if(isDf) {
		if(depositList.subitemData()) {
			top.layer.confirm((publicData.length > 0 ? '邀请' : '') + '{' + depositList.subitemData() + '},保证金没有转移到本项目，确认不转移吗？', function(index) {
				form();
				parent.layer.close(index);
			})
		} else {
...
function getDeposit() {
	if(!depositList) {
		depositList = $("#depositHtml").deposit({
			...
			isPayDeposit: purchaseaData.autionProjectPackage[0].isPayDeposit,
			packageData: [{
				packageId: purchaseaData.autionProjectPackage[0].id,
				projectId: projectIds,
				projectForm: 2
			}]
		})
...
/* ********************************************       竞卖项目增加报价单位              *************************** */
$('[name=quotationType], [name=quotationMethod]').change(function(){
	showQuotationPriceUnit($('[name=quotationType]:checked').val(), $('[name=quotationMethod]:checked').val());
})
/* 报价单位 
1、报价类型选“价格”、报价方式选“报总价”时，默认为“元”，不可编辑；

2、报价类型选“价格”、报价方式“报单价”时，价格单位显示“元”（不可修改），同时显示文本框，需填写商品计量单位，如：吨，文本长度限制10字以内且不得为空，不允许输入空格，最终“报价单位”字段值为两部分合并结果，如：“元/吨”。

3、报价类型选“费率”、报价方式“报总价”时，报价单位为“%”，不可修改；

4、报价类型选“费率”、报价方式“报单价”时，价格单位显示为“%”，不可修改，显示文本框，需填写商品计量单位，如：吨，文本长度限制10字以内且不得为空，不允许输入空格，最终“报价单位”字段值为两部分合并结果，如：“%/吨”
val1: 报价类型(0:价格；1:费率)
val2: 报价方式(0:报总价；1:报单价)
*/

function showQuotationPriceUnit(val1, val2){
	if(val1 == '0' && val2 == '0'){
		$('#quotationUnitText').hide().removeAttr('datatype');
		$('.unitContect').hide();
	}
	if(val1 == '0' && val2 == '1'){
		$('#quotationUnitText').show().attr('datatype','*');
		$('.unitContect').show();
	}
	if(val1 == '1' && val2 == '0'){
		$('#quotationUnitText').hide().removeAttr('datatype');
	...
	if(val1 == '0'){
		$('#quotationPriceUnit').val('元');
		$('.quotationPriceUnit').html('元');
		$('#priceReduction').attr('oninput','verifyMoney(this, 2, "请正确输入涨价幅度")');
		$('#rawPrice').attr('oninput','verifyMoney(this, 2, "请正确输入竞卖起始价"))');
	}else if(val1 == '1'){
		$('#quotationPriceUnit').val('%');
		$('.quotationPriceUnit').html('%');
		//2025-11-20 按项目经理要求，费率报价的涨价幅度与起始价能输入小数
		$('#priceReduction').attr('oninput','verifyMoney(this, 2, "请正确输入涨价幅度")');
		$('#rawPrice').attr('oninput','verifyMoney(this, 2, "请正确输入竞卖起始价")');
	}
}