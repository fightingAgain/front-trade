...
function checkDetail(packageId, type, index) {
	...
	if(rows.groupCode=='00270012'){
		var InfoHtml = 'Auction/Sale/Agent/SaleNotice/modal/SaleWasteNoticeInfo.html?';
	}else{
		var InfoHtml = 'Auction/Sale/Agent/SaleNotice/modal/SaleNoticeInfo.html?';
	}
	top.layer.open({
		type: 2,
		title: title,
				...
				formatter: function(value, row, index) {
					if(row.createType != undefined && row.createType == 1) {
						...
						if(typeof(row.itemState) == 'undefined' || row.itemState == 3 || row.itemState == 0) {
							return "<button onclick='checkDetail(\"" + row.packageId + "\",\"RELEASE\",\"" + index + "\")' class='btn btn-primary btn-xs'><span class='glyphicon glyphicon-pencil'>发布公示</button>"
						} else {
							if(row.itemState == 1) { //审核中
								return "<button onclick='checkDetail(\"" + row.packageId + "\",\"view\",\"" + index + "\")' class='btn btn-primary btn-xs'><span class='glyphicon glyphicon-eye-open'>查看详情</button>&nbsp;&nbsp" +
