			...
			// },
			{
				...
				formatter: function(value, row, index) {
					if(row.createType != undefined && row.createType == 1){
						...
						if(row.checkState == "0") {
							return '<button type="button" onclick="AuctionFileCheck(\'' + row.projectId + '\',\'audit\',\'' + row.createType + '\')" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-pencil"></span>设置</button>'
						} else {
							return '<button type="button" onclick="AuctionFileCheck(\'' + row.projectId + '\',\'check\',\'' + row.createType + '\')" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-eye-open"></span>查看</button>'
						}
					}
...
function AuctionFileCheck(data, type,createType) {
	...
	top.layer.open({
		...
		closeBtn: 1,
		id:'SetSupplier',
		content: 'Auction/Sale/Agent/SaleFile/modal/SaleSetSupplierInfo.html?createType='+createType+'&projectId='+data,
	});
}