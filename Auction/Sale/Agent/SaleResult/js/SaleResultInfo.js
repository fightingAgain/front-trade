...
$(function() {
	...
	if(type=='RELEASE' || type == 'CHANGE'){
		checkState = "预览";
		$('#btn_save').show();
        resultNoticeCheckSatisfaction('2', projectId, packageId);//验证满意度评价
	}
			...
			/* 代理询比采购项目，采购结果通知发布结果页面，修改公告信息内部编辑方式。  media/js/Model/public.js */
				...
				if(auctionModel == 0) { //按照包件
					...
					for(var i = 0; i < BidResultItem.length; i++) {
						...
						html += "<td align='left' class='enterpriseName'>" + showBidderRenameList(BidResultItem[i].supplierId, BidResultItem[i].enterpriseName, RenameData, 'body') + "</td>";
						html += "<input type='hidden' name='bidResultItems[" + i + "].supplierId' value=" + BidResultItem[i].supplierId + ">";
						if(quotationType == 1){
							html += "<td style='width:150px;'><input type='hidden' name='bidResultItems[" + i + "].lastOffer' value=" + BidResultItem[i].offerMoney + ">"+(BidResultItem[i].offerMoney?BidResultItem[i].offerMoney:"")+"</td>"
						}
						...
						if(quotationType == 1){
							html+= "<td  style='width:100px;text-align:center'><input oninput='verifyMoney(this, 2)' type='text' class='form-control bidPrice' name='bidResultItems[" + i + "].bidPrice'  value='"+(BidResultItem[i].bidPrice?(Number(BidResultItem[i].bidPrice).toFixed(2)):"")+"' /></td>";
						}else{
					...
					//"<td>供应商名称</td><tr>";
					var html = "<tr><td width='50px'>序号</td><td style='text-align:left;'>名称</td><td>规格型号</td><td>数量</td><td>单位</td><td width='50px'>排名</td><td>供应商名称</td>"
					if(quotationType == 1){
						html +="<td>报价金额（"+quotationUnit+"）</td>"
					}
...
$("#btn_submit").on("click", function() {
    resultNoticeCheckSatisfaction('2', projectId, packageId, function(){//验证满意度评价
	var isPublic = $("input[name='isPublic']:checked").val();
	var resultType = $("input[name='resultType']:checked").val();
	var isShow = $("input[name='isShow']:checked").val();
	var isWinner = $("input[name='isWinner']:checked").val();
	...
	if(quotationType == 1){
		...
		$("#bidResultInfo .isBid").each(function () {
			if($(this).find('option:selected').val() == 0){
				if($.trim($(this).closest('tr').find('.bidPrice').val()) == ''){
					top.layer.alert('请输入供应商【'+$(this).closest('tr').find('.enterpriseName').html()+'】的成交金额');
					isGo = false;
					return false
				}
			}
		...
		success: function(data) {
			if(data.success) {
				...
				if(projectServiceFee && ($("#btn_submit").attr("data-type") == 0)){
					...
					var params ={
						'relationId':findPackageInfoData.projectId,
...
function editBidResult($index){
	...
	top.layer.open({
		...
		closeBtn: 1,
		// btn:["保存","取消"],
		content: 'Auction/Sale/Agent/SaleResult/model/editViewResult.html',
		success: function(layero, index) {
			var body = layer.getChildFrame('body', index);
			var iframeWin = layero.find('iframe')[0].contentWindow;
