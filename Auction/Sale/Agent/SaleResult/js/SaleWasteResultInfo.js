			...
			if (auctionType == '6') {
				...
				if (type == "RELEASE") { //当发布时				
					...
					if (auctionModel == 0) { //按照包件
						...
						//"<td>供应商名称</td><tr>";
						var html = "<tr><td style='width:50px;text-align:center'>序号</td><td style='text-align:left;'>名称</td><td>规格型号</td><td>单位</td><td width='50px'>排名</td><td style='vertical-align: middle;text-align:left'>供应商名称</td><td>通知类型</td><td>成交金额(元)</td><td style='width:120px'>操作</td></tr>";
						var index = -1;
...
//							html += "<td rowspan='" + count + "' style='vertical-align: middle;'>" + (item[j].detailedCount || "") + "</td>";
					...
					if (auctionModel == 0) {
						...
						//"<td>供应商名称</td><tr>";
						var html = "<tr><td width='50px'>序号</td><td style='text-align:left;'>名称</td><td>规格型号</td><td>单位</td><td width='50px'>排名</td><td style='text-align: left;'>供应商名称</td><td>通知类型</td><td>成交金额(元)</td><td style='width:120px'>操作</td></tr>";
						for (var j = 0; j < item.length; j++) {
