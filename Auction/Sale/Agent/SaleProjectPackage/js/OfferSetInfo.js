...
var urlDownloadAuctionFile = config.FileHost + "/FileController/download.do"; //下载竞卖文件地址
...
$(function(){
	...
	auctionInfo()
	getSuperviseInfo();
	isShowSupplierInfo(projectId, packageId, '', 'offer', '2', function(data, html){
		if(data.isShowSupplier == 1){
			getOfferFileInfo();//竞卖文件提交信息列表
...
function auctionInfo(){
	$.ajax({
		...
		success: function(res) {
			...
			if (msgData.projectSource > 0) {
				$("#projectName").html(msgData.projectName + '<span class="red">(重新竞卖)</span>');
			}
...
//挂载采购文件下载记录请求
function setPurchaseFileDownloadDetail() {
	$.ajax({
		type: "get",
