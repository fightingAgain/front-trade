...
var createType = getQueryString("createType");
var supplementurl = 'Auction/Sale/Agent/SaleSupplement/model/add_supplement.html' //添加弹出框路径
var findPageList = config.AuctionHost + '/ProjectSupplementController/findProjectSupplementInfo.do' //
var viewsupplement = 'Auction/Sale/Agent/SaleSupplement/model/view_supplement.html'
...
function getMsg(eid, pid, create_type) {
	if(create_type != undefined && create_type == 1) {
		$("#addSupplement").hide();
	}
	/*if(eid != "" && pid != ""){
		if(eid != pid){
...
$("#addSupplement").click(function() {
	...
	parent.layer.open({
		...
		maxmin: true,
		resize: false,
		content: supplementurl + "?projectId=" + projectId + "&isFile=" + isFile,
	...
	$.ajax({
		...
		success: function(data) {
			...
			if(data.success) {
				...
				$("#NoticeEndDate").html(data.auctionStartDate || data.noticeEndDate);
				...
				if(data.projectSupplements.length > 0) {
					...
					for(var i = 0; i < data.projectSupplements.length; i++) {
						...
						tabel_table += '<tr>' +
							'<td>' + (i + 1) + '</td>' +
							'<td style="text-align:left;"><span style="white-space: normal;word-break: break-all;">' + data.projectSupplements[i].title + '</td>' +
							'<td>' + (data.projectSupplements[i].subDate||"") + '</span></td>';
						if(checkState == "审核通过") {
...
function up(id) {
	parent.layer.open({
		...
		maxmin: true,
		resize: false,
		content: supplementurl + "?projectId=" + projectId + "&isFile=" + isFile +  "&isUpdate=1&supplierId="+id,
