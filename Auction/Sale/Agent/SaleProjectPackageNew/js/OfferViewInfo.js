...
var recordOnceData,quotationUnit;
	...
	$.ajax({
		...
		success: function (res) {
			if (res.success) {
				...
				quotationUnit = packageData.quotationUnit;
				...
				if(packageData.auctionType == 0){
					column_left = [{
						...
						field: 'maxPrice',
						title: '最高报价（'+(quotationUnit?quotationUnit:'元')+'）',
						align: 'center',
					},
					];
					...
					{
						field: 'startPrice',
						title: '起始报价（'+(quotationUnit?quotationUnit:'元')+'）',
						align: "center",
						formatter: function (value, row, index) {
...
function jumpToHistory() {
	...
	// sessionStorage.setItem("jmqkquotationUnit", quotationUnit);
	location.href = $.parserUrlForToken("OfferHistoryInfo.html?projectId=" + projectId + "&id=" + packageId + "&tType=1&quotationUnit="+packageData.quotationUnit);
}