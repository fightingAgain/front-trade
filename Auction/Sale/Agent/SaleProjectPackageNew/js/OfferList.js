...
var urlHistoryInfo = 'Auction/Sale/Agent/SaleProjectPackageNew/model/OfferHistoryInfo.html';
var urlOfferSetInfo = 'Auction/Sale/Agent/SaleProjectPackage/model/OfferSetInfo.html';
var urlOfferViewInfo = 'Auction/Sale/Agent/SaleProjectPackageNew/model/OfferViewInfo.html';
...
var urlJMHistoryInfo = 'Auction/Sale/Agent/SaleProjectPackage/model/JMOfferHistoryInfo.html';
...
function AddFunction(value, row, index) { //把需要创建的按钮封装到函数中
	...
	if(row.isStopCheck == 1){
		createType = 1;
		return [
			'<button type="button" id="btnShow" onclick=btnButton("' + row.projectId + '","' + row.id + '","' + createType + '") class="btn-xs btn btn-primary"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span>查看历史</button>',
		].join("")
	}else{
			...
			if(result.isStopCheck == 1){
				...
				if(result.auctionStatus == '未开始') {
					parent.layer.open({
						...
						resize: false //是否允许拉伸
							,
						content: urlOfferSetInfo + '?projectId=' + projectId + '&id=' + packageId + '&createType=' + creType,
						success: function(layero, index) {
							iframeWin = layero.find('iframe')[0].contentWindow;
							/* iframeWin.passMessage({
								'quotationUnit': result.quotationUnit
