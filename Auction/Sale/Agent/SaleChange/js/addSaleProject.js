 ...
 var findProjectSupplementInfoUrl = config.AuctionHost + "/ProjectSupplementController/findProjectSupplementInfo.do";
 ...
 $(function() {
	...
	new UEditorEdit({
 	...
 	$('#btn_submit').click(function() {
 		...
 		// 					parent.layer.alert('竞卖开始时间必须大于等于公告截止时间、提出澄清截止时间、答复截止时间、竞卖文件递交截止时间、竞卖文件审核截止时间!');
 		// 					return false;
 		// 				}
 		// 			} else {
 		// 				parent.layer.alert('竞卖开始时间必须大于等于公告截止时间、提出澄清截止时间、答复截止时间!');
 		// 				return false;
 		// 			}
 		// 		}
 		...
 		if(globalInfo.isFile != 1 && globalInfo.isFile != undefined) {
 			...
 			if(NewDate(fileCheckEndDate) < NewDate(fileEndDate)) {
 				parent.layer.alert('竞卖文件审核截止时间必须大于等于竞卖文件递交截止时间!');
 				return false;
 			}
 		}
 ...
 function choseProject() {
 	parent.layer.open({
 		...
 		title: '选择项目',
 		area: ['1100px', '600px'],
 		content: 'Auction/Sale/Agent/SaleChange/model/Purchasers.html?tenderType=2'
 			//,btn: ['关闭']
 			,
 ...
 function showPurchse(Data) {
 	...
 	time()
 	projectDataID = Data;
 	if(projectDataID.id && urlType == 'edit') {
 		findWorkflowCheckerAndAccp(projectDataID.id, 'xmby');
 	}
 	if(Data.id) {
 		var DataList = {
 			id: Data.id,
...
function changHtml(templateId){
 ...
 function Purchase(id) {
 	...
 	//findWorkflowCheckerAndAccp(projectId, 'xmby');
