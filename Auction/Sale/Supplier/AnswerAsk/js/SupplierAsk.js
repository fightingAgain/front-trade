...
$(function() {
	...
	$("#btnSearch").on('click', function() {
		$('#PurchaseAsk').bootstrapTable('destroy');
		initTable();
	});
	//提问状态的改变
...
window.operateEvents = { //添加一个按钮对应的事件
	"click #btn_add": function(e, value, row, index) {
		...
		layer.open({
			...
			resize: false, //是否允许拉伸
			area: ['700px', '600px'],
			content: 'Auction/Sale/Supplier/AnswerAsk/CommitAsk.html',
			// btn: ['提交', '取消'],
			yes: function(index, layero) {
				var iframeWin = layero.find('iframe')[0].contentWindow;
				
				if($.trim(iframeWin.$("#askTitle").val()).length > 100){
					layer.msg("提问标题不能超过100字");
					return ;
				}
				
				if($.trim(iframeWin.$("#askTitle").val()).length == 0){
					layer.msg("提问标题不能为空");
					return ;
				}
				if($.trim(iframeWin.$("#askContent").val()).length > 1000){
					layer.msg("提问内容不能超过1000字");
					return ;
				}
				 
				if($.trim(iframeWin.$("#askContent").val()).length == 0){
					layer.msg("提问内容不能为空");
					return ;
				} 
				iframeWin.Ti();
			...
			success: function(layero, index) {
				...
				iframeWin.passMessage(function(){
					if($.trim(iframeWin.$("#askTitle").val()).length > 100){
						layer.alert("提问标题不能超过100字");
						return ;
					}
					
					if($.trim(iframeWin.$("#askTitle").val()).length == 0){
						layer.alert("提问标题不能为空");
						return ;
					}
					if($.trim(iframeWin.$("#askContent").val()).length > 1000){
						layer.alert("提问内容不能超过1000字");
						return ;
					}
					 
					if($.trim(iframeWin.$("#askContent").val()).length == 0){
						layer.alert("提问内容不能为空");
						return ;
					} 
					iframeWin.Ti();
				})
	...
	"click #btnShow": function(e, value, row, index) {		
		layer.open({
			...
			// maxmin: false, //该参数值对type:1和type:2有效。默认不显示最大小化按钮。需要显示配置maxmin: true即可
			resize: false, //是否允许拉伸
			content: 'Auction/Sale/Supplier/AnswerAsk/Ask.html',
			success: function(layero, index) {
				var body = layer.getChildFrame('body', index);
				var iframeWin = layero.find('iframe')[0].contentWindow;
				var data = {
...
function AddFunction(value, row, index) { //把需要创建的按钮封装到函数中
	if($("#checkState").val() == '0' || $("#checkState").val() == '') {
		return [
			'<button type="button" id="btn_add" class="btn-xs btn btn-primary"><span class="glyphicon glyphicon-edit " aria-hidden="true"></span>提问</button>',
					...
					formatter:function(value, row, index){
						if(row.isPublic>1){
							if(row.project.projectSource==1){
								return row.project.projectName+'<span class="text-danger" style="font-weight: bold;">(重新竞卖)</span><span class="text-danger" style="font-weight: bold;">(邀请)</span>'
							...
							if(row.project.projectSource==1){
								return row.project.projectName+'<span class="text-danger" style="font-weight: bold;">(重新竞卖)</span>'
							}else if(row.project.projectSource==0){
