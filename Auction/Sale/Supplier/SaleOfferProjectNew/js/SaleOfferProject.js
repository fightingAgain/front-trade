...
function initTable() {
	$('#projectPackage').bootstrapTable({
		url: config.AuctionHost + '/AuctionProjectPackageController/pageAuctionHighPackage.do',
		cache: false,
		pagination: true,
				...
				},
				{
					...
					events: {
						'click .btn-opera': function(e, value, row, index) {
								...
								openIfiame(row.projectId, row.id, 2)
						}
					},
					formatter: function(value, row, index) {
						var now = new Date();
...
function affirmBtn() {
	...
	parent.layer.open({
		...
		resize: false, //是否允许拉伸
		closeBtn: 1,
		content: 'Auction/Sale/Supplier/SupplierSalePurchase/model/affirm.html?projectId=' + rowProId,
...
function openOrder(packageId, uid) {
	var rowData = $('#projectPackage').bootstrapTable('getData');
	for(var i = 0; i < rowData.length; i++) {
		if(packageId == rowData[i].id) {
			groupCodedata = rowData[i].groupCode;
		}

	}
	if(groupCodedata == null || groupCodedata == '' || groupCodedata == undefined) {
		var openhtml = 'Auction/Sale/Supplier/SaleOfferProjectNew/model/view_Offer.html'
	} else if((groupCodedata != null || groupCodedata != '' || groupCodedata != undefined) && groupCodedata == '00270012') {
		var openhtml = 'Auction/Sale/Supplier/SaleOfferProjectNew/model/JMview_Offer.html' //编辑路径
	}
	var index = layer.open({
		...
		area: ['100%', '100%'],
		resize: false, //是否允许拉伸
		content: $.parserUrlForToken(openhtml) + '#' + packageId
	});
}