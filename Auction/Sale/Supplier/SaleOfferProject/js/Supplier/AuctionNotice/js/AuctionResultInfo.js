				...
				if(auctionModel == 0) { //按照包件
					...
					for(var i = 0; i < BidResultItem.length; i++) {
						...
						if(BidResultItem[i].isEliminated != undefined && BidResultItem[i].isEliminated == '0'){//未淘汰
							html += "<td style='width:110px;'><select class='form-control isBid' name='bidResultItems[" + i + "].isBid'><option value='1'>未中选</option><option value='0'>中选</option></select></td>";
						}else{
					...
					for(var j = 0; j < item.length; j++) {
						...
						for(var i = 0; i < item[j].offerItems.length; i++) {
							...
							html += "<td style='vertical-align: middle;text-align:left' class='enterpriseName' data-tr='" + j + "'  data-td='" + i + "'>" + item[j].offerItems[i].enterpriseName + "</td>";
...
function viewBidResult($index,temp) {	
	...
	top.layer.open({
		...
		resize: false,
		closeBtn: 1,
		content: '0502/Bid/BidNotice/modal/newViewResult.html',
...
function editBidResults($index,$temp){
	if($('input[name="isShow"]:checked').val()!=undefined&&$('input[name="isShow"]:checked').val()!=""){
		item[$temp].offerItems[$index].isShow= $('input[name="isShow"]:checked').val();
	}else{
		item[$temp].offerItems[$index].isShow=0;
	...
	top.layer.open({
		...
		yes:function(index,layero){ 
			...
			item[$temp].offerItems[$index].resultContent=iframeWin.editor.txt.html()//编辑后的通知临时存到数据中
			$(".bidContent[data-tr='" + $temp + "'][data-td='" + $index + "']").val(iframeWin.editor.txt.html())
			parent.layer.close(index);
		}
	});
}