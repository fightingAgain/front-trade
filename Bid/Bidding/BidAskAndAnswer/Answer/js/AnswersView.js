...
var editView = 'Bidding/BidAskAndAnswer/Answer/model/AnswersEdit.html';
...
function editAnswers(id,askTitle){
	if(GetTime(new Date()) > GetTime(answersEndDate) ){
		...
		parent.layer.open({
			...
			area: [width + 'px', height + 'px'],
			resize: false,
			content: editView + "?id=" + id+ "&askTitle=" + askTitle+ "&examType=" + examType,
			success: function(layero, index) {
