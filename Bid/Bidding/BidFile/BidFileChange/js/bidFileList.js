...
var urlBidFileList = config.tenderHost + '/DocClarifyController/findToChangDocPageList.do'; //招标文件--标段查询分页接口
var resetflieurl = config.tenderHost + '/DocClarifyController/resetDocClarifyByIdForChange.do'; //撤回招标文件及附件信息
...
function openEdit(index,btnType) {
	...
	jumpUrl = url + '?id=' + rowData.bidSectionId + '&special='+ btnType
	layer.open({
		type: 2,
		title: "招标文件变更",
...
function openView(index) {
	...
	layer.open({
		...
		area: [width + 'px', height + 'px'],
		resize: false,
		content: pageView + "?source=0" + "&id=" + rowData.bidSectionId + "&isThrough=" + (rowData.bidDocClarifyState == 2 ? 1 : 0), //标段主键id
		success: function(layero, idx) {
			var iframeWin = layero.find('iframe')[0].contentWindow;	
