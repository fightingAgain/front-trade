...
var urlBidFileList = config.tenderHost + '/DocClarifyController/findBidSectionPageList.do'; //招标文件--标段查询分页接口
...
var pageEdit = 'Bidding/BidFile/BidFileManage/model/BidFileEdit.html';//编辑
var pageChange = 'Bidding/BidFile/BidFileChange/model/BidFileEdit.html';//变更
...
$(function() {
	...
	$("#table").on("click", ".btnDel", function() {
		var index = $(this).attr("data-index");
		var rowData = $('#table').bootstrapTable('getData')[index];
		parent.layer.confirm('确定删除该招标文件?', {
			icon: 3,
			title: '询问'
		}, function(index) {
			parent.layer.close(index);
...
function openEdit(index, type) {
	...
	var url = (type == 'change'?pageChange:pageEdit) + '?id='+ rowData.bidSectionId;
	layer.open({
		type: 2,
		title: type == 'change'?"招标文件变更":"招标文件编辑",
		area: ['100%','100%'],
		resize: false,
		content: url,
...
function openView(index) {
	...
	layer.open({
		...
		area: ['100%', '100%'],
		resize: false,
		content: pageView + "?source=0" + "&id=" + rowData.id + "&isThrough=" + (rowData.bidDocClarifyState == 2 ? 1 : 0)+"&bidSectionId="+rowData.bidSectionId, //标段主键id
		...
		},
		{
			...
			formatter: function(value, row, index) {
				...
				if(row.owner && row.owner == 1){//权限  是否有操作权限1是  0否
					if(row.states != 1){
						if(row.bidDocClarifyState || row.bidDocClarifyState === 0){
							return strSee;
						} else {
							return "";
						}
					...
					if(row.bidDocClarifyState == 0) { //0为临时保存，1为提交审核，2为审核通过，3为审核未通过
						...
						str += strSee + strEdit + strDel;
					} else if(row.bidDocClarifyState == 4) {
						str += strSee + strEdit + strDel;
					} else {
						str += strEdit;
					}
