/**
 *  hwf
 *  2019-12-10
 */

//var listUrl = config.Reviewhost+'/ProjectController/findManagerPageList.do';//招标文件--标段查询分页接口
var listUrl = config.tenderHost + '/CheckController/findBidSectionPageList.do'; //招标文件--标段查询分页接口

var pageEdit = 'Bidding/ReviewSet/model/reviewEdit.html'; //编辑
var pageView = 'Bidding/ReviewSet/model/reviewView.html'; //查看

var fileState = 1; //文件当前状态，1是购买，2是下载
//表格初始化
$(function() {
	initTable();

	$('#query').queryCriteria({
		isExport: 0, //0是不需要导出，1是需要导出
		isQuery: 1, //0是不查询，1是查询
		isAdd: 0, //0是不新增，1是新增
		QueryName: 'btnSearch',
		queryList: [{
				name: '标段编号',
				value: 'interiorBidSectionCode',
				type: 'input',
			},
			{
				name: '标段名称',
				value: 'bidSectionName',
				type: 'input',
			},
			{
				name: '资格审查方式',
				value: 'examType',
				type: 'select',
				option: [{
					name: '全部',
					value: '',
				}, {
					name: '资格预审',
					value: '1',
				}, {
					name: '资格后审',
					value: '2',
				}, ]
			}
		]
	});

	// 搜索按钮触发事件
	$("#btnSearch").click(function() {
		$("#table").bootstrapTable('destroy');
		initTable();
	});

	//编辑
	$("#table").on("click", ".btnEdit", function() {
		var index = $(this).attr("data-index");
		var row = $('#table').bootstrapTable('getData')[index]
		$.ajax({
			type: "post",
			url: config.Reviewhost +'/ReviewControll/getProjectState',
			async: false,
			data: {
				examType: row.examType,
				bidSectionId: row.bidSectionId,
			},
			success: function (data) {
				if (data.success) {
					if (data.data) {
						top.layer.confirm('温馨提示：已开始评标，修改评审项后评标自动回退，需重新评标，确定修改评审项？', function (layerIndex) {
							openEdit(index,1);
							parent.layer.close(layerIndex);
						});
					} else {
						openEdit(index);
					}
				} else {
					top.layer.alert("温馨提示："+data.message)
				}
			}
		})
	});
	//查看
	$("#table").on("click", ".btnView", function() {
		var index = $(this).attr("data-index");
		openView(index);
	});
	//
	$("[name='examType']").change(function() {
		$("#table").bootstrapTable('destroy');
		initTable();
	});

});
//编辑
function openEdit(index,isTip) {
	var bidData = $('#table').bootstrapTable('getData')[index];
	layer.open({
		type: 2,
		title: "评审项设置",
		area: ['80%', '90%'],
		resize: false,
		content: pageEdit + "?id=" + bidData.bidSectionId + "&examType=" + bidData.examType +"&isTip="+isTip
	});
}
//查看
function openView(index) {
	var bidData = $('#table').bootstrapTable('getData')[index];
	layer.open({
		type: 2,
		title: "评审项设置",
		area: ['80%', '90%'],
		resize: false,
		content: pageView + "?id=" + bidData.bidSectionId + "&examType=" + bidData.examType
	});
}
//设置查询条件
function queryParams(params) {
	return {
		'pageNumber': params.offset / params.limit + 1, //当前页数
		'pageSize': params.limit, // 每页显示数量
		'offset': params.offset, // SQL语句偏移量
		'interiorBidSectionCode': $("#interiorBidSectionCode").val(),
		'bidSectionName': $("#bidSectionName").val(),
		'examType': $("[name='examType']").val()
	}
};

function initTable() {
	$('#table').bootstrapTable({
		method: 'GET', // 向服务器请求方式
		contentType: "application/x-www-form-urlencoded", // 如果是post必须定义
		url: listUrl, // 请求url		
		cache: false, // 是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
		striped: true, // 隔行变色
		dataType: "json", // 数据类型
		pagination: true, // 是否启用分页
		showPaginationSwitch: false, // 是否显示 数据条数选择框
		pageSize: 15, // 每页的记录行数（*）
		pageNumber: 1, // table初始化时显示的页数
		pageList: [10, 15, 20, 25],
		search: false, // 不显示 搜索框
		sidePagination: 'server', // 服务端分页
		classes: 'table table-bordered', // Class样式
		silent: true, // 必须设置刷新事件
		toolbar: '#toolbar', // 工具栏ID
		toolbarAlign: 'left', // 工具栏对齐方式
		queryParams: queryParams, // 请求参数，这个关系到后续用到的异步刷新
		queryParamsType: "limit",
		height: top.tableHeight,
		toolbar: "#toolbar",
		onCheck: function(row) {
			id = row.id;
			projectId = row.peojectId;
		},
		columns: [{
				field: 'xh',
				title: '序号',
				align: 'center',
				width: '50px',
				formatter: function(value, row, index) {
					var pageSize = $('#table').bootstrapTable('getOptions').pageSize || 15; //通过表的#id 可以得到每页多少条  
					var pageNumber = $('#table').bootstrapTable('getOptions').pageNumber || 1; //通过表的#id 可以得到当前第几页  
					return pageSize * (pageNumber - 1) + index + 1; //返回每条的序号： 每页条数 * （当前页 - 1 ）+ 序号 
				}
			}, {
				field: 'interiorBidSectionCode',
				title: '标段编号',
				align: 'left'
			}, {
				field: 'bidSectionName',
				title: '标段名称',
				align: 'left'
			}, {
				field: 'examType', //招标文件主表的创建时间
				title: '资格审查方式',
				width: '180',
				align: 'center',
				formatter: function(value, row, index) {
					if(value == 1) {
						return "资格预审";
					} else if(value == 2) {
						return "资格后审";
					}
				}
			},
			//		{
			//			field: '',
			//			title: '评审类型',
			//			align: 'center',
			//			formatter:function(value, row, index){
			//				if(row.examType == 1){
			//
			//				} else {
			//					
			//				}
			//			}
			//		},
			{
				field: '',
				title: '操作',
				align: 'left',
				width: '150',
				formatter: function(value, row, index) {
					var strSee = '<button  type="button" class="btn btn-primary btn-sm btnView" data-index="' + index + '"><span class="glyphicon glyphicon-eye-open"></span>查看</button>';
					var strEdit = '<button  type="button" class="btn btn-primary btn-sm btnEdit" data-index="' + index + '"><span class="glyphicon glyphicon-edit"></span>编辑</button>';
					if(row.owner && row.owner == 1){//权限  是否有操作权限1是  0否
						if(row.status == 2){//评审报告已审核通过
							return strSee ;
						}else{
							return strSee + strEdit;
						}
					}else{
						return strSee;
					}
				}
			}
		],
	})
};