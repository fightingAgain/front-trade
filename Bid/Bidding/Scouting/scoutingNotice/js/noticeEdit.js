...
var saveUrl = config.tenderHost+'/ReconnaissanceSiteController/saveAndUpdate.do';//保存接口
...
function passMessage(data,callback){
	...
	$('#btnSubmit').click(function(){
		...
		if(noticeEndTime <= noticeSendTime){
			parent.layer.alert('温馨提示：踏勘结束时间应在踏勘开始时间之后！');
			return
		}
		if(checkForm($("#formEle"))){//必填验证，在公共文件unit中
			if($.trim($('#linkMen').val()).length > 10){
				parent.layer.alert('温馨提示：请正确输入联系人！');
				return
			}else if(ue.getContentTxt()==''){
				parent.layer.alert('温馨提示：请填写通知内容！',function(ind){
					parent.layer.close(ind);
//					$('#collapseSeven').collapse('show');
				})
			}else{
				parent.layer.confirm('温馨提示：确定发送？',{icon:3,title:'询问'},function(index){
