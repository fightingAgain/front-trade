	var bidpageUrl = config.openingHost  + "/BidOpeningSituationController/findSituationPageList.do"; //标段分页查询接口
		...
		$("#tableList").bootstrapTable({
			...
			columns: [
				...
				},
				{
					...
					formatter: function (value, row, index) {
						//开标已完成 开标结束中，为查看
						if(typeof(row.stageState)=="undefined" || parseInt(row.stageState)<=3){
							var handler = "<a class='btn btn-primary btn-sm' href='Bidding/OpenTender/manager/model/bidRoom.html?bid="+row.id+"' target='开标大厅'><span class=\"glyphicon glyphicon-edit\"></span>进入开标室</a>";
						}else{
							var handler = "<a class='btn btn-primary btn-sm' href='Bidding/OpenTender/manager/model/bidRoomView.html?bid="+row.id+"' target='开标大厅' ><span class='glyphicon glyphicon-eye-open'></span>查看</a>";
						}
