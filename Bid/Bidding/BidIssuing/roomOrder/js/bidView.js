...
function getDetail() {
	$.ajax({
		...
		success: function (data) {
			...
			for (var key in arr) {
				if (key == "projectCosts") {
					formatCost(arr[key], arr.priceUnit);
				} else if (key == "bidType") {
				...
				} else if(key == "enterSignUp" || key == "aloneSetCheck" || key == "signUp" || key == "isCollectDeposit"){
         			$("#" + key).html(arr[key] == 1 ? "是" : "否");
					if(key == "isCollectDeposit"){
						payDeposit(arr[key], arr.depositCollect);
					}
				...
				} else if (key == "bidderPriceType") {
					$("#bidderPriceType").html(arr[key] == 1 ? "金额（元/万元）" : "费率");
				} else {
					/*控制台过来时无法通过url传递相关信息*/
					if (arr.tenderMode == 1) {
						$(".isSignUpShow").show();
					...
					// $('#tenderProjectClassifyCode').html(getTenderType(arr.tenderProjectClassifyCode))
			...
			if(arr.depositChannelType == 1){
				$('.type_offline').show();
				$("#bankType").html(arr.bankType == 1 ? "工商银行" : arr.bankType == 2? "招商银行":'');
			}else{
				$('.type_offline').hide();
			}
			...
			if (arr.signUpType == 1 && arr.deliverFileType == 1 && arr.bidOpenType == 1 && arr.bidCheckType == 1) {
				...
				$("[name='signUpType'][value=" + arr.signUpType + "]").prop("checked", "checked");
				$("[name='deliverFileType'][value=" + arr.deliverFileType + "]").prop("checked", "checked");
				$("[name='bidOpenType'][value=" + arr.bidOpenType + "]").prop("checked", "checked");
				$("[name='bidCheckType'][value=" + arr.bidCheckType + "]").prop("checked", "checked");
...
//格式化费用
function formatCost(data, priceUnit) {
			...
			if (data[i].costName == $("[name='projectCosts[" + j + "].costName']").val()) {
				for (var key in data[i]) {
					if (key == "isPay") {
						...
						winBid(data[i][key], data[i].discount, priceUnit);
...
//中标服务费收取方式
function winBid(payType, discount, priceUnit) {
	if (payType == 3) {
		//		if(discount == 1){
		...
		$(".payTypeTit").html('中标服务费收费金额(' + (priceUnit == 1 ? '万元' : '元') + ')');
		$(".zjPayMoney").show();
		$(".zjPayRatio").hide();
		$(".zjDiscount").hide();
	}
...
/************************  是否收取保证金   *******************/
function payDeposit(val, gather){
	if(val == 1){
