...
var tenderUrl = config.tenderHost + '/GDServerController/findPTGDPageList.do'; //获取所有标段列表
			...
			events: {
				'click .btnEdit': function(e,value, row, index){
					top.layer.open({
						...
						resize: false,
//						id:'layerPackage',
						content: editHtml+'?bidSectionId='+row.bidSectionId+'&examType='+row.examType+'&tenderMode='+row.tenderMode+'&projectId='+row.projectId+'&id='+(row.gdId?row.gdId:''),
...
//									iframeWin.getLoginInfo(loginInfo);
			...
			formatter:function(value, row, index){
				...
				return strView;
//          	if(row.filingState == 2){
//          		return strView;
//          	}else if(row.filingState == 0 || row.filingState == 1 || row.filingState == 3){
