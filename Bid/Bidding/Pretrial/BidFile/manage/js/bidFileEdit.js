...
var saveUrl = config.tenderHost + '/PretrialDocClarifyController/savePretrialDocClarify.do';  //保存接口
var detailUrl = config.tenderHost + '/PretrialDocClarifyController/findPretrialDocByBidId.do';  //资格预审文件详情

var bidSectionPage = 'Bidding/BidFile/BidFileManage/model/bidSectionList.html';  //选择标段页面
...
//		bidSectionId = data.bidSectionId;
		...
		/* typeCode = data.tenderProjectType.substring(0,1);
		isPay = data.isPay;
		if(typeCode == "A"){
			if(isPay == 0){
				$(".drawShow").hide();
	...
	$("#btnSubmit").click(function() {
		...
		if($("#addChecker").length<=0){
			parent.layer.confirm('此流程未设置审批人，提交后将自动审核通过，是否确认提交审核', {
				title: "提交审核",
				btn: [' 是 ', ' 否 '],
	...
	$('#fileBid').click(function(){
		...
		if(bidSectionId==""){
			parent.layer.alert("请选择标段",{icon:7,title:'提示'},function(ind){
				parent.layer.close(ind);
				$('#collapseOne').collapse('show');
			});
		...
		if(!(fileId && fileId!="")){
			saveForm('true', true, callback, function(){
...
function chooseCallback(data){
	if(data.length > 0){
		...
		bidSectionId = data[0].bidSectionId;
		typeCode = data[0].tenderProjectType.substring(0,1);
		isPay = data[0].isPay;
		if(typeCode == "A"){
			if(isPay == 0){
				$(".drawShow").hide();
		...
		pretrialOpenType = data[0].pretrialOpenType;
		if(pretrialOpenType == 1){
			extFilters = [".zswj"];
		} else {
			extFilters = [".doc", ".docx", ".pdf"];
		}
	}
}
 ...
 * callback 回调方法
 */
function saveForm(isSave, isAlert, callback ,back){
	var data;
	if(!isSave){
		...
		if(!CAcf){
			CAcf = new CA({
				...
				confirmCA:function(flag){
					...
					savePost(data,isSave, isAlert, callback ,back);
				}
			});
		}
		setTimeout(function(){
...
function savePost(arr,isSave, isAlert, callback, back){
	$('#btnSubmit').attr('disabled', true);
	$.ajax({
		type: "post",
		url: saveUrl,
...
function detail(id){
	$.ajax({
		...
		url: top.config.tenderHost,
				...
				if(!pretrialOpenType){
					pretrialOpenType = arr.pretrialOpenType;
				}
				if(pretrialOpenType == 1) { 
					extFilters = [".zswj"];
				} else {
...
//						drawUploads.fileHtml(fileArr.file2);
				...
				/*if(arr.marginPayType && arr.marginPayType!=undefined && arr.marginPayType!=''){				
