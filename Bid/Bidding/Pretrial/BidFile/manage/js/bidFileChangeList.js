...
var urlBidFileList = config.tenderHost + '/PretrialDocClarifyController/findToChangPretrialDocPageList.do'; //资格预审文件--标段查询分页接口
var resetflieurl = config.tenderHost + '/PretrialDocClarifyController/resetPretrialDocClarifyForChange.do'; //撤回资格预审文件及附件信息
var deleteUrl = config.tenderHost + '/PretrialDocClarifyController/deleteClarifyByBidId.do'; //删除资格预审文件及附件信息

var pageEdit = 'Bidding/Pretrial/BidFile/manage/model/bidFileChangeEdit.html';
var pageView = 'Bidding/Pretrial/BidFile/manage/model/bidFileChangeView.html';
...
$(function() {
	...
	//编辑补充说明
	$("#table").on("click", ".btnAddRemark", function() {
		var index = $(this).attr("data-index");
		openRemark(index);
	});
 ...
 */
function openEdit(rowData,btnType) {
	var width = top.$(window).width() * 0.8;
	var height = top.$(window).height() * 0.9;
	var url = "";
	url = pageEdit + '?id=' + rowData.bidSectionId + '&special='+ btnType;
	layer.open({
		type: 2,
		title: "资格预审文件变更",
...
function openView(index) {
	...
	layer.open({
		...
		area: [width + 'px', height + 'px'],
		resize: false,
		content: pageView + "?source=0" + '&id=' + rowData.bidSectionId + "&isThrough=" + (rowData.pretrialDocClarifyState == 2 ? 1 : 0), //标段主键id
		success: function(layero, idx) {
			var iframeWin = layero.find('iframe')[0].contentWindow;	
	...
	$('#table').bootstrapTable({
		...
		},
		{
			...
			formatter: function(value, row, index) {
				...
				if(row.isReplenish == 1){
					str += " 时间变更"; 
				}
				return str;
			}
		},*/{
			field: 'bidOpenTime', //招标文件主表的创建时间
			title: '文件开启时间',
			align: 'center',
			cellStyle:{
				css:widthDate
			...
			events:{
				...
				'click .btnChange':function(e,value, row, index){
					openEdit(row,"CHANGE")
				},
			...
			formatter: function(value, row, index) {
				...
				if(row.owner && row.owner == 1){//权限  是否有操作权限1是  0否
					if(row.pretrialDocClarifyState == 0) { //0为临时保存，1为提交审核，2为审核通过，3为审核未通过
						...
						if(row.isOpend == 1){
							str += strSee + strChange +strCancel;
						}else{
							str += strSee;
						}
