...
*  方法列表及功能描述
*/
var listUrl = config.tenderHost + '/ResultNoticeController/findResultNoticeYuShenPageList.do';  //列表接口
...
var editHtml = 'Bidding/Pretrial/Judge/judgeResultNotice/model/edit.html';//编辑
var viewHtml = 'Bidding/Pretrial/Judge/judgeResultNotice/model/view.html';//查看
...
function openEdit(data){
	var dataUrl;
	if(data.id){
		dataUrl = editHtml+'?id='+data.id+'&examType='+data.examType+'&bidId='+data.bidSectionId; 
	}else{
		dataUrl = editHtml+'?examType='+data.examType+'&bidId='+data.bidSectionId;
	}
	layer.open({
		type: 2,
		title: '编辑预审结果通知',
...
function openView(index){
	...
	layer.open({
		...
		title: '查看预审结果通知',
		area: ['100%', '100%'],
		content: viewHtml+'?id='+rowData.id+'&bidId='+rowData.bidSectionId + "&isThrough=" + (rowData.noticeState == 2 ? 1 : 0)+'&source=0',
		resize: false,
		success:function(layero, index){
			var iframeWin = layero.find('iframe')[0].contentWindow;
			//调用子窗口方法，传参
				...
				},
				{
					...
					formatter: function(value, row, index){
						...
						if(row.owner && row.owner == 1){//权限  是否有操作权限1是  0否
							if(row.states != 1){
								if(row.noticeState || row.submitState == 0){
									return strView;
								} else {
									return "";
								}
