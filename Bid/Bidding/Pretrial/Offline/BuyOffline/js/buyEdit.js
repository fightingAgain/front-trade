...
var pageEdit = 'Bidding/Pretrial/Offline/BuyOffline/model/supplierEdit.html';  //编辑投标人购标信息
var pageView = 'Bidding/Pretrial/Offline/BuyOffline/model/supplierView.html';  //编辑投标人购标信息

var listUrl = config.tenderHost + '/SupplierSignController/selectPageByYSBidSectionId.do';  //线下购标的供应商列表
var deleteUrl = config.tenderHost + '/SupplierSignController/delete.do';  //删除
var importExcelUrl = config.tenderHost + '/SupplierSignController/importPreExcel.do';  //导入
var exportExcelUrl = config.tenderHost + '/SupplierSignController/exportExcel.do';  //导出
var bidSectionId = "";  //招标文件id

var examType = 2;
var tenderMode;//招标方式 1 公开  2 邀请
$(function(){
...
function importf(obj){ 
   ...
   $.ajax({
		...
		success: function(data) {
			$(".iptFile").val("");
			if(data.success){
				parent.layer.alert(data.data,function(idx){
...
function passMessage(data){
	if(data){
		...
		tenderMode = data.tenderMode;
		if(data.tenderMode == 2){//邀请
			$("#addBtn").hide();
		} else {
			$("#addBtn").show();
		}
...
function openEdit(index) {
	...
	parent.layer.open({
		...
		resize: false,
		content: pageEdit + "?bidSectionId=" + bidSectionId + "&id=" + tenderId + "&supplierId="+sid,
...
function openView(index) {
	...
	parent.layer.open({
		...
		area: [width + 'px', height + 'px'],
		resize: false,
		content: pageView + "?bidSectionId=" + bidSectionId + "&id=" + tenderId,
		success: function(layero, idx) {
//			var iframeWin = layero.find('iframe')[0].contentWindow;	
	...
	$('#tableList').bootstrapTable({
		...
		},
		{
			...
			formatter: function (value, row, index) {
				...
				if(value == 0) { //0为临时保存，1为提交审核，2为审核通过，3为审核未通过
					if(tenderMode == 1){
						str += strEdit + strDel;
					}else{
						str += strEdit
