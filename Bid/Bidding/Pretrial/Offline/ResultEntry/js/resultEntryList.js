...
*  方法列表及功能描述
*/
var listUrl = config.tenderHost + '/UnderLineReportController/findPagePreAuditList.do';  //列表接口


var editHtml = "Bidding/Pretrial/Offline/ResultEntry/model/resultEntryEdit.html";  //编辑
var viewHtml = 'Bidding/Pretrial/Offline/ResultEntry/model/resultEntryView.html';//查看

var bidHtml = 'Bidding/Pretrial/Offline/ResultEntry/model/bidList.html';//选择标段
...
//						var strDel = '<button  type="button" class="btn btn-danger btn-sm btnDel" data-index="'+index+'"><span class="glyphicon glyphicon-remove"></span>删除</button>';
						if(row.owner && row.owner == 1){//权限  是否有操作权限1是  0否
							...
							if(row.reportState || row.reportState  == 0){
								return strView;
							}else{
								return '';
							}
