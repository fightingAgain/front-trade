...
var bidderUrl = config.tenderHost + '/UnderLineReportController/findPreUnderLineSuppliers.do';//新增时获取投标人信息
...
fileUploads = null;
var reg=new RegExp(/^(\d{1,4})(\.\d{1,2})?$/);
		...
		for(var i = 0;i<tr.length;i++){
			...
			if(isKey == 1){
				if($(tr[i]).find('[name="offlineCheckResults['+(i-1)+'].supplierOrder"]').val() == ''){
					parent.layer.alert('请输入未被淘汰的投标人名次！',{icon:7,title:'提示'}, function(idx){
		...
		for(var i = 0;i<tr.length;i++){
			...
			if(isKey == 1){
				if($(tr[i]).find('[name="offlineCheckResults['+(i-1)+'].supplierOrder"]').val() == ''){
					...
					parent.layer.alert('请输入未被淘汰的投标人得分！',{icon:7,title:'提示'}, function(idx){
						parent.layer.close(idx);
						$('#collapseTwo').collapse('show');
					...
					return
				}else if(!reg.test($(tr[i]).find('[name="offlineCheckResults['+(i-1)+'].score"]').val())){
						parent.layer.alert('温馨提示：输入值限定为最多四位整数两位小数的值！',{icon:7,title:'提示'}, function(idx){
						parent.layer.close(idx);
						$('#collapseTwo').collapse('show');
						$('[name="offlineCheckResults['+(i-1)+'].score"]').focus();
				...
				if($(tr[i]).find('[name="offlineCheckResults['+(i-1)+'].reason"]').val() == ''){
					parent.layer.alert('请输入被淘汰的投标人的淘汰原因！',{icon:7,title:'提示'}, function(idx){
						parent.layer.close(idx);
						$('#collapseTwo').collapse('show');
...
//		if(winner.length < 1){
//			parent.layer.alert('请选择中标候选人！',{icon:7,title:'提示'},function(idx){
//				parent.layer.close(idx);
//				$('#collapseTwo').collapse('show');
//			});
//			return;
//		}
		...
		if(checkEndDate <= checkStartDate){
			parent.layer.alert('评标结束时间应在评标开始时间之后！',{icon:7,title:'提示'},function(idx){
				parent.layer.close(idx);
...
function passMessage(data){
	...
	if(data.pretrialCheckType ){//预审评审办法 1.合格制 2.有限数量制  3.评审制
		if(data.pretrialCheckType == 1){
			$("#checkType").html("合格制");
		}else if(data.pretrialCheckType == 2){
			$("#checkType").html("有限数量制");
		}else if(data.pretrialCheckType == 3){
			$("#checkType").html("评审制");
		}
	}
}
...
//		};
		...
		var textarea = '';
		if(data[i].isKey == 0){
			option1 = '<option value="0" selected>否</option>'
					+'<option value="1">是</option>';
					
			option2 = '<option value="1">未淘汰</option>'
					+'<option value="0" selected>淘汰</option>';
			textarea = '<textarea name="offlineCheckResults['+i+'].reason"  class="form-control reason" rows="2" cols="" style="resize: none;">'+data[i].reason+'</textarea>';
...
//			disable = 'disabled';
			...
			textarea = '<textarea name="offlineCheckResults['+i+'].reason"  class="form-control reason" rows="2" cols="" style="resize: none;display:none;">'+data[i].reason+'</textarea>';
		};
		...
		html += '<tr>'
			...
			+'<td style="width: 100px;text-align: center;"><input type="number"   name="offlineCheckResults['+i+'].score"  oninput="priceInput(this,2)"  onblur="priceTest(this)" value="'+data[i].score+'"  class="form-control sumNumber" style="text-align: center;"/></td>'		//最终分值
			
			+'<td style="width: 90px;text-align: center;display:none;">'
...
function priceTest(ele){
	/* if(Number(ele.value) >= 100){
		parent.layer.alert('得分超过100，请确认！',{title:'提示'},function(ind){
...
function save(state,isTips,callback){
	...
	var param = $.param({'reportState':state})+'&'+$('#addNotice').serialize();
	$.ajax({
		type:"post",
		url: saveUrl,
		async:false,
...
$('#bidResult').on('change','.sumNumber',function(){
	
	if(!reg.test($(this).val())){
		parent.layer.alert("温馨提示：输入值限定为最多四位整数两位小数的值！");
		return ;
	}
