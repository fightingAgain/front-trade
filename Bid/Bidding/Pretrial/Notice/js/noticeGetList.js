...
*  方法列表及功能描述
*/
var viewHtml = "Bidding/Pretrial/Notice/model/noticeGetView.html"; // 查看
...
function openView(index){
	...
	layer.open({
		...
		title: "资格预审公告",
		area: ['100%', '100%'],
		content: viewHtml + "?id=" + rowData[index].id+"&noticeId=" + rowData[index].noticeId + "&signState=" + rowData[index].signState + "&bidSectionId="+rowData[index].bidSectionId + "&bidFileId="+rowData[index].pretrialDocClarifyId + "&states="+rowData[index].states,
		resize: false,
		success: function(layero, idx){
			var iframeWin = layero.find('iframe')[0].contentWindow;
			iframeWin.getMethod(rowData[index]); 
		}
	});
}
