...
*  方法列表及功能描述
*/
var detailUrl = config.tenderHost + '/SupplierSignController/findPitchYuShenList.do';//供应商
var viewHtml = 'Bidding/Pretrial/BidSituation/TenderSituation/model/tenderHistory.html';//查看历史回执
var fileDownloadUrl = config.tenderHost + '/PretrialFileController/downloadFile.do';//下载解密文件
...
$(function(){
	...
	isShowSupplierInfo(bidId, '1', 'submit', function(data, html){
...
function bidderHtml(data){
	...
	for(var i = 0;i < data.length;i++){
		...
		if(data[i].decryptResult == 1){
			...
			if(isShowBidFile){
				decryptBut = '<a href="javascript:;" style="color: #fff;" class="btn btn-primary btn-sm downFile" data-id="'+data[i].pretrialFileId+'" >下载</a>';
			}
		}else{
...
function getIpList(){
	$.ajax({
		type:"post",
		url:config.tenderHost + "/SupplierSignController/findPitchYuShenIpList",
		async:true,
		data: {
			'bidSectionId': bidId
		},
