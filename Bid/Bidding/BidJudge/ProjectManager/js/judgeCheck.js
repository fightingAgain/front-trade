...
var findBidCheckItemUrl = config.tenderHost + "/BidCheckItemController/findBidCheckItemMark.do";  //  评审项详细内容
...
//	});
	...
	$('#judgesTabs').on("click", "a", function (e) {
	  	e.preventDefault();
	  	$(this).tab('show');
	  	expertId = $(this).attr("data-id");
	  	
	  	if($("#reviewTabs li[class='active']").find("a").attr("href") == "#clarify"){
	  		expertClarifyList();
	  	} else {
	  		findBidCheckItem(checkId);
	  	}
		...
		url: findBidCheckItemUrl,
     	...
     	success: function (data) {
			...
			for(var i = 0; i < reviewItemData.length; i++){
				if(reviewItemData[i].bidCheckItemDtosNum == 0){
...
//	if(checkState == 1 || (reviewItemData[index].isLast == 1 && reviewItemData[index].keepState == 1)){
...
//	}
	...
	if(reviewItemData[index].theKey != undefined){
		$("#theKey").html(reviewItemData[index].theKey == 0 ? "不合格" : "合格");  
		$("#reason").html(reviewItemData[index].reason ? reviewItemData[index].reason : "");
		
		$("#isDiverge").html(reviewItemData[index].theKey == 0 ? "偏离" : "不偏离");  
		$("#divergeReason").html(reviewItemData[index].reason ? reviewItemData[index].reason : "");
		
		$("#isRespond").html(reviewItemData[index].theKey == 0 ? "不响应" : "响应");  
		$("#respondReason").html(reviewItemData[index].reason ? reviewItemData[index].reason : "");
	} else {
		$("#theKey").html("暂未评审");  
	...
	$("#score").html(reviewItemData[index].score ? reviewItemData[index].score : "");
	$("#mark").html(reviewItemData[index].mark ? toDecimal(reviewItemData[index].mark,2) : "");
...
//		$(".pdfView").attr("src", $.parserUrlForToken(top.config.Syshost + "/FileController/download.do?fname=eeeeeee.pdf&ftpPath=" + reviewItemData[index].fileChapterUrl )+"#page=" + reviewItemData[index].fileChapterPage);
	}
}
/**
 * 评审类型
...
//       	var arr = data.data;
 			...
 			reviewItemData[idx].score = score;
     	},
     	error: function (data) {
         	showDialog("加载失败");
     	}
...
function judgesHtml(data){
	...
	for(var i = 0; i < data.length; i++){
		if(i == 0){
			expertId = data[i].expertId
			raterHtml += '<li class="active"><a href="#" data-id="'+data[i].expertId+'">'+data[i].expertName+'</a></li>';
		}else{
