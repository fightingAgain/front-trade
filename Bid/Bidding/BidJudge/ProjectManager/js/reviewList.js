...
*  方法列表及功能描述
*/
var listUrl = config.tenderHost + '/ReviewListController/findPageReviewList.do';  //列表接口
var pageCheck = "Bidding/BidJudge/ProjectManager/model/judgeCheck.html";  //评审查看
...
function openEdit(index){
	...
	layer.open({
		...
		resize: false,
		content: pageCheck +"?id=" + rowData.bidSectionId+'&examType='+rowData.examType,
...
function openSign(index){
	...
	layer.open({
		...
		title: '查看评委签到情况',
		area: ['800px', '600px'],
		content: signHtml+'?id='+rowData.bidSectionId+'&recordid='+rowData.mRecordID+'&examType='+rowData.examType,
		resize: false,
		success:function(layero, index){
			var iframeWin = layero.find('iframe')[0].contentWindow;
		...
		columns: [
			[{
				...
				},
				{
					...
					width: '230px',
					formatter: function(value, row, index){
						var strSee = '<button  type="button" class="btn btn-primary btn-sm btnView" data-index="'+index+'"><span class="glyphicon glyphicon-eye-open"></span>查看评审</button>';
