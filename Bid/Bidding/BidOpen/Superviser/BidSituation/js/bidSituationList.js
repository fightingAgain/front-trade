...
*  方法列表及功能描述
*/
var roomHtml = "Bidding/BidOpen/Superviser/BidSituation/model/bidRoom.html"; // 开标室页面
var pageUrl = config.tenderHost  + "/BidOpeningConfigurationController/pageSuperviseList.do"; 	//分页
var signHtml = "Bidding/BidOpen/Superviser/BidSituation/model/signIn.html"; // 签到页面
var viewHtml = "Bidding/BidOpen/Superviser/BidSituation/model/bidSituationView.html"; //  查看页面
...
$(function(){
	...
	countTimer = setInterval(show_time, 1000);
});
//查看是否签到
	...
	if(data.length > 0){
		top.layer.open({
			...
			success:function(layero, index){
				var iframeWin = layero.find('iframe')[0].contentWindow;
				//调用子窗口方法，传参
				iframeWin.bidFromFathar(data, passMessage);
