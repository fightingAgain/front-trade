 var saveUrl = config.tenderHost + '/BidOpeningOffController/save.do'; // 点击添加项目保存的接口
 //var detailUrl = config.tenderHost + '/BidOpeningOffController/findOfflineBidOpenging.do'; // 开标结果详情
 var detailUrl = config.tenderHost + '/BidOpeningOffController/findOfflineBidOpengingByBidId.do';// 开标结果详情
 var supplierUrl = config.tenderHost + '/BidOpeningOffController/findSupplier.do'; // 开标结果详情
 var customAddUrl = config.tenderHost + '/BidOpeningOffController/saveBidOpeningData.do'; // 添加自定义列
 ...
 var projectEditPage = "Bidding/BidOpen/OfflineResult/model/projectEdit.html"; //编辑开标记录  工程
 var goodsEditPage = "Bidding/BidOpen/OfflineResult/model/goodsEdit.html"; //编辑开标记录  货物
 var serviceEditPage = "Bidding/BidOpen/OfflineResult/model/serviceEdit.html"; //编辑开标记录  服务
...
//			});
	...
	$("#biderBlock").on("click", ".btnDelBider", function(){
		...
		top.layer.confirm('确定要删除该开标记录？', {icon: 3, title:'提示'}, function(idx){
...
function enterpriseHtml() {
 ...
 function saveForm(isSave) {
	...
	para.examType = 2;
 	for(var i = 0; i < biderData.length; i++){
 		if(biderData[i].bidOpeningDetailsId){
 			para.bidOpeningDetailss.push({id:biderData[i].bidOpeningDetailsId});
 		}
 	}
