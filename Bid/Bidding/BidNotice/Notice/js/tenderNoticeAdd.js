...
//var resiveUrl = config.tenderHost + "/NoticeController/getAndFile.do";	//公告详情 
...
var CAcf = null; //实例化CA
var noticeNature;
var bidSectionName = ''; //标段名称

var transformBidder; //保存保证金相关数据
			...
			onpicked: function(dp) {
				...
				if(selectTime && selectTime != '') {
					...
					if(timeArr.length == 0) {
						...
						// $('#noticeEndTime').val(automatic(selectTime, 5, '1')); //招标公告发布截止时间
...
function passMessage(data, callback) {
	...
	//	$('#bidId').val(data.bidSectionId);//标段Id
	$('#interiorBidSectionCode').val(data.interiorBidSectionCode); //标段编号
	/*保存*/
	...
	$('#btnSubmit').click(function() {
		...
		if(checkForm($("#addNotice"))) { //必填验证，在公共文件unit中
			...
			if(docGetStartTime < noticeSendTime || docGetStartTime > noticeEndTime || docGetStartTime == noticeEndTime) {
				parent.layer.alert('招标文件获取时间应大于等于公告发布时间之后，小于公告截止时间！', function(ind) {
					parent.layer.close(ind);
					//				$('.collapse').collapse('hide');
					$('#collapseFive').collapse('show');
				});
			...
			if(verifyTime) {
				if(isLaw == 1) {
					if(differTime < times) {
						parent.layer.alert('依法招标的项目，公告截止时间与开始时间相差的天数要至少5天', function(ind) {
							parent.layer.close(ind);
							$('#collapseFive').collapse('show');
						});
						return
					...
					if(time2 < times20) {
						parent.layer.alert('依法招标的项目，开标时间与公告开始时间相差的天数要大于等于20天', function(ind) {
							parent.layer.close(ind);
							$('#collapseFive').collapse('show');
						});
						return
			...
			if(!mediaEditor.isValidate()) {
				...
				/*var secret = hex_md5(editor.txt.text());
				var sign = signName(secret);
				if(!sign){
					return
				}
				if(sign.caDn != ''){
					$('#caConfirmCode').val(sign.caConfirmCode);
					$('#caCode').val(sign.caCode);
					$('#caDn').val(sign.caDn);*/
				// 检测保证金转移
				let isalert = false;
				var text = '';
					...
					parent.layer.confirm(text, {
						...
						if($("#addChecker").length <= 0) {
							...
							parent.layer.alert('确认提交审核？', function(index) {
								...
								var festival = ["2019-09-13","2019-09-14","2019-09-15","2019-10-01","2019-10-02","2019-10-03","2019-10-04","2019-10-05","2019-10-06","2019-10-07"];
								var  openTime = $('#bidOpenTime').val().split(' ')[0];
								var  startTime = $('#noticeSendTime').val().split(' ')[0];
								var timeArr = getAll(startTime, openTime).split(',');
								for(var i=0;i<festival.length;i++){
									for(var j = 0;j<timeArr.length;j++){
										if(timeArr[j] == festival[i]){
											isFestival = true
										}
									}
								}
								if(isFestival){
									parent.layer.confirm('您所选的时间包含节假日，是否继续？',{title:'询问'},function(ind){
										save(1);
										parent.layer.close(ind);
									})
								}*/
 ...
 * callback：点击上传文件时调用
 */
function save(state, isBtn, callback) {
	var bidCodes = [];
	...
	if(state == 1) {
		if(!CAcf) {
			CAcf = new CA({
				...
				confirmCA: function(flag) {
					...
					saveForm(param, state, isBtn, callback)
				}
			});
		}
		CAcf.sign();
		...
		// 变更编辑保存公告发布时间&招标文件获取开始时间时间不能编辑
		if (changeCount > 0) {
			$("#noticeSendTime,#docGetStartTime").attr("disabled","disabled");
		}
		saveForm(param, state, isBtn, callback)
	}
...
function saveForm(param, state, isBtn, callback) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			if(data.success) {
				...
				//                  parent.$('#tenderNoticeList').bootstrapTable('refresh');
			} else {
				parent.layer.alert(data.message);
				if(noticeNature && noticeNature == 2) {
					// processTime(serialize($('#addNotice').serializeArray()))
				}
				//             		reviseNotice(id);
			}
	...
	$.ajax({
		...
		success: function(data) {
			if(data.success) {
				...
				$('#interiorBidSectionCode').val(arr.interiorBidSectionCode); //标段编号
				$('#bidSectionName').val(arr.bidSectionName); //标段名称
				...
				for(var key in arr) {
					...
					if(key == "tenderProjectType") {
						...
						//						console.log(id)
						/*if(!id){
							$('#noticeName').val(arr.bidSectionName +'-招标公告')
						}*/
					}
				}
			}
...
function processTime(timeData) {
	...
	var signStartDate = timeData.signStartDate ? Date.parse(new Date(timeData.signStartDate.replace(/\-/g, "/"))) : 0;
	var signEndDate = timeData.signEndDate ? Date.parse(new Date(timeData.signEndDate.replace(/\-/g, "/"))) : 0;
