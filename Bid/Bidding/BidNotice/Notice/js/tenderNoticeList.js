...
var changeUrl = "Bidding/BidNotice/Notice/model/tenderNoticeChange.html"; // 新增
...
var delUrl = config.tenderHost + "/NoticeController/deleteNoticeByBidId.do";	//删除
var backUrl = config.tenderHost + "/NoticeController/revokeNoticeByBidId.do";	//撤回
...
$(function () {
	...
	//编辑
	$("#tenderNoticeList").on("click", ".btnEdit", function () {
		var index = $(this).attr("data-index");
		var rowData = $('#tenderNoticeList').bootstrapTable('getData')[index]; //bootstrap获取当前页的数据
		openEdit(rowData);
	});
	//变更
	$("#tenderNoticeList").on("click", ".btnChange", function () {
		var index = $(this).attr("data-index");
		var rowData = $('#tenderNoticeList').bootstrapTable('getData')[index];
		if (rowData.openingState == 1) {//是否开标状态   是否已经开标     当前时间大于开标开始时间为1  小于开标开始时间为0
			layer.alert('该公告开标时间已过，不能变更！', { icon: 7, title: '提示' })
		} else {
			layer.confirm('确定变更?', { icon: 3, title: '询问' }, function (ind) {
				...
				$.ajax({
					...
					data: { noticeId: rowData.noticeId },
	...
	//删除
	$("#tenderNoticeList").on("click", ".btnDel", function () {
		var rowData = $('#tenderNoticeList').bootstrapTable('getData'); //bootstrap获取当前页的数据
		var index = $(this).attr("data-index");
		layer.confirm('确定删除?', { icon: 3, title: '询问' }, function (ind) {
			layer.close(ind);
			requst(delUrl, rowData[index].bidSectionId, 1);
		});
	});
	//取消变更
	$("#tenderNoticeList").on("click", ".btnCancel", function () {
		var rowData = $('#tenderNoticeList').bootstrapTable('getData'); //bootstrap获取当前页的数据
		var index = $(this).attr("data-index");
		layer.confirm('确定取消变更?', { icon: 3, title: '询问' }, function (ind) {
			layer.close(ind);
			requst(delUrl, rowData[index].bidSectionId, 2);
		});
	});
	//撤回
	$("#tenderNoticeList").on("click", ".btnBack", function () {
		var rowData = $('#tenderNoticeList').bootstrapTable('getData'); //bootstrap获取当前页的数据
		var index = $(this).attr("data-index");
		...
		layer.confirm('确定撤回?', { icon: 3, title: '询问' }, function (ind) {
			layer.close(ind);
			requst(backUrl, rowData[index].bidSectionId, 3)
		});
		//		}
	});
	/*查询*/
	...
	//发布
	$('#tenderNoticeList').on('click', '.btnSend', function () {
		var rowData = $('#tenderNoticeList').bootstrapTable('getData'); //bootstrap获取当前页的数据
		var index = $(this).attr("data-index");
...
function sendNotice(bid, state) {
	...
	$.ajax({
		...
		success: function (data) {
			if (data.success) {
				if (data.data == '1') {
					layer.confirm('公告开始时间小于当前时间，若要继续发布，请点确定?', { icon: 3, title: '询问' }, function (ind) {
						sendNotice(bid, 1)
						layer.close(ind);
					});
				} else if (data.data == '2') {
					...
					layer.confirm('公告结束时间小于当前时间，若要继续发布，请点确定?', { icon: 3, title: '询问' }, function (ind) {
						sendNotice(bid, 1)
						layer.close(ind);
					});
				}
...
function openEdit(data) {
	...
	/*if(data.noticeId){
		if(data.noticeNature == 2){
			url = changeUrl+'?bidSectionId='+data.bidSectionId + "&id=" + data.id + "&noticeId=" + data.noticeId;
			title = "变更公告";
		} else {
			url = editUrl+'?bidSectionId='+data.bidSectionId + "&id=" + data.id + "&noticeId=" + data.noticeId;
			title = "修改公告";
		}
	}else{
		url = editUrl+'?bidSectionId='+data.bidSectionId + '&isPublicProject=' + data.isPublicProject,
		title = "添加公告"
	}*/
	if (data.noticeId) {
		if (data.noticeNature == 2) {
			url = changeUrl + '?id=' + data.bidSectionId + '&special=CAIDAN';
			title = "变更公告";
		} else {
			url = editUrl + '?id=' + data.bidSectionId;
			title = "修改公告";
		}
	} else {
		url = editUrl + '?id=' + data.bidSectionId,
...
function openView(index) {
	...
	layer.open({
		...
		title: "查看公告",
		area: ['100%', '100%'],
		content: viewUrl + "?id=" + rowData[index].bidSectionId + "&noticeId=" + rowData[index].noticeId + "&isThrough=" + (rowData[index].noticeState == 2 ? 1 : 0) + '&htmlTypes=view',
		resize: false
	});
}
/**
					...
					if(row.owner && row.owner == 1){//权限  是否有操作权限1是  0否
						...
						if (row.noticeState == -1) {
							...
							if (row.publishState == 1) {//已发布
								if (topTime > Date.parse(new Date(row.noticeSendTime.replace(/\-/g, "/")))) {//发布时间已到
