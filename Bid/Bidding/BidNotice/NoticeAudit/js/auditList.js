...
*  方法列表及功能描述
*/
var auditHtml = "Bidding/BidNotice/NoticeAudit/model/audit.html"; // 审核
var viewHtml = "Bidding/BidNotice/NoticeAudit/model/auditView.html"; // 查看
var pageUrl = config.tenderHost  + "/NoticeController/pageCheckList.do"; 	//分页
//入口函数 
$(function(){
	//加载列表
	initJudgeTable();
...
function openView(index){
	...
	layer.open({
		...
		title: "查看公告",
		area: ['80%', '80%'],
		content: viewHtml + "?id=" + rowData[index].id+"&noticeId=" + rowData[index].noticeId,
		resize: false,
...
function openAudit(index,source){
	...
	layer.open({
		...
		title: "审核公告",
		area: ['80%', '80%'],
		content: auditHtml + "?id=" + rowData[index].id+"&noticeId=" + rowData[index].noticeId + '&source=' + source,
		resize: false,
		success: function(layero, index){
