...
$(function(){
	...
	$('body').on('click','.relevantBid',function(){
		var surl = bidHtml + '?id=' + bidId + '&examType=2' + '&isAgency=' + employeeInfo.isAgency + '&isBlank=1'
...
//					iframeWin.bidDetail(bidDetail);
	...
	function auditList(data){
		...
		for(var i = 0;i<data.length;i++){
			...
			+'<td style="width: 100px;text-align:center">'+data[i].checkState+'</td>'
			+'<td style="width: 200px;text-align:center">'+data[i].userName+'</td>'
...
//							$('.signUp .time').removeAttr('datatype');
						}else if(arr.signUp == 1){	
							$('.signUp').css({'display':'table-row'});
//							$('.signUp .time').attr('datatype','*');
						}
					}
