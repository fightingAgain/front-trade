
var saveUrl = config.tenderHost + '/ClarifyController/saveAndUpdate.do';//保存

var bidHtml = 'Bidding/Clarify/manager/model/bidList.html';//标段列表

var htmlToPdf = config.tenderHost + '/htmlToPdf.do';  //招标文件详情
var signView = '../../../Model/signView.html';  //签章页面

var fileUploads = null;
var clarifyId = '';
var bidId = '';//标段id
var loginInfo = entryInfo();//登录人信息
var CAcf = null;  //实例化CA

var isBeAsk = '';//是否针对提问回复
var askEmployeeId = '';//针对提问回复时的提问人id
var answerId = '';//针对提问回复时的提问id
var clarifyData = {};//针对提问回复时的回复数据
$(function(){
	if($.getUrlParam('isChoose') && $.getUrlParam('isChoose') != undefined){
		$('#chooseBid').show();
		$('[name=clarifyType]').removeAttr('disabled');
	}else{
		$('#chooseBid').hide();
		$('[name=clarifyType]').attr('disabled',true);
	}
	/*关闭*/
	$('#btnClose').click(function(){
		var index=parent.layer.getFrameIndex(window.name);
        parent.layer.close(index);
	});
	/*
	 *上传文件
	 * */
	$('#fileUp').click(function(){
//		var obj = $(this);
		if(clarifyId == '' || !clarifyId){
			save(false,false,function(businessId){
				clarifyId = businessId;
				//上传文件
				if(!fileUploads){
					fileUploads = new StreamUpload("#fileContent",{
						basePath:"/"+loginInfo.enterpriseId+"/"+bidId+"/"+clarifyId+"/604",
						businessId: clarifyId,
						status:1,
						businessTableName:'T_CLARIFY',  
						attachmentSetCode:'ASK_FILE'
					});
				}
				$('#fileLoad').trigger('click');
			});
		}else{
			//上传文件
			if(!fileUploads){
				fileUploads = new StreamUpload("#fileContent",{
					basePath:"/"+loginInfo.enterpriseId+"/"+bidId+"/"+clarifyId+"/604",
					businessId: clarifyId,
					status:1,
					businessTableName:'T_CLARIFY',  
					attachmentSetCode:'ASK_FILE'
				});
			}
			$('#fileLoad').trigger('click');
		}
		
	});
	//选择标段
	$('#chooseBid').click(function(){
		var askType = $('[name=clarifyType]').val();
		getBid(askType)
	});
	//切换澄清类型
	$('[name=clarifyType]').change(function(){
		bidId = '';
		$('.changeEmpty').val('');
		$('.changeEmpty').html('');
	});
	
	$("#btnModel").click(function(){
		modelCreat({
			'bidSectionId':bidId,
			'tempType':'clearlyApply',
			'examType':2
		});
			
	});
})
//选择标段
function getBid(type){
	parent.layer.open({
		type: 2,
		title: '选择标段',
		area: ['1000px', '650px'],
		content: bidHtml+'?type='+type,
		resize: false,
		success:function(layero, index){
			var iframeWin = layero.find('iframe')[0].contentWindow;
			//调用子窗口方法，传参
			iframeWin.bidFromFathar(formFather);
		}
	})
}
function formFather(data){
	bidId = data.bidSectionId;
	for(var key in data){
		$('[name='+key + ']').val(data[key]);
		$('#'+key).html(data[key]);
		
	}
}
function passMessage(data,type,callback){
	if($.getUrlParam('isBeAsk') && $.getUrlParam('isBeAsk') != undefined){
		isBeAsk = $.getUrlParam('isBeAsk');
		$('.isBeAsk').css('display','none');
	}else{
		$('.isBeAsk').css('display','table-row');
	}
	if(data){
		bidId = data.bidSectionId;
		
		if(data.bidStage == 1){
			$('[name=clarifyType]').val('1')
		}else if(data.bidStage == 2){
			$('[name=clarifyType]').val('4')
		}
		if(isBeAsk == 1){
			askEmployeeId = data.askEmployeeId;
			answerId = data.id;
			$('[name=bidSectionId]').val(data.bidSectionId);
			$('[name=answersEndDate]').val(data.answersEndDate);
			if(data.clarifyId){
				clarifyId = data.clarifyId;
				clarifyData = data.clarify;
				for(var key in clarifyData){
					$('[name='+key + ']').val(clarifyData[key]);
					$('#'+key).html(clarifyData[key]);
				}
			}else{
				//“澄清标题”默认与当前要回复的问题标题相同，可修改；
				$('[name=clarifyTitle]').val(data.askTitle);
			}
		}else{
			for(var key in data){
				$('[name='+key + ']').val(data[key]);
				$('#'+key).html(data[key]);
			}
			if(type == 'edit'){
				if(data.id){
					clarifyId = data.id;
				}
			}
		}
		if(clarifyId && clarifyId != ''){
			if(!fileUploads){
				fileUploads = new StreamUpload("#fileContent",{
					basePath:"/"+loginInfo.enterpriseId+"/"+bidId+"/"+clarifyId+"/604",
					businessId: clarifyId,
					status:1,
					businessTableName:'T_CLARIFY',  
					attachmentSetCode:'ASK_FILE'
				});
			}
		}
		
		if(isBeAsk == 1){
			if(clarifyData.projectAttachmentFiles){
				var projectAttachmentFiles = clarifyData.projectAttachmentFiles;
				fileUploads.fileHtml(projectAttachmentFiles);
			}
		}else{
			if(data.projectAttachmentFiles){
				var projectAttachmentFiles = data.projectAttachmentFiles;
				fileUploads.fileHtml(projectAttachmentFiles);
			}	
		}
	}
	
	//保存
	$('#btnSave').click(function(){
		save(false,true,callback)
	})
	//发送
	$('#btnSubmit').click(function(){
		if(!bidId || bidId == ''){
			parent.layer.alert('请选择标段!');
			return
		}
		if(checkForm($("#formEle"))){//必填验证，在公共文件util中
			parent.layer.confirm('确定发送?', {title:'询问'}, function(index){
  				save(true,false,callback)
			  	parent.layer.close(index);
			})
		}
	})
}
/*
 * 保存
 * isSend true 发送  false保存
 * isTips 保存时是否提示 
 */
function save(isSend,isTips,callback){
	$('[name=clarifyType]').removeAttr('disabled');
	if(isSend){
		if(!CAcf){
			CAcf = new CA({
				target:"#formEle",
				confirmCA:function(flag){ 
					if(!flag){  
						return;
					}
					arr = parent.serializeArrayToJson($('#formEle').serializeArray());
					arr.state = 1;
					if(clarifyId && clarifyId != ''){
						arr.id = clarifyId;
					}
					savePost(arr, isSend,isTips,callback);
				}
			});
		}
		CAcf.sign();
	} else {
		arr = parent.serializeArrayToJson($('#formEle').serializeArray());
		arr.state = 0;
		if(clarifyId && clarifyId != ''){
			arr.id = clarifyId;
		}
		savePost(arr, isSend,isTips,callback);
	}
	
}
function savePost(arr, isSend,isTips,callback){
	console.log(askEmployeeId)
	if(isBeAsk == 1){
		arr.askEmployeeId = askEmployeeId;
		arr.answerId = answerId;
	}
	$('#btnSave,#btnSubmit').attr('disabled', true);
	$.ajax({
		type:"post",
		url:saveUrl,
		async:false,
		data: arr,
		success: function(data){
			$('#btnSave,#btnSubmit').attr('disabled', false);
			if(data.success){
				clarifyId = data.data;
				if(isSend){
					parent.layer.alert('发送成功！',{icon:6,title:'提示'});
					var index=parent.layer.getFrameIndex(window.name);
       				parent.layer.close(index);
				}else{
					if(isTips){
						parent.layer.alert('保存成功！',{icon:6,title:'提示'});
						if($.getUrlParam('isChoose') && $.getUrlParam('isChoose') != undefined){
							$('#chooseBid').show();
							$('[name=clarifyType]').removeAttr('disabled');
						}else{
							$('#chooseBid').hide();
							$('[name=clarifyType]').attr('disabled',true);
						}
					}
				}
				if(callback){
					callback(data.data,bidId)
				}
				parent.$('#tableList').bootstrapTable('refresh');
			}else{
				parent.layer.alert(data.message);
			}
		},
		error: function(data) {
		 $('#btnSave,#btnSubmit').attr('disabled', false);
		 parent.layer.alert("提交失败！");
		}
	});
}
