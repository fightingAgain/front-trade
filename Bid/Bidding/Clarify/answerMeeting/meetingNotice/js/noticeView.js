...
var listUrl = config.tenderHost + '/AnswerMeetingEnterpriseController/findChoosePageList.do';//项目经理在详情页面查看投标人接收/拒绝的分页数据，按投标人名称倒叙排列
	...
	$.ajax({
		...
		success: function(data){
			if(data.success){
				var arr = data.data;
				noticeId = arr.id;
				isShowSupplierInfo(id, '2', 'answer', function(data, html){
...
function getQueryParams(params) {
	var projectData = {
		...
		noticeId: noticeId, // 通知公告id
		bidderName: $("#bidderName").val(), // 投标人名称
		answerState: $("#answerState").val() // 确认状态   接受/拒绝 0为未确认 1为放弃 2为同意
	};
	return projectData;
};
//表格初始化
