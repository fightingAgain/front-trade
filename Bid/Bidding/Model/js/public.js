var entryUrl = top.config.tenderHost + '/getEmployeeInfo.do';  //获取登录人信息
var optionUrl = top.config.Syshost + '/SysDictController/findOptionsByName.do';	// 获取下拉框
var optionValueUrl = top.config.Syshost + '/SysDictController/getValue.do';	// 获取指定的值

var modelOptionUrl = top.config.tenderHost + '/TempBidWebController/findTempBidWebList.do';//模板下拉框
var modelUrl =top.config.tenderHost + '/TempBidWebController/toTemplat.do';//模板地址

var findProjectDetailUrl = top.config.tenderHost + '/TenderProjectController/findTenderProjectType.do';//标段信息

var urlCheckBlackList = top.config.HghHost + '/blackListController/checkBlackList';//黑名单接口
var getBidUrl = top.config.tenderHost + '/BidSectionController/get.do'; // 标段详情
var testHolidayUrl = top.config.Syshost + '/HolidaySettingController/isRestTime.do';//查询是否是休息日期
...
if(!window.top.options){
	window.top.options = {
		...
		//标室所在区域
		areaCode:{val:{"4206":"襄阳市", "4203":"十堰市", "4201":"武汉市", "6101":"西安市", "1101":"北京市", "1306":"保定市", "4401":"广州市", "5001":"重庆市", "3301":"杭州市", "4101":"郑州市", "5101":"成都市"}, isCustom:true},
		...
		//招标项目分类
		tenderProjectClassify:{optionName:"SYS_PROJECT_CLASSIFY"},
		//标段分类
		...
		//异议阶段
		objectionType:{val:{"1":"资格预审文件","4":"招标文件","5":"开标","7":"中标候选人公示", "9": "项目异常公示"}, isCustom:true},
		//项目类别
		...
		//广联达清标接口编码
		interfaceCode: {optionName:'INTERFACE_CODE'},
...
var reviewTypeData = {
	...
	gdsp: '项目资料归档审批',
	qlcxxhsgg: '全流程线下招标公告',
	qlcxxysgg: '全流程线下预审招标公告',
	qlcxxkblr: '全流程线下开标录入',
	qlcxxpbjglr: '全流程线下评标结果录入',
	qlcxxzbxm: '全流程线下招标项目录入',
	qlcxxzbyq: '全流程线下邀请函',
	qlcxxzbwjlr: '全流程线下招标文件',
	qlcxxzbjgtz: '全流程线下确认中标人',
	qlcxxzgyswj: '全流程线下预审招标文件',
	qlcxxzgsckq: '全流程线下预审开启审查文件',
	qlcxxzgjglr: '全流程线下预审结果录入',
	qlcxxzgjgtz: '全流程线下预审审查通知',
	caxgsp:'CA订单（新购）',
	cabgsp:'CA订单（变更）',
	cayzcjsp:'CA订单（印章图片采集）',
	qyxxsp:'企业信息',
	rgcqsp: '组建评委会(人工抽取)审批'
}
 ...
 * defaultTime 1默认当天最晚时间 即23:59  2 默认次日最早时间 即09:30
 */
function automatic(time, num, defaultTime){
	time.replace(/\-/g,"/");
	var startData =new Date(time);
	
	if(defaultTime == '1'){
		startData.setDate(startData.getDate()+num);
		return startData.Format("yyyy-MM-dd hh:mm").split(' ')[0] + ' 23:59';
	}else if(defaultTime == '2'){
		startData.setDate(startData.getDate() + num + 1);
		return startData.Format("yyyy-MM-dd hh:mm").split(' ')[0] + ' 09:30';
	}else{
		startData.setDate(startData.getDate()+num);
...
function initMedia(para){
	...
	var html = "";
	if(options.check.length == 0 && options.isChecked){
...
function showTip(link,data){
	/**
	 link:              tip:
	a)报名：				“贵公司被汽车集团（或企业名称）列入黑名单，禁止业务领域是xxx，禁入有效期为xxx，不能参加该项目投标”
	b)邀请：				“该投标人被汽车集团（或企业名称）列入黑名单，禁止业务领域是xxx，禁入有效期为xxx，不能参加该项目投标”
	b1)全选邀请：		“xxx被汽车集团（或企业名称）列入黑名单，禁止业务领域是xxx，禁入有效期为xxx，不能参加该项目投标”
	c)被邀请：			“贵公司被汽车集团（或企业名称）列入黑名单，禁止业务领域是xxx，禁入有效期为xxx，不能参加该项目投标”
	d)下载\购买招标文件：“贵公司被汽车集团（或企业名称）列入黑名单，禁止业务领域是xxx，禁入有效期为xxx，不能参加该项目投标”
	e)递交投标文件：		“贵公司被汽车集团（或企业名称）列入黑名单，禁止业务领域是xxx，禁入有效期为xxx，不能参加该项目投标”
	f)开标：				“贵公司被汽车集团（或企业名称）列入黑名单，禁止业务领域是xxx，禁入有效期为xxx，不能参加该项目投标”
	g)确定中标人：		“该投标人被汽车集团（或企业名称）列入黑名单，禁止业务领域是xxx，禁入有效期为xxx，不能被确定为中标人”
	h)中标结果通知书：	“该投标人被汽车集团（或企业名称）列入黑名单，禁止业务领域是xxx，禁入有效期为xxx，无法发送中标通知书”
	 */
...
function checkBlackList(enterpriseCode,projectType,link) {
	...
	/**
	BLACL_INFO_PLATFORM_CODE_DF("00270007","业务咨询管理系统"),
...
//获取标段信息
function getBidSectionDetail(id){
	var rst;
	$.ajax({
		type:"post",
		url:getBidUrl,
...
/* 撤回招标项目或标段时验证是否有招标（预审）文件 */
function getHasFile(type, id, callback, tips){
	var postUrl = '',postData={},msg='';
	if(type == 'project'){
		postUrl = top.config.tenderHost + '/DocClarifyController/getBidFileStatesByTenderProjectId.do';
		if(id != ''){
			postData = {'tenderProjectId': id};
		}
		
	}else if(type == 'bid'){
		postUrl = top.config.tenderHost + '/DocClarifyController/getBidFileStatesBySectionId.do';
		if(id != ''){
			postData = {'sectionId': id};
		}
	} else if (type == 'prebid') {
		postUrl = top.config.tenderHost + '/DocClarifyController/getBidFileStatesBySectionIdNew.do';
		if(id != ''){
			postData = {'sectionId': id};
		}
	}
	$.ajax({
		...
		success: function(data) {
			...
			if(data.data == '1'){
				if(tips){
					top.layer.alert(tips,{end: function(){
						callback(id, true);
					}})
				}else{
					callback(id, true);
...
/* 日期落在节假日（法定节假日、非调休上班的双休日）时，时间标红且显示备注文字“请按需延长公示期，使截止日期为工作日”，仅提示不作校验； */
function dateInHolidayTip(selectTime, ele){
	$.ajax({
		type: "post",
		url: testHolidayUrl,
		async: true,
		data: {'date': selectTime},
		success: function (data) {
			if (data.success) {
				$(ele).closest('td').find('.red').remove();
				if(data.data){
					$(ele).css('color','#BB2413').after('<span class="red holidayTips">该时间非工作日，请按需调整</span>');
				}else{
					$(ele).css('color','#555');
				}
			} else {
...
/* ***************                  环境检测工具下载地址配置                   *************** */
$('#environmentCheckDownload').attr('href', top.environmentCheckDownloadUrl)

/* ****************************                 隐藏投标人信息                   *********************** */
//id标段id   exam资格审查方式（ 0为资格预审，1为资格后审） tenderType 采购方式
	//stage阶段 （buy-购买、submit-递交、review-评审、deposit-保证金、reconnaissance-踏勘、answer-答疑）
function isShowSupplierInfo(id, exam, stage, callback){
	var postData = {};
	if(id != ''){
		 postData.bidSectionId = id;
	};
	if(exam != '' || exam == 0){
		 postData.examType = exam;
	};
	if(stage != ''){
	...
	$.ajax({
		...
		data: postData,
	   	...
	   	success: function(data){
	   		if(data.success){
				var html = '<div class="red" style="text-align: center;padding: 30px 0px;">';
				if(data.data.isShowSupplier != 1 && data.data.isShowBidFile !=1){
					if(data.data.buyFileText){
						if(exam == 2){
							html += '当前已购买招标文件的投标人数量为"'+data.data.buyFileText+'"'
						}else if(exam == 1){
							html += '当前已购买预审文件的投标人数量为"'+data.data.buyFileText+'"'
						}
					}
					if(data.data.downloadFileText){
						if(exam == 2){
							html += '当前已下载招标文件的投标人数量为"'+data.data.downloadFileText+'"'
						}else if(exam == 1){
							html += '当前已下载预审文件的投标人数量为"'+data.data.downloadFileText+'"'
						}
							
					}
					if(data.data.submitFileText){
						if(exam == 1 ){
							html += '当前已递交资格申请文件的投标人数量为"'+data.data.submitFileText+'"'
						}else if(exam == 2){
							html += '当前已递交投标文件的投标人数量为"'+data.data.submitFileText+'"'
						}
					}
					if(data.data.depositText){
						html += '当前已缴纳投标保证金的投标人数量为"'+data.data.depositText+'"'
					}
					if(data.data.reconnaissanceText && !$.isEmptyObject(data.data.reconnaissanceText)){
						$.each(data.data.reconnaissanceText, function(key, value){
							if(key == '0'){
								html += '当前{未确认}的投标人数量为"'+value+'"'
							}else if(key == '2'){
								html += '当前{已同意}的投标人数量为"'+value+'"'
							}else if(key == '1'){
								html += '当前{已放弃}的投标人数量为"'+value+'"'
							}
						})
					}
					if(data.data.answerText && !$.isEmptyObject(data.data.answerText)){
						$.each(data.data.answerText, function(key, value){
							if(key == '0'){
								html += '当前{未查看}的投标人数量为"'+value+'"'
							}else if(key == '2'){
								html += '当前{已查看}的投标人数量为"'+value+'"'
							}
						})
					}
				}
				html += '</div>'
	   			callback(data.data, html);
	   		}else{
				top.layer.alert(data.message)
			}
	   	},
...
/* ****************************                 隐藏投标人信息   -end                *********************** */
/* ****************************                 投标人更名提醒                   *********************** */
//id  标段id   bidderId投标人id    投标人名称
//获取投标人更名信息
function getBidderRenameMark(id){
	var bidderObject = {};
	$.ajax({
		url:  top.config.tenderHost + '/EnterpriseRecordController/findListByBidSectionId.do',
		type: "post",
		async: false,
		data: {bidSectionId: id},
		success: function(data) {
...
//展示投标人更名信息
function showBidderRenameMark(bidderId, name, data, ele){
	var bidderName = name?name:'';
	var html = '', parentHtml = '';
	var hasParent = $('#renameListWrap').length > 0?true:false;
	if(!$.isEmptyObject(data) && data[bidderId] && data[bidderId].length > 1){
		bidderName = '<div id="" style="position: relative;padding-right:65px;display: inline-block;">' + name
			bidderName += '<button type="button" class="btn btn-xs btn-primary btnRenameMore" data-id="'+bidderId+'" style="position: absolute;right: -10px;top: -5px;transform: scale(0.8);">更名记录</button>'
		bidderName += '</div>'
		parentHtml += hasParent?'':'<div id="renameListWrap"></div>'
		html += '<div class="renameList bidderRenamelist_'+bidderId+'" style="display: none;">'
			// html += '<div style="text-align: center;font-weight: 600;padding: 5px;">本项目使用名称</div>'
			html += '<div style="padding: 10px;">'
			html += '<table class="table table-bordered">'
				html += '<tr><th>企业名称</th><th style="text-align: center;width: 150px;">生效时间</th></tr>'
				for(var i=0;i<data[bidderId].length;i++){
					html += '<tr><td>'+(data[bidderId][i].enterpriseName?data[bidderId][i].enterpriseName:'')+'</td><td style="text-align: center;width: 150px;">'+(data[bidderId][i].createTime?data[bidderId][i].createTime:'')+'</td></tr>'
				}
				
			html += '</table>'
		html += '</div></div>'
		if(!hasParent){
			$(parentHtml).appendTo($('#'+ele))
		}
		$(html).appendTo($('#renameListWrap'))
	}
	return bidderName
}
$('body').on('click', '.btnRenameMore', function(e){
	e.stopPropagation();
	var id = $(this).attr('data-id');
	// $('.bidderRenamelist_'+id).siblings().hide();
	// $('.bidderRenamelist_'+id).toggle();
	top.layer.open({
		type: 1,
		title: "本项目使用名称",
		area: '500px',
		resize: false,
		content:$('.bidderRenamelist_' + id).html(),
...
/* ****************************                 存储预警信息                  *********************** */
//开标存储预警信息
function saveWarningInfo(packageId, examType){
	$.ajax({
		type: 'post',
		url: top.config.Reviewhost + '/WarningInfoController/saveWarningInfo.do',
		data: { 
			'packageId': packageId,
			'examType': examType
		},
