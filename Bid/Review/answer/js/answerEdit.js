...
var modelCreatUrl =config.tenderHost + '/TempBidWebController/toIsPublicTemplat.do';//模板地址
...
var isPublicProject = $.getUrlParam("isPublicProject");
var ue;
...
function getDetail() {
	$.ajax({
		...
		success: function(data) {
			...
			var arr = data.data;
			if(isPublicProject == 1){
				$("#askContent").html('<button type="button" data-url="'+arr.askPdfUrl+'" class="btn btn-primary btn-sm btn_view">预览</button>')
			}else{
...

			{
				...
				events:{
					'click .btnDownload':function(e,value, row, index){		
						...
						name.splice(leng,1);
						fileName = name.join(' ');
						var newUrl =$.parserUrlForToken(dowoloadFileUrl) + '&ftpPath=' + row.url + '&fname=' + encodeURIComponent(row.attachmentFileName);
						window.location.href = newUrl;
					},
					'click .btnPreview':function(e,value, row, index){
						parent.previewPdf(row.url);
					},
				},
				formatter: function(value, row, index) {
...
$("#btnSubmit").click(function() {
	var answersContent = '';
	if(isPublicProject == 1){
		answersContent= ue.getContent();
	}else{
		answersContent= $('#answers_Content').val();
	}
	
//	saveList.answerContent = answersContent;
	if(!answersContent){
		top.layer.alert("温馨提示：请填写回复内容",{icon:3,title:"提示"});
		return;
	}	
...
/********************************************        进场回复       ************************************************/
/******    预览提问内容    *******/
$('#askContent').on('click','.btn_view',function(){
	var pdf = $(this).attr('data-url');
	parent.previewPdf(pdf);
})
//生成模版进场
function modelCreat(para){
	$.ajax({
		type:"post",
		url:modelCreatUrl,
		async:true,
