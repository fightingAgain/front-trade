var experts, judgesType;
function du(expertsList,bidSectionId,examType,expertType){
    judgesType = expertType;
    experts = {
        'bidSectionId':bidSectionId,
        'examType':examType,
        'extracts':expertsList
    }
}
...
$("#btnSubmit").on('click',function(){
    ...
    var result = idCardValid[idCardType](idCard);

    if(result.status == 0){
        var that = $("#idCard");
        window.top.layer.alert('温馨提示：'+result.msg, function(index){
            top.layer.close(index);
            if($(".panel-collapse").length > 0){
                that.closest('.panel-collapse').collapse('show');
                that.closest('.collapse').siblings().find(".panel-collapse").collapse("hide");
            }
            that.focus();
        });
        return;
    ...
    var expert = {
        ...
        "idCard":idCard,
        "isChairMan": 0,
        "expertType": judgesType,
        "phoneNumber": $("#phoneNumber").val(),
        "duty": $("#duty").val(),
        "professionalName": $("#professionalName").val(),
    }
    experts.extracts.push(expert);
	if(checkForm($("#editExpert"))){
		$.ajax({
	        type:"post",
