...
var bidsectionUrl=config.Reviewhost+"/ProjectController/findBidSectionInfo";
...
function getBidsectionInfo(bidSectionId){
	var bidSectionInfo;
    //获取标段详情
    $.ajax({
        type:"post",
...
function keyTest(ele,type,maxLength,tips){
	...
	if(ele.value != "") {
		if(regu.test(ele.value)) {
			...
			if(tips){
                top.layer.alert('温馨提示：'+tips,function(ind){
                    top.layer.close(ind);
					ele.focus();
					ele.value = '';
				});
			}else{
                top.layer.alert("温馨提示：请输入正确的值",function(ind){
                    top.layer.close(ind);
					ele.focus();
					ele.value = '';
				});
...
//			ele.value = ele.value.substring(0, maxLength - 1);
			$(ele).addClass('red');
			top.layer.alert('温馨提示：输入长度超过限制长度'+maxLength+'位');
			if(isEmpty){
				ele.value = '';
			}
		}else{
...
//	console.log(noticeMedia);
//	var options = {
//		target:".media-box",
//		isPublicProject:1,  //是否公告资源
//		isLaw:1,  //是否依法
//		tenderMode:1,
//		isDisabled:false
//	}
...
//		}
	...
	for(var key in noticeMedia){
		...
		if(options.proType.isLaw == 1 && !options.isDisabled){
			if(options.proType.tenderMode == 1 && key == "000"){
				isDisabled = 'disabled="disabled"';
			}
			if(options.proType.tenderMode == 1 && key == "001"){
				isDisabled = 'disabled="disabled"';
			}
		}
...
//获取投标人更名信息
function getBidderRenameMark(id, token){
	var bidderObject = {};
	$.ajax({
...
//     if (!clickTarget.is('.renameList') && !clickTarget.closest('.renameList').length && !clickTarget.is('.btnRenameMore')) {
//         // 点击在#yourElement之外
//         $('.renameList').hide()
//     }
// });
