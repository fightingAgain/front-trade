...
function bidFileList(){
	$('#bidFile').bootstrapTable({
		...
		},
		{
			...
			halign: "left",	
			formatter: function(value, row, index) {
				if(row.bidFileType == 'ZW'){
					return row.bidderName + '投标文件正文'
				}else{
					return value
				}
			},
		...
		},
		{
			...
			events:{
				'click #download':function(e,value, row, index){
					if(row.bidFileType == 'ZW'){
						var filesnamel = row.bidderName + '投标文件正文'
					}else{
			...
			formatter: function(value, row, index) {
				...
				if(bidFileUrlS == 'PNG'||bidFileUrlS == 'JPG'||bidFileUrlS == 'JPGE'||bidFileUrlS == 'PDF'){
					return btn1+btn2;
				}else{
					return btn1;
				}
