...
var pageAskEdit = "Review/dfJudgeCheck/model/askEdit.html";  //提出质疑页面
var SupplierAllList,supplierArr,supplierArrId,clarifyTimer,isLeaders,isFinishc,datastate=1;
//澄清获取投标人列表
function clarify(){
    var flag = false;
	$.ajax({
...
function supplierAllHtml(){
	...
	for(var i=0;i<SupplierAllList.length;i++){
		if(i==0){
			list+='<li role="presentation" class="active askBidder" data-bidderId="'+SupplierAllList[i].supplierId+'"><a href="#" data-toggle="tab">'+ showBidderRenameMark(SupplierAllList[i].supplierId, SupplierAllList[i].enterpriseName, RenameData, 'content')  +'</a></li>'
		}else{
			list+='<li role="presentation" class="askBidder" data-bidderId="'+SupplierAllList[i].supplierId+'"><a href="#" data-toggle="tab">'+ showBidderRenameMark(SupplierAllList[i].supplierId, SupplierAllList[i].enterpriseName, RenameData, 'content')  +'</a></li>'
		}							
	};			
	list+='</ul>'
...
//	html1+='<a type="button" class="btn btn-primary btn-strong" style="margin-left:5px;" href="javascript:void(0)" id="btnRefurbish">刷新</a>'
	...
	for(var i=0; i < supplierArr.length; i++){
		html += '<table class="table table-bordered" style="width: 80%;">'
				+'<tr><td class="th_bg">提问内容</td><td><pre>'+supplierArr[i].askContent+'</pre><td class="th_bg">提问时间</td><td style="width:220px;text-align:center;">'+supplierArr[i].askDate+'</td></td>'
				+'</tr>'
				+'<tr><td class="th_bg">提问附件</td><td colspan="3">';
		...
		html+= '<tr><td class="th_bg">答复内容</td><td colspan="3">'+(supplierArr[i].fileUrl ? "<button type='button' class='btn btn-primary btn-sm answerContent' data-pdf='"+supplierArr[i].fileUrl+"' style='margin-right:5px;cursor:pointer;text-decoration:none'><span class='glyphicon glyphicon-eye-open'></span>查看</button>" : "")+'</td></tr>'
			+'<tr><td class="th_bg">答复附件</td><td colspan="3">';
	...
	$("#btnSend").click(function(){
        top.layer.open({
			...
			area: ['1000px','600px'],
			resize: false,
			content: pageAskEdit + "?packageid=" + bidSectionId + "&supplierid=" + supplierArrId + "&examtype=" + examType,
	...
	$(".btnRecall").on("click", function(){
        ...
        top.layer.confirm('确定是否撤回该提问吗？',{icon:7,title:'询问'},
		function(index){
			retractExpertAsk(id);
    	},function(index) {
	...
	$(".btnConfirm").on("click", function(){
		var id = $(this).attr("data-id");
        top.layer.confirm('确定是否发送该提问吗？',{icon:7,title:'询问'},
		function(index){
			checkRaterAsk(id);
    	},function(index) {
                top.layer.close(index);
...
function showImages(obj) {
	parent.openPreview(obj, "1000px", "600px");
}
//下载
