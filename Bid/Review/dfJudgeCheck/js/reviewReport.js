...
var getSignSiteUrl = top.config.Reviewhost + '/BidCheckReportController/getSignSite.do';//获取评标报告签章相关信息
var signatureUrl = top.config.Reviewhost + '/PdfService.do';//签完章后的保存路径
var reportUe, reportUeId;
var expertReportSign;
var checkReportItems = [];//可修改的评标报告列表

var isCloseSign = true;//签名页面是否已关闭
var singReportNode; //评标报告节点
	...
	$.ajax({
	    ...
	    success: function(rsp) {
	        if(rsp.success) {
	            ...
	            top.layer.alert('温馨提示：'+rsp.message);
	        }
	    }
	});
	// return expertReportSign
}
...
function showImage(filePath) {
	parent.openPreview(filePath, "100%", "100%");
}
//下载
