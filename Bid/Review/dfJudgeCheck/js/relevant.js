...
function ipshowlist(showlist){
	...
	var html='<p style="color:red">表中“/”表示无值，红字标识信息为系统检测到不同投标人的重复数据（仅供参考），绿字标识信息为东风咨询有限公司对外提供服务的公用电脑或公用外网IP地址数据。</p>'
	...
	// 	html += '<p style="color:red">根据《东风汽车集团有限公司电子采购招投标过程信息监管规定》，有下列情形的，应认定为串通投标行为：1.电子交易系统CA证书号相同的；2.下载或者上传采购招投标文件的IP地址相同的；3.MAC地址相同的；4.硬盘序列号相同的；5.施工项目工程造价软件锁序列号相同的；6.法律法规或公司制度规定的其它串通投标情形。</p>'
 //    }
	$("#ipshowlist").html(html);
//						<p>外网IP:10.192.168.1，东风咨询（武汉）工程有限公司外网地址</p>
}
