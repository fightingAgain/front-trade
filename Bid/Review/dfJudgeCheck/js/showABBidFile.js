		...
		success:function(res){
			if(res.success){
				...
				for(var m = 0;m<lists.length;m++){
					if(lists[m].bidFileType == 'ABZW'){
						bidOpeningFilesList.push(lists[m]);
					}else if(lists[m].bidFileType == 'ABFJ'){
...
function bidderList(supplierdata){
	var list='<ul class="nav nav-tabs" style="margin:5px auto">';
   	for(var i=0;i<supplierdata.length;i++){
		...
		var ABenterpriseName = supplierdata[i].isBlind ==1?supplierdata[i].enterpriseName:showBidderRenameMark(supplierdata[i].supplierId, supplierdata[i].enterpriseName, RenameData, 'filesContent')
   		if(supplierId && supplierId != 'undefined'){
			list+='<li role="supplier" class="'+(supplierdata[i].supplierId == supplierId?"active":"")+'"><a onclick=getAllFill(\"'+ supplierdata[i].supplierId +'\") href="#supplier"  ria-controls="supplier" role="tab" data-toggle="tab">'+  ABenterpriseName +'</a></li>'
			
   		}else{
			if(i==0){
				list+='<li role="supplier" class="active"><a onclick=getAllFill(\"'+ supplierdata[i].supplierId +'\") href="#supplier"  ria-controls="supplier" role="tab" data-toggle="tab">'+ ABenterpriseName  +'</a></li>'
			}else{
				list+='<li role="supplier" class=""><a onclick=getAllFill(\"'+ supplierdata[i].supplierId +'\") href="#supplier"  ria-controls="supplier" role="tab" data-toggle="tab">'+ ABenterpriseName  +'</a></li>'
			}
		}
	...
	if(supplierId && supplierId != 'undefined'){
		...
		getAllFill(supplierdata[0].supplierId)
	}
}
