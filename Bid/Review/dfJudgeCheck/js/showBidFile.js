...
var zbwjAndTbwjListUrl=config.Reviewhost+"/ReviewControll/findZbwjAndABTbwjList";//相关文件数据接口
		...
		success:function(res){
			if(res.success){
				...
				for(var m = 0;m<lists.length;m++){
					if(lists[m].bidFileType == 'ZW'|| lists[m].bidFileType =='ABZW'){
						bidOpeningFilesList.push(lists[m]);
					}else if(lists[m].bidFileType == 'FJ'||lists[m].bidFileType == 'ABFJ'){
...
function bidderList(supplierdata){
	var list='<ul class="nav nav-tabs" style="margin:5px auto">';
   	for(var i=0;i<supplierdata.length;i++){
		...
		var changeEnterpriseName = showBidderRenameMark(supplierdata[i].supplierId, supplierdata[i].enterpriseName, RenameData, 'filesContent')
   		if(supplierId && supplierId != 'undefined'){
			list+='<li role="supplier" class="'+(supplierdata[i].supplierId == supplierId?"active":"")+'"><a onclick=getAllFill(\"'+ supplierdata[i].supplierId +'\") href="#supplier"  ria-controls="supplier" role="tab" data-toggle="tab">'+ changeEnterpriseName  +'</a></li>'
			
   		}else{
			if(i==0){
				list+='<li role="supplier" class="active"><a onclick=getAllFill(\"'+ supplierdata[i].supplierId +'\") href="#supplier"  ria-controls="supplier" role="tab" data-toggle="tab">'+ changeEnterpriseName  +'</a></li>'
			}else{
				list+='<li role="supplier" class=""><a onclick=getAllFill(\"'+ supplierdata[i].supplierId +'\") href="#supplier"  ria-controls="supplier" role="tab" data-toggle="tab">'+ changeEnterpriseName  +'</a></li>'
			}
		}
	...
	if(supplierId && supplierId != 'undefined'){
		...
		getAllFill(supplierdata[0].supplierId, true)
	}
}
