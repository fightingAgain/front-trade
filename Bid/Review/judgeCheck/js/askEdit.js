...
$(function() {
	...
	if(getUrlParam("isPublicProject") && getUrlParam("isPublicProject") != "undefined") {
		...
		if(isPublicProject == 1){
			$('.isPublicProject').show();
			$('.notPublicProject').hide();
		}else{
			$('.isPublicProject').hide();
			$('.notPublicProject').show();
		}
	}	
	...
	if(!fileUploads){
		fileUploads = new StreamUpload("#fileContent",{
			basePath:"/"+enterpriseId+"/"+packageId+"/507",
			businessId: "",
			status:4,
	...
	function modelCreat(para){
		$.ajax({
			...
			success:function(data){
				if(data.success){
	//				if(para.tempType == "clearly" || para.tempType == "clearlyApply"){
					ue.ready(function() {
						ue.setContent(data.data);
					})
					$('#askContent').val(data.data)
	//				} else {
...
$("#btnSubmit").click(function() {
	var askContent = '';
	if(isPublicProject == 1){
		askContent = ue.getContent();
	}else{
		askContent = $('#askContent').val()
	}
	if(askContent.replace(/,/g,'') == ""){
		top.layer.alert("温馨提示：请输入问题");
		return;
	};
	//提交结果   
	top.layer.confirm('温馨提示：确定是否提交？',{
		icon: 7,
		title: '询问'
	}, function() {
		/**
			...
			$.ajax({
				url: saveExpertAskUrl,
		     	...
		     	success: function (data) {
                    ...
                    top.layer.alert("温馨提示：提交成功",function(i){
                        var index = top.layer.getFrameIndex(window.name);
                        top.layer.close(i);
                        top.layer.close(index);
                    });
