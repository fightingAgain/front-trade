...
var editHtml = 'Review/reportSet/model/addReport.html';//编辑评标报告信息
		...
		},
		{
			...
			formatter:function(value, row, index){
                var html = '<button  type="button" class="btn btn-primary btn-sm btnEdit"><span class="glyphicon glyphicon-edit"></span>编辑</button>';
                    html += '<button  type="button" class="btn btn-primary btn-sm btnEditPart"><span class="glyphicon glyphicon-edit"></span>编辑章节</button>';
        		return html;
            }
		}]
    });
...
function openEdit(checkReportCode) {
    var  contentPath = checkReportCode? editHtml+'?checkReportCode=' + checkReportCode :  editHtml
    top.layer.open({
        type: 2,
        title: '评标报告设置信息',
        area: ['500px', '400px'],
...
function queryParams(params) {
    return {
        ...
        'checkReportName': $('#checkReportName').val(),//评标报告名称
        'checkReportType': $('#checkReportType').val(),//评标报告类型
    }
}
