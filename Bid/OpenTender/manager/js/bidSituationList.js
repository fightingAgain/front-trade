	var bidpageUrl = config.OpenBidHost  + "/BidOpeningSituationController/findSituationPageList.do"; //标段分页查询接口
	var fileDownloadUrl = config.OpenBidHost + '/OpeningRecordController/createOpeningRecord.do'; //下载开标记录
					...
					events:{
						'click .btn_entry':function(e,value, row, index){
							...
							if(row.owner && row.owner == 1){//权限  是否有操作权限1是  0否
								openHtml = 'OpenTender/manager/model/bidRoom.html?id=' + row.id;
							}else{
								openHtml = 'OpenTender/manager/model/bidRoomVG.html?bid=' + row.id;
							}
							top.layer.open({
								type: 2,
								area: ['100%', '100%'],
