...
var addReplyPopHtml = 'Bidding/ObjectionManage/Manager/model/addReplyPop.html';//新增回复
var replyUrl = config.tenderHost + "/ObjectionAnswersReplyController/list.do"; // 查询回复信息
var examType = 2; // 资格审查方式  预审还是后审
var baseInfo = {};
...
$(function(){
	...
	$("#btnAddReply").click(function () {
		parent.layer.open({
			...
			title: '新增回复',
			area: ['80%', '60%'],
			content: addReplyPopHtml + '?dataId=' + dataId + '&state=' + status + '&examType=' + examType + '&bidId=' + bidId+ '&isShowResult=' + isShowResult,
			resize: true,
...
function getDetail(id){
	$.ajax({
		type:"post",
		url:config.OpenBidHost+'/ObjectionAnswersController/getAndFile.do',
		async:true,
		data: {
			'id': id
		},
...
function doCanReply() {
	...
	$.ajax({
		type: "get",
		url: canReplyUrl + '?bidSectionId=' + bidId + '&examType=' + examType,
		async: true,
		success: function (data) {
			if (data.success) {
				var res = data.data;
