...
function saveDecrypt(){
	...
	if($('#select').is(':checked')) {
		...
		if(letterDecrypt == ''){
			...
			var urlDecrypt = config.OpenBidHost + '/FileOpenDetailsController/addEnvelopeDecrypt'; //解密路径
		...
		CAcf.decrypt(keytext).then(function (user){
			if(user){
				var parameters = {"bidOpeningId":opid,"bidSectionId":bidId,"keyText":user.decryptedData,"fileUrl":fileUrl,"signedData":user.caDn,"examType":1};
				var urlDecrypt = config.OpenBidHost + '/FileOpenDetailsController/addBidderCFCADecrypt';
