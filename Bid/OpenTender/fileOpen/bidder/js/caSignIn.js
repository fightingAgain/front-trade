function caSignIn(){
	let	CAcf = new CA({
		confirmCA:function(flag, user){
			...
			var params = {"bidSectionId": bidId,"examType":1,"signedData": user.signedData,"random":user.strsource};

			//执行签到
			if($.trim(user.signedData) != '') {
				$.ajax({
					type: "post",
					url: config.OpenBidHost + "/FileOpenSignInController/addBidderCfCASignIn",
					async: false,
					data:params,
					dataType: "json",
