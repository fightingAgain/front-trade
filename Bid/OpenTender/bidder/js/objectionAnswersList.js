...
$(document).ready(function(){
	...
	$("#btnSave").click(function(){
		top.layer.open({
			...
			title: '新增异议',
			area: ['1200px', '600px'],
			content: 'OpenTender/bidder/model/objectionEdit.html?bidSectionId='+bidSectionId,
			resize: false,
			success: function(layero, idx) {
				var iframeWin = layero.find('iframe')[0].contentWindow;	
				iframeWin.passMessage(listTable);  //调用子窗口方法，传参
			}
		});
	});
});
...
function listTable(){
	$.ajax({
		type: "get",
		url: config.OpenBidHost+"/ObjectionAnswersController/findBidderGroupList",
		data:getQueryParams(),
		async: false,
		...
		success: function(rsp) {
			...
			if(rsp.data.length>0){
				rsp.data.forEach(function(item,index){
					...
					var strBack = '<button  type="button" class="btn btn-primary btn-sm" onclick="openBack(\''+item.id+'\')"><span class="glyphicon glyphicon-repeat"></span>撤回</button>';
					...
					switch(parseInt(item.status)) {
				    	...
				    	case 8: str = "撤回"; btn = strView;break;
					...
					var tr = "<tr>"
						   ...
						   + "<td>" + (item.objectionTitle == undefined ? '' : item.objectionTitle)+ "</td>"
...
function openEdit(id){
	top.layer.open({
		...
		area: ['1200px', '600px'],
		resize: false,
		content: 'OpenTender/bidder/model/objectionEdit.html?dataId=' + id,
		success: function(layero, idx) {
			var iframeWin = layero.find('iframe')[0].contentWindow;	
			iframeWin.passMessage(listTable);  //调用子窗口方法，传参
		}
...
function openView(id,state){
	top.layer.open({
		...
		title: '查看异议',
		area: ['1200px', '600px'],
		content: 'OpenTender/bidder/model/objectionView.html?dataId='+id + '&state=' + state,
		resize: false,
	});
};
...
//删除
function cutOff(id){
	layer.confirm('确定删除该异议?', {icon: 3, title:'提示'}, function(index){
		layer.close(index);
		$.ajax({
			type:"post",
			url:config.OpenBidHost + "/ObjectionAnswersController/deleteByHard.do",
			async:true,
			data: {
				'id': id
			},
...
function openBack(id){
	layer.alert('是否要撤回申请？',{icon:3,title:'询问'},function(ask){
		...
		layer.prompt({
			...
			$.ajax({
				type:"post",
				url:config.OpenBidHost + '/ObjectionAnswersController/revocation.do',
				async:true,
				data: {
					'id': id,
					'instructions': value
